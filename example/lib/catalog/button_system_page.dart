...
final buttonSystemPage = CatalogPageData(
  ...
  title: 'Button system',
  description:
      '緊湊高度 28／32／36／40／48px；Primary 使用 semiBold，並依實際背景亮度在 160 切換淺／深色字。',
  icon: KlpIcons.check,
  specimens: const [],
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        KlpMasonryGrid(
          children: [
            for (final tone in KlpButtonTone.values) _ToneCard(tone: tone),
          ],
        ),
...
class _ToneCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return KlpSurface(
      ...
      padding: EdgeInsets.all(space.contentInset),
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: space.contentStackGap),
          Wrap(
            spacing: space.actionGap,
            runSpacing: space.contentStackGap,
            children: [
              _StateSample(label: 'Rest', button: _button('Rest')),
              _StateSample(label: 'Hover', button: _button('Hover me')),
              _StateSample(
                label: 'Selected',
                button: _button('Selected', selected: true),
              ),
              _StateSample(
                label: 'Disabled',
                button: KlpButton(
