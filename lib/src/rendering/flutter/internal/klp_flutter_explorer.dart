...
final class _KlpFlutterExplorerState extends State<KlpFlutterExplorer> {
	...
	void didUpdateWidget(KlpFlutterExplorer oldWidget) {
		...
		if (_dropPreview != null && !content.canDrop(_dropPreview!)) { _dropPreview = null; }
	...
	void _primary(KlpExplorerItemSnapshot item) {
		switch (item.capabilities.primaryAction) {
			case KlpExplorerPrimaryAction.activate: content.onActivate?.call(item.id);
	...
	void _toggle(KlpExplorerItemSnapshot item) {
		...
		content.onExpandedChanged?.call(item.id, !content.tree.expandedIds.contains(item.id));
	}
	...
	void _click(KlpExplorerItemSnapshot item) {
		...
		if (item.capabilities.selectable && scope.mode != KlpExplorerSelectionMode.none) {
			...
			content.onSelectionChanged?.call(KlpExplorerSelectionChange(scopeId: scope.id, selectedIds: next, anchorId: anchor));
		}
	...
	void _menu(BuildContext context, KlpExplorerItemSnapshot item, Offset anchor) {
		...
		unawaited(showKlpCommandMenu(context, item.row.contextActions.map(KlpBoundWorkspaceCommand.fromCommand).toList(), anchor, _commandStyle, active));
	}
	...
	KeyEventResult _key(BuildContext context, KlpExplorerItemSnapshot item, KeyEvent event) {
		...
		if (key == LogicalKeyboardKey.arrowDown || key == LogicalKeyboardKey.arrowUp) {
			...
			if (command != null) { unawaited(runKlpCommand(context, KlpBoundWorkspaceCommand.fromCommand(command), _commandStyle, content.isActive)); }
		}
	...
	void _previewDrop(KlpExplorerDropRequest? request) {
		final next = request != null && content.canDrop(request) ? request : null;
	...
	Widget build(BuildContext context) {
		...
		return Localizations.override(context: context, delegates: const [DefaultMaterialLocalizations.delegate], child: Column(
			...
			children: [for (final (index, id) in content.tree.visibleIds.indexed) ...[
				...
				Builder(key: ValueKey(id), builder: (rowContext) => _withDropLine(id, _row(rowContext, content.snapshot.items[id]!), marker)),
			]],
		));
	}
	...
	Widget _row(BuildContext context, KlpExplorerItemSnapshot item) {
		...
		final disclosure = expandable ? Semantics(label: '${expanded ? content.collapseLabel : content.expandLabel} ${item.row.title}', button: true, child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _toggle(item), child: arrow)) : ExcludeSemantics(child: arrow);
				...
				child: Semantics(
					...
					child: GestureDetector(
						...
						child: Padding(padding: EdgeInsetsDirectional.only(start: depth * content.indent.value), child: SizedBox(height: extent, child: Row(children: [
							...
							for (final command in item.row.inlineActions) Semantics(label: command.label, button: true, enabled: command.enabled, child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: !command.enabled ? null : () => unawaited(runKlpCommand(context, KlpBoundWorkspaceCommand.fromCommand(command), _commandStyle, content.isActive)), child: ConstrainedBox(constraints: BoxConstraints(minWidth: content.actionExtent.value), child: Padding(padding: EdgeInsets.symmetric(horizontal: content.inset.value), child: Text(command.label, style: _text.copyWith(color: klpFlutterColor(command.enabled ? content.foreground : content.mutedForeground))))))),
							SizedBox(width: content.inset.value),
						]))),
					),
				),
	...
	Widget _drag(BuildContext context, KlpExplorerItemSnapshot item, Widget row) {
		if (content.onDrop == null) { return row; }
		...
		final target = DragTarget<Set<KlpId>>(
			...
			onAcceptWithDetails: (details) {
				_previewDrop(null);
				content.onDrop!(request(details.data, details.offset));
			},
