...
final class KlpFlutterRetainedStack extends StatefulWidget {
	...
	@override
	State<KlpFlutterRetainedStack> createState() => _KlpFlutterRetainedStackState();
}

final class _KlpFlutterRetainedStackState extends State<KlpFlutterRetainedStack> {
	...
	void _updateScopes(bool changed) {
		...
		for (final entry in _scopes.entries) {
			if (primary != null && primary.ancestors.contains(entry.value)) _remembered[entry.key] = primary;
		}
		final retained = widget.content.pages.map((page) => page.id).toSet();
		for (final id in _scopes.keys.where((id) => !retained.contains(id)).toList()) {
		...
		WidgetsBinding.instance.addPostFrameCallback((_) {
			if (!mounted || generation != _generation || widget.content.activeId != activeId) return;

			final previous = _remembered[activeId];
			if (previous != null && previous.context?.mounted == true && previous.canRequestFocus && previous.ancestors.contains(scope)) {
	...
	Widget build(BuildContext context) {
		...
		return Stack(fit: StackFit.expand, children: [
			for (final page in widget.content.pages) _page(page),
		]);
	}

	Widget _page(KlpBoundPlacement page) {
		final active = page.id == widget.content.activeId;
		final focus = FocusScope.withExternalFocusNode(focusScopeNode: _scopes[page.id]!, child: KlpFlutterRenderer(content: page));
