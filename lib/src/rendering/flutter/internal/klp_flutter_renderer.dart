...
final class KlpFlutterRenderer extends StatelessWidget {
	...
	KlpFlutterRenderer({required this.content, Key? key}) : super(key: key ?? _placementKey(content));
	...
	Widget build(BuildContext context) {
		...
		return switch (content) {
			...
			KlpBoundText value => Text(value.text, style: klpFlutterTextStyle(value.style)),
	...
	Widget _surface(KlpBoundSurface value) {
		...
		final decoration = BoxDecoration(color: klpFlutterColor(value.background), borderRadius: radius);
		final child = Padding(padding: EdgeInsets.all(value.inset.value), child: KlpFlutterRenderer(content: value.child));
		return ClipRRect(borderRadius: radius, child: DecoratedBox(decoration: decoration, child: child));
	}
}
