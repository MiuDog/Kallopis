...
final class KlpFlutterWorkspaceBlock extends StatelessWidget {
	...
	final void Function()? activation;
	final FocusNode? focusNode;
	final bool? expanded;
	final bool? selectedOverride;

	const KlpFlutterWorkspaceBlock({required this.content, this.activation, this.focusNode, this.expanded, this.selectedOverride, super.key});
	...
	/// 工具列只提供共用 header 高度與正方形命中區，表面及內外距由外層群組負責。
	Widget _toolbar() => SizedBox(height: content.headerExtent.value, child: Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: content.toolbarAlignment == 1 ? MainAxisAlignment.end : MainAxisAlignment.start, children: [
		for (final (index, item) in content.items.indexed) ...[
			if (index > 0) SizedBox(width: content.compactGap.value),
			Tooltip(message: item.title, child: _interactive(item.title, item.onPressed, SizedBox.square(dimension: content.headerExtent.value, child: Center(child: item.icon == null ? const SizedBox.shrink() : _icon(item.icon!))), selected: item.selected)),
		],
	]));
	...
	KlpFlutterCommandStyle get _commandStyle => KlpFlutterCommandStyle(
		surface: klpFlutterColor(content.background), foreground: klpFlutterColor(content.foreground), muted: klpFlutterColor(content.mutedForeground), interaction: klpFlutterColor(content.selectedBackground), destructive: klpFlutterColor(content.accentColor), text: _style(), extent: content.rowExtent.value, inset: content.compactGap.value, radius: content.radius.value,
	);

	Widget _commandButton(BuildContext context) => Localizations.override(context: context, delegates: const [DefaultMaterialLocalizations.delegate], child: Builder(builder: (buttonContext) => Semantics(
		label: content.actionsLabel,
		button: true,
		child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () {
			...
			bool active() => context.mounted && context.widget is KlpFlutterWorkspaceBlock && identical((context.widget as KlpFlutterWorkspaceBlock).content, content);
			unawaited(showKlpCommandMenu(buttonContext, content.actions, anchor, _commandStyle, active));
	...
	Widget _action() => _interactive(content.title, activation ?? content.onPressed, SizedBox(height: content.rowExtent.value, child: Row(children: [
		...
		focusNode: focusNode,
		expanded: expanded,
		selected: selectedOverride ?? content.selected,
	);
	...
	Widget _interactive(String label, void Function()? action, Widget child, {bool? checked, bool? selected, FocusNode? focusNode, bool? expanded}) {
		Color? background;
