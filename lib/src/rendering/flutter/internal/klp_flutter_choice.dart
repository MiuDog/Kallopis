...
final class _KlpFlutterChoiceState extends State<KlpFlutterChoice> {
	...
	void didUpdateWidget(KlpFlutterChoice oldWidget) {
		...
		if (!identical(oldWidget.content.selection, widget.content.selection) ||
	...
	Widget build(BuildContext context) {
		...
		final visual = KlpFlutterSelectionSurface(
			color: klpFlutterColor(style.selectedBackground),
			backgroundColor: klpFlutterColor(style.background),
			radius: style.radius.value, selected: _selected, focused: _focused, trackFocus: false, enabled: enabled,
			child: Center(child: KlpFlutterRenderer(content: content.child)),
		);
		final extent = SizedBox(
			height: style.extent.value,
			width: style.extent.value,
			child: visual,
		);
