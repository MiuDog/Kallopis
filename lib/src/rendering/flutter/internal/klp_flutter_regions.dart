...
final class KlpFlutterRegions extends StatelessWidget {
  ...
  Widget _layout(BuildContext context, BoxConstraints constraints) {
    ...
    final maximum = axis == Axis.vertical
        ? constraints.maxHeight
        : constraints.maxWidth;
    final required =
        content.leadingExtent.value +
        content.trailingExtent.value +
        content.minimumBodyExtent.value;
    ...
    final bodyExtent = fits
        ? maximum - content.leadingExtent.value - content.trailingExtent.value
        : null;
    final children = <Widget>[
      _extent(
        axis,
        fits ? content.leadingExtent.value : null,
        _scroll(axis, leading),
      ),
      _extent(axis, bodyExtent, _scroll(axis, body)),
      _extent(
        axis,
        fits ? content.trailingExtent.value : null,
        _scroll(axis, trailing),
      ),
    ];
    final flow = Flex(
      direction: axis,
  ...
  Widget _scroll(Axis axis, Widget child) => SingleChildScrollView(
    scrollDirection: axis,
    primary: false,
    child: child,
  );

  Widget _extent(Axis axis, double? extent, Widget child) => SizedBox(
