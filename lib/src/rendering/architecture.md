...
KP-R1 所屬精確路徑為 `flutter/internal/klp_flutter_block_note_editing.dart`、`flutter/internal/klp_block_note_web_session_loader.dart`、`flutter/internal/klp_flutter_workspace_components.dart`，以及新增 `flutter/internal/klp_block_note_flow_transport.dart`、`flutter/internal/klp_page_reference_drag.dart`。這是 module packet 輸入，非已派工授權。
...
| `REND-V1-06` | 呈現 Sidebar v2 Explorer 的 branch、不可選取結構列及內容選單專用政策。 | `flutter/internal/klp_flutter_workspace_components.dart` | Features 提供 `SIDEBAR-V2-EXPLORER-r1` 已準備資料；rendering 不推導產品頁面型別。 | branch 的展開／選取、結構列不選取、無尾端按鈕及三種內容選單鍵盤／指標入口通過局部互動測試。 | READY |
...
`REND-V1-06` 與 features 的配對契約及精確邊界見 [Sidebar v2 Explorer 配對計畫](../../../docs/architecture/sidebar-v2-explorer-plan/README.md)。
