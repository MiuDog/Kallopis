...
class KlpAccessibilityContractPanel extends StatelessWidget {
	...
	Widget build(BuildContext context) => KlpSurface(
		...
		child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [KlpText(title, role: KlpTextRole.bodyStrong), SizedBox(height: context.klp.space.contentStackGap), for (final item in items.entries) Padding(padding: EdgeInsets.only(bottom: context.klp.space.tight), child: Row(children: [Expanded(child: KlpText(item.key, tone: KlpTextTone.muted)), KlpText(item.value, role: KlpTextRole.code)]))]),
	);
}
