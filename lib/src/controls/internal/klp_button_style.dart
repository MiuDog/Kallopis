...
import '../button/klp_button_types.dart';
	...
	factory KlpButtonStyle.resolve({
		...
		if (!disabled) {
			background = switch (tone) {
				...
				KlpButtonTone.danger => tokens.danger.withValues(alpha: klp.surface.statusFillOpacity),
			};
		}
		...
		if (selected && !disabled) {
			final selectedColor = tone == KlpButtonTone.primary ? klp.onPrimary : tokens.interaction;
			...
			if (tone == KlpButtonTone.primary) {
				stateWash = klp.onPrimary.withValues(alpha: klp.surface.selectionWashOpacity);
			}
		}
		...
		if (!disabled) {
			foreground = switch (tone) {
				...
				KlpButtonTone.secondary || KlpButtonTone.ghost || KlpButtonTone.dashed => tokens.text,
			};
		}

		// 五段尺寸保留既有映射，MD 的水平內距仍經 component override 解析。
		final (height, paddingX, textRole) = switch (size) {
			KlpControlSize.xs => (klp.buttonHeightXSmall, klp.space.controlInset, KlpTextRole.caption),
			KlpControlSize.sm => (klp.buttonHeightSmall, klp.space.controlPaddingXSmall, KlpTextRole.caption),
			KlpControlSize.md => (klp.buttonHeight, klp.buttonInsets.left, KlpTextRole.body),
			KlpControlSize.lg => (klp.buttonHeightLarge, klp.space.controlPaddingXLarge, KlpTextRole.lead),
			KlpControlSize.xl => (klp.buttonHeightXLarge, klp.space.controlPaddingXXLarge, KlpTextRole.lead),
		};
		...
		final progressBase = tone == KlpButtonTone.primary ? klp.onPrimary : tokens.interactionSoft;
		return KlpButtonStyle._(
			...
			progressColor: progressBase.withValues(alpha: klp.surface.pressProgressOpacity),
