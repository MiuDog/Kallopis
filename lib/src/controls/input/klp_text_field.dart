...
class _KlpTextFieldState extends State<KlpTextField> {
  ...
  Widget build(BuildContext context) {
    ...
    final isInvalid = hasError || widget.conflict;
		...
		int? maxLines = 1;
		if (widget.multiline) {
			minLines = widget.minLines ?? klp.geometry.control.textFieldMinLines;
			if (widget.unboundedLines) {
				maxLines = null;
			}
			else {
				maxLines = widget.maxLines ?? klp.geometry.control.textFieldMaxLines;
			}
		}
    final fillColor = KlpFieldStyle.inputFill(
      ...
      surface: klp.surface,
		...
		if (widget.outlined) {
			borderWidth = borderWidth.clamp(klp.shape.stroke, double.infinity).toDouble();
		}
		final borderColor = widget.outlined && _focused ? tokens.interaction : tokens.border;
