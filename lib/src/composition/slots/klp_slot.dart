...
final class KlpSlot<C extends KlpNode> {
  ...
  KlpSlot({required this.owner, required this.name, this.min = 0, this.max}) {
    if (!isKlpIdentifier(owner) || !isKlpIdentifier(name)) {
      throw const KlpContractError(
        'invalid_slot_id',
        'Slot owner and name must be identifiers.',
      );
    }
    if (min < 0 || (max != null && max! < min)) {
      throw KlpContractError('invalid_slot_cardinality', '$owner.$name');
    }
  }
  ...
  KlpSlotAssignment<C> assign(List<C> children) {
    ...
    if (snapshot.length < min || (max != null && snapshot.length > max!)) {
      throw KlpContractError('slot_cardinality', '$owner.$name');
    }
