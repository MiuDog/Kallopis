...
final class KlpDefinition<T extends KlpNode> {
  ...
  KlpDefinition(
    ...
    KlpSemanticSchema? semantics,
    Iterable<KlpSlot<KlpNode>> slots = const [],
  }) : semantics = semantics ?? KlpSemanticSchema(id, const []),
    ...
    if (this.semantics.owner != id) {
      throw KlpContractError(
        'semantic_owner_mismatch',
        '$id owns ${this.semantics.owner}',
      );
    }
    final names = <String>{};
    for (final slot in this.slots) {
      if (slot.owner != id) {
        throw KlpContractError(
          'slot_owner_mismatch',
          '$id owns ${slot.owner}.${slot.name}',
        );
      }
      if (!names.add(slot.name)) {
        throw KlpContractError('duplicate_slot', '$id.${slot.name}');
      }
    }
  }
