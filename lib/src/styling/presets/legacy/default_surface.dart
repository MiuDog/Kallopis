part of '../../legacy_theme/klp_surface_theme.dart';

/// 此分類的預設風格 recipe；schema 的公開 static 成員保留相同值與 const 契約。

const KlpSurfaceTheme _defaultSurface = KlpSurfaceTheme(
	separation: KlpSurfaceSeparation.shadow,
	overlayBlur: KlpPaperShadowRecipe.ambientBlur,
	overlaySpread: 0,
	overlayOffsetY: KlpPaperShadowRecipe.ambientOffsetY,
	overlayShadowOpacity: KlpPaperShadowRecipe.alpha / 255,
	contactBlur: KlpPaperShadowRecipe.contactBlur,
	contactOffsetY: KlpPaperShadowRecipe.contactOffsetY,
	contactShadowOpacity: KlpPaperShadowRecipe.alpha / 255,
	overlayShadowColor: _defaultOverlayShadowColor,
	scrimOpacity: 0.6,
	selectionWashOpacity: KlpScale.opacity100,
	focusWashOpacity: KlpScale.opacity180,
	statusFillOpacity: KlpScale.opacity160,
	pressProgressOpacity: KlpScale.opacity550,
	diffFillOpacity: KlpScale.opacity120,
	gridLineOpacity: KlpScale.opacity180,
	veilOpacity: KlpScale.opacity820,
	statusRowOpacity: KlpScale.opacity140,
	statusRowSelectedOpacity: KlpScale.opacity320,
	statusRowOpacityDark: KlpScale.opacity280,
	statusRowSelectedOpacityDark: KlpScale.opacity480,
	frostedOpacity: KlpScale.opacity140,
	frostedVeilOpacity: KlpScale.opacity120,
);
