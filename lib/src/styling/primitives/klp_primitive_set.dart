...
final class KlpPrimitiveSet {
	...
	T read<T extends KlpStyleValue>(KlpStyleKind<T> kind, KlpPrimitiveIndex index) {
		final KlpStyleValue value = switch (kind.name) {
			...
			_ => throw KlpContractError('unknown_primitive_kind', 'primitives.${kind.name}: unsupported kind.'),
		};
		return value as T;
	}

	static List<T> _copySlots<T extends KlpStyleValue>(List<T> values, String path) {
		final snapshot = List<T>.unmodifiable(values);
		if (snapshot.length != KlpPrimitiveIndex.values.length) {
			throw KlpContractError('invalid_primitive_slots', 'primitives.$path: expected exactly ${KlpPrimitiveIndex.values.length} slots, found ${snapshot.length}.');
		}
		return snapshot;
	}
}
