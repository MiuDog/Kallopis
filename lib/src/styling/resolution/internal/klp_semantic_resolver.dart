...
final class KlpSemanticResolver {
  ...
  KlpSemanticResolver(Iterable<KlpSemanticSchema> schemas) {
    for (final schema in schemas) {
      if (_schemas.containsKey(schema.owner)) {
        throw KlpContractError('duplicate_semantic_owner', schema.owner);
      }
  ...
  KlpSemanticResolution resolve(KlpPrimitiveSet primitives) {
    ...
    for (final token in _order) {
      final value = switch (token.reference) {
        KlpPrimitiveRef(:final kind, :final index) => primitives.read(
          kind,
          index,
        ),
  ...
  void _visitToken(
    KlpSemanticToken<KlpStyleValue> token,
    List<(String, String)> path,
    Set<(String, String)> complete,
  ) {
    ...
    if (path.contains(identity)) {
      throw KlpContractError(
        ...
        [...path, identity].map((key) => '${key.$1}/${key.$2}').join(' -> '),
      );
    }
    path.add(identity);
    final reference = token.reference;
    if (reference is KlpSemanticRef) {
      final target = _requireReference(
        token.key.owner,
        reference.key,
        token.key.path,
      );
  ...
  KlpSemanticToken<KlpStyleValue> _requireReference(
    String owner,
    KlpSemanticKey<KlpStyleValue> key,
    String source,
  ) {
    final schema = _schemas[owner];
    ...
    if (target == null) {
      throw KlpContractError('unknown_semantic', '$source -> ${key.path}');
    }
    // 引用者可建同名假 key，必須核對真正目標，不能只信來源標記。
    if (!identical(key.kind, target.key.kind)) {
      throw KlpContractError(
        'semantic_kind_mismatch',
        '$source -> ${target.key.path}',
      );
    }
    if (owner != target.key.owner) {
      if (!schema.dependencies.contains(target.key.owner)) {
        throw KlpContractError(
          'undeclared_semantic_dependency',
          '$source -> ${target.key.path}',
        );
      }
      if (!target.isPublic) {
        throw KlpContractError(
          'private_semantic_reference',
          '$source -> ${target.key.path}',
        );
      }
    }
