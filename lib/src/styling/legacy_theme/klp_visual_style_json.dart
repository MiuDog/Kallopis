...
import 'internal/klp_visual_style_json_migration.dart';
...
abstract final class KlpVisualStyleJson {
	...
	static KlpVisualStyle decode(
		...
		json = migrateLegacySpacing(json, base);
		json = migrateLegacyDimensions(json);
	...
	static void _validateSchemaVersion(KlpJsonMap json) {
		...
		if (value is! int || (value != 1 && value != 2 && value != schemaVersion)) {
			jsonError('schemaVersion', 'must be integer 1, 2 or $schemaVersion');
		}
	}
}
