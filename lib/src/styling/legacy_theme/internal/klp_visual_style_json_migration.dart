...
KlpJsonMap migrateLegacySpacing(KlpJsonMap json, KlpVisualStyle base) {
  ...
  final compact = hasCompact
      ? readNonNegativeDouble(original, 'compact', 'spacing', 0)
      : base.spacing.navigationSectionGap + base.spacing.hairline;
  final hairline = readNonNegativeDouble(
    original,
    'hairline',
    'spacing',
    base.spacing.hairline,
  );
  ...
  spacing.putIfAbsent(
    ...
    () => compact > hairline ? compact - hairline : 0.0,
  );
  ...
  final control = <String, Object?>{
    ...?readMap(geometry, 'control', 'geometry'),
  };
...
KlpJsonMap migrateLegacyDimensions(KlpJsonMap json) {
  ...
  if (!original.keys.any(
    (key) => controlKeys.contains(key) || layoutKeys.contains(key),
  )) {
    return json;
  }
