...
part '../presets/legacy/default_surface.dart';

const Color _defaultOverlayShadowColor = KlpPalette.ink1000;
...
class KlpSurfaceTheme extends ThemeExtension<KlpSurfaceTheme> {
	const KlpSurfaceTheme({
		...
		this.contactBlur = 0,
		this.contactOffsetY = 0,
		this.contactShadowOpacity = 0,
		this.overlayShadowColor = _defaultOverlayShadowColor,
	});
	...
	/// 接觸陰影與環境陰影共用色彩，透明度可各自調整。
	final double contactBlur;
	final double contactOffsetY;
	...
	/// 依分層手法產生浮層陰影；非陰影風格回傳空清單。
	/// 舊呼叫端的前景色參數保留相容，實際陰影色由主題負責。
	List<BoxShadow> overlayShadow(Color shadowColor) {
		if (!usesShadow) return const <BoxShadow>[];
		return [
			if (contactShadowOpacity > 0)
				BoxShadow(
					color: overlayShadowColor.withValues(alpha: contactShadowOpacity),
					blurRadius: contactBlur,
					offset: Offset(0, contactOffsetY),
				),
			BoxShadow(
				color: overlayShadowColor.withValues(alpha: overlayShadowOpacity),
	...
	KlpSurfaceTheme copyWith({
		...
		return KlpSurfaceTheme(
			...
			contactBlur: contactBlur ?? this.contactBlur,
			contactOffsetY: contactOffsetY ?? this.contactOffsetY,
			contactShadowOpacity: contactShadowOpacity ?? this.contactShadowOpacity,
	...
	bool operator ==(Object other) =>
			...
			other is KlpSurfaceTheme &&
					...
					contactBlur == other.contactBlur &&
					contactOffsetY == other.contactOffsetY &&
					contactShadowOpacity == other.contactShadowOpacity &&
