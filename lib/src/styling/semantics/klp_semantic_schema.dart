...
final class KlpSemanticSchema {
  ...
  KlpSemanticSchema(
    this.owner,
    Iterable<KlpSemanticToken<KlpStyleValue>> tokens, {
    ...
    for (final token in this.tokens) {
      if (token.key.owner != owner) {
        throw KlpContractError(
          'semantic_owner_mismatch',
          '$owner contains ${token.key.path}',
        );
      }
      if (!names.add(token.key.name)) {
        throw KlpContractError('duplicate_semantic', token.key.path);
      }
    }
    final unique = <String>{};
