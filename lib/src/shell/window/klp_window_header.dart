...
/// Header 版面占位高度；windowHeaderMargin 僅作用於左右 padding。
double klpWindowHeaderHeight(
...
class KlpWindowHeader extends StatelessWidget implements PreferredSizeWidget {
	...
	Widget build(BuildContext context) {
		...
		final isMac = klpUsesMacOSWindowChrome(context, platform: platform);
		final controlsStyle = isMac
		...
		final extras = actions == null && trailing == null
				...
				: buildKlpWindowHeaderRegion(
						...
						children: [
							if (actions != null) ...[
								...actions!,
								SizedBox(width: klp.space.chromeToolbarGap),
							],
							if (trailing != null) ...[
								trailing!,
								SizedBox(width: klp.space.chromeToolbarGap),
					...
					child: buildKlpWindowHeaderContent(
						identity: Row(
							children: [
								...
								Expanded(
									child: _buildDoubleTapRegion(child: const SizedBox.expand()),
								),
							],
						),
						extras: extras,
			...
			children: [
				...
				Align(
					...
					child: Row(
						...
						children: [
							_buildDoubleTapRegion(child: identity),
							if (titleTrailing != null) ...[
								SizedBox(width: klp.geometry.layout.windowIdentityGap),
								titleTrailing!,
							],
	...
	Widget _buildDoubleTapRegion({required Widget child}) {
		return GestureDetector(
			behavior: HitTestBehavior.translucent,
