...
final class KlpRoute<P, R> {
  ...
  KlpRoute(
    KlpDestination<P, R> destination, {
  ...
  KlpRoutePolicy get _policy => KlpRoutePolicy(
  ...
  KlpScreen _project(KlpNavigationEntry entry, _KlpRouteActions actions) {
    if (!identical(entry.location.destination, destination)) {
      throw ArgumentError('Route destination identity mismatch.');
    }
    final parameters = entry.location.parameters;
    if (!destination.acceptsParameters(parameters)) {
      throw ArgumentError('Invalid route parameters.');
    }
