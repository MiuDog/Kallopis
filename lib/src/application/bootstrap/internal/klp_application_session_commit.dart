...
extension _KlpApplicationSessionCommit on _KlpApplicationSession {
	...
	void _refresh(KlpApplication next) {
		...
		if (!identical(initial.destination, oldInitial.destination) || initial.parameters != oldInitial.parameters) {
		...
		on KlpNavigationCommitException catch (error) {
			...
			// 保留原始堆疊，讓宿主取得實際提交失敗的位置。
			Error.throwWithStackTrace(error.cause, error.stackTrace);
		}
		machine.replacePolicies(next.router.routes.map((route) => route._policy));
	}

	void _commit(KlpApplication application, KlpNavigationSnapshot snapshot, {required bool replaceRouteInformation}) {
		// 封存既有影格並標記投影期間，避免未完成的啟動操作提前生效。
		...
		try {
			...
			for (final entry in snapshot.entries) {
				...
				screens[entry.id] = route._project(entry, _KlpSessionRouteActions(this, epoch, entry));
			}
			final root = KlpScopeBoundary(id: application.router.id, child: KlpRetainedScreens(id: application.router.id / 'retained', activeEntry: snapshot.current.id, screens: screens));
			...
			try {
				...
				if (frame != null && !identical(frame, previous)) {
					...
					_publishRouteInformation(application, snapshot, replace: replaceRouteInformation);
					onChanged();
				}
			}
		}
		catch (error, stack) {
			final committed = !identical(previous, frame) || (error is KlpInstallationException && error.committed);
	...
	void _publishRestoration(KlpApplication application, KlpNavigationSnapshot snapshot) {
		// 依觀察者與正規 URI 排除重複還原通知。
		final callback = application.onNavigationRestorationChanged;
		if (callback == null) {
			_lastRestorationCallback = null;
			_lastRestorationUri = null;
			return;
		}
		final restoration = KlpNavigationRestoration.fromSnapshot(application.router.id.value, snapshot);
		final uri = KlpRouteUri.encodeRestoration(restoration);
		if (identical(callback, _lastRestorationCallback) && uri == _lastRestorationUri) return;

		// 只記錄成功通知的狀態，失敗仍允許後續重試。
		try {
			callback(restoration);
			_lastRestorationCallback = callback;
			_lastRestorationUri = uri;
		}
		catch (error, stack) {
	...
	void _publishRouteInformation(KlpApplication application, KlpNavigationSnapshot snapshot, {required bool replace}) {
		if (!application.router.supportsRestoration) return;

		// 由已提交快照產生宿主路由資訊，失敗交由非同步錯誤出口處理。
		try {
			final restoration = KlpNavigationRestoration.fromSnapshot(application.router.id.value, snapshot);
			onRouteInformationChanged(KlpRouteUri.encodeRestoration(restoration), replace);
		}
		catch (error, stack) {
			onAsyncError(error, stack);
		}
