...
final class _KlpApplicationSession {
	final KlpFileSelectionPort fileSelection;
	...
	KlpApplication? get application => _application;
	...
	void _finishStart(KlpNavigationStart result, int generation) {
		...
		if (!result.decision.committed || frame == null) {
			_startupError = switch (outcome) {
				KlpNavigationFailed<void> failure => failure.error,
				_ => outcome,
			};
			_machine?.dispose();
			_machine = null;
