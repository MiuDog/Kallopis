...
final class KlpInstallation {
	...
	final bool Function(KlpValidatedNode, KlpPlacementResource) _canReuse;
	...
	KlpInstallation(
		this.registry, {
		KlpPlacementResource Function(KlpValidatedNode)? create,
		bool Function(KlpValidatedNode, KlpPlacementResource)? canReuse,
	}) : _create = create ?? KlpDefaultPlacement.new,
			 _canReuse = canReuse ?? ((node, resource) => true);
	...
	void _apply(KlpTreeValidation next, {void Function()? onCommitted}) {
		...
		final previousTree = _tree;
		final oldNodes = {
			for (final node in previousTree?.nodes ?? <KlpValidatedNode>[])
				node.placementId: node,
		};
		...
		try {
			for (final node in next.nodes) {
				...
				final oldResource = previous[node.placementId];
				if (old != null && old.definitionId == node.definitionId && _canReuse(node, oldResource!)) {
					pending[node.placementId] = oldResource;
				} else {
		...
		for (final node in next.nodes) {
			final old = oldNodes[node.placementId];
			if (old == null ||
					!identical(previous[node.placementId], pending[node.placementId])) {
				continue;
			}
		...
		for (final node in previousTree?.nodes.reversed ?? <KlpValidatedNode>[]) {
			...
			if (!identical(resource, pending[node.placementId])) {
				_cleanup(resource, issues);
			}
		}
		if (issues.isNotEmpty) throw KlpInstallationException(true, issues);
	...
	bool _sameChildren(KlpValidatedNode left, KlpValidatedNode right) {
		if (left.childrenPlacements.length != right.childrenPlacements.length) {
			return false;
		}

		for (var index = 0; index < left.childrenPlacements.length; index++) {
			if (left.childrenPlacements[index] != right.childrenPlacements[index]) {
				return false;
			}
		}
		return true;
	...
	void _cleanup(
		...
		List<({Object error, StackTrace stackTrace})> issues,
	) {
		try {
			resource.dispose();
		} catch (error, stackTrace) {
