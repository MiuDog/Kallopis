...
final class KlpTreeRuntime {
	...
	void update({
		...
		try {
			...
			final customIds = {for (final definition in definitions) definition.contract.id};
			final compiler = KlpComponentCompiler(definitions, sharedDefinitions: registry.definitions.where((definition) => !customIds.contains(definition.id)));
			final captured = captureKlpTree(registry, root);
			final context = KlpPrepareContext(
				sources: captured.sources,
				nodes: {for (final node in captured.validation.nodes) node.placementId: node},
				components: compiler,
				primitives: primitives,
				style: KlpSemanticResolver(registry.definitions.map((definition) => definition.semantics)).resolve(primitives),
				actionHandler: actionHandler,
			);
			final byDefinition = {for (final adapter in registered) adapter.contract.id: adapter};
			final prepared = <KlpPlacementId, KlpPreparedNode>{};
			// 外部模板資格先全樹檢查，避免較晚的模板失配落在文字投影之後。
			for (final node in captured.validation.nodes) {
				if (customIds.contains(node.definitionId)) compiler.validateCaptured(captured.sources[node.placementId]!, node);
			}
			for (final node in captured.validation.nodes) {
			...
			final leases = <KlpPlacementId, KlpFrameLease>{captured.validation.rootPlacement: lease};
			for (final node in captured.validation.nodes) {
			...
			_pending = prepared;
			final installation = _installation ??= KlpInstallation(registry, create: _create);
			...
			try {
				...
				_frame?.lease.revoke();
				_frame = null;
				throw KlpInstallationException(true, [...?committedError?.issues, (error: error, stackTrace: stackTrace)]);
			}
