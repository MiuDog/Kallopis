...
@immutable
class KlpTimelineEntry {
  const KlpTimelineEntry({
    required this.id,
    required this.title,
    ...
    this.tone,
    this.actor,
    this.status = KlpTimelineStatus.idle,
  ...
  final Color? tone;
  final String? actor;
  final KlpTimelineStatus status;
  final bool superseded;
  final bool redacted;
}

enum KlpTimelineStatus { success, failure, warning, info, idle }

@immutable
class KlpTimelineGroup {
  const KlpTimelineGroup({
    required this.id,
    required this.label,
  ...
  final String id;
  final String label;
  final List<KlpTimelineEntry> entries;
}
...
class KlpTimeline extends StatelessWidget {
  ...
  final List<KlpTimelineEntry> entries;
  final List<KlpTimelineGroup> groups;
  final String? selectedId;
  final bool hasMore;
  final ValueChanged<String>? onSelected;
  ...
  Widget build(BuildContext context) {
    final effectiveGroups = groups.isEmpty
        ? [KlpTimelineGroup(id: 'default', label: '', entries: entries)]
        : groups;
...
class _KlpTimelineRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color =
        entry.tone ??
        switch (entry.status) {
          KlpTimelineStatus.success => tokens.success,
          KlpTimelineStatus.failure => tokens.danger,
          KlpTimelineStatus.warning => tokens.warning,
          KlpTimelineStatus.info => tokens.info,
          KlpTimelineStatus.idle => tokens.textFaint,
        };
        ...
        child: IntrinsicHeight(
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: Container(
                  margin: const EdgeInsets.only(bottom: KlpSpace.sm),
                  padding: const EdgeInsets.symmetric(
...
enum KlpMetricTrend { up, down, flat }

class KlpMetric extends StatelessWidget {
  const KlpMetric({
    super.key,
    required this.label,
    required this.value,
        ...
        child: Column(
          ...
          children: [
            ...
            if (loading)
              ...
              const KlpText('No data', tone: KlpTextTone.faint)
            else
              ...
              FittedBox(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    if (unit != null) ...[
                      const SizedBox(width: KlpSpace.xs),
                      KlpText(unit!, role: KlpTextRole.caption),
                    ],
                    if (trendLabel != null) ...[
                      const SizedBox(width: KlpSpace.xs),
                      KlpText(
