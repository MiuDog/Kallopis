  ...
  List<_KlpRegisteredBinding> get _resolvedBindings {
    final result = _bindings.where((item) {
      ...
      return switch (binding.scope) {
        ...
        KlpKeyBindingScope.region ||
        KlpKeyBindingScope.component => binding.scopeId == _activeRegionId,
      };
    }).toList();
    // 先放廣 scope，再由較窄 scope 覆蓋相同按鍵。
    result.sort(
      (a, b) => b.binding.scope.index.compareTo(a.binding.scope.index),
    );
    return result;
  }
}
