...
class KlpSurface extends StatelessWidget {
	...
	Widget build(BuildContext context) {
		...
		final effectiveRadius = radius ?? (tone == KlpSurfaceTone.app ? 0 : klp.cardRadius);
		...
		final preservesExactRadius = tone == KlpSurfaceTone.transparent || tone == KlpSurfaceTone.app;
		var resolvedRadius = effectiveRadius;
		if (!preservesExactRadius) {
			resolvedRadius = effectiveRadius.clamp(klp.shape.control, double.infinity).toDouble();
		}
		...
		if (frosted && background != tokens.clear) {
			effectiveBackground = background.withValues(alpha: klp.surface.frostedOpacity);
		}
