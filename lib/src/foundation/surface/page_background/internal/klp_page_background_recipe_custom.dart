...
class KlpPageBackgroundPoint {
	...
	@override
	bool operator ==(Object other) => other is KlpPageBackgroundPoint && other.id == id && other.position == position;
	@override
...
class KlpPageBackgroundLine {
	const KlpPageBackgroundLine({required this.id, required this.startPointId, required this.endPointId});
	final int id;
...
final class KlpCustomPageBackgroundRecipe extends KlpPageBackgroundRecipe {
	KlpCustomPageBackgroundRecipe({
		...
		if (resolvedSnapSpacing != null) _requirePositiveFinite(resolvedSnapSpacing, 'snapSpacing');
	...
	KlpCustomPageBackgroundRecipe removePoint(int id) {
		return copyWith(
			points: [for (final point in points) if (point.id != id) point],
			lines: [for (final line in lines) if (line.startPointId != id && line.endPointId != id) line],
		);
	}

	KlpCustomPageBackgroundRecipe removeLine(int id) {
		return copyWith(lines: [for (final line in lines) if (line.id != id) line]);
	}
	...
	@override
	int get hashCode => Object.hash(Object.hashAll(points), Object.hashAll(lines), pointStyle, lineStyle, snapSpacing, strokeBehavior);
}

void _validateElements(List<KlpPageBackgroundPoint> points, List<KlpPageBackgroundLine> lines) {
	final pointIds = <int>{};
	for (final point in points) {
		if (point.id < 0 || !pointIds.add(point.id)) throw ArgumentError.value(point.id, 'points', 'ids must be unique');
		if (!_isFiniteOffset(point.position)) throw ArgumentError.value(point.position, 'points', 'positions must be finite');
	}

	final lineIds = <int>{};
	for (final line in lines) {
		if (line.id < 0 || !lineIds.add(line.id)) throw ArgumentError.value(line.id, 'lines', 'ids must be unique');
		if (line.startPointId == line.endPointId || !pointIds.contains(line.startPointId) || !pointIds.contains(line.endPointId)) {
...
void _requirePositiveFinite(double value, String name) {
	if (!value.isFinite || value <= 0) throw ArgumentError.value(value, name, 'must be finite and positive');
}
