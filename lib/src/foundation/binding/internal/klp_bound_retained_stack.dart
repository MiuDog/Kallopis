...
final class KlpBoundRetainedStack extends KlpBoundTemplate {
  ...
  KlpBoundRetainedStack({
    required Iterable<KlpBoundPlacement> pages,
    required this.activeId,
  }) : pages = List.unmodifiable(pages) {
    final identities = <KlpPlacementId>{};
    for (final page in this.pages) {
      if (!identities.add(page.id)) {
        throw ArgumentError('Duplicate retained page identity.');
      }
    }
    if (!identities.contains(activeId)) {
      throw ArgumentError('Active retained page must exist.');
    }
  }
}
