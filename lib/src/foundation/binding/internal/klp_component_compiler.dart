...
final class KlpComponentCompiler {
  ...
  KlpComponentCompiler(
    Iterable<KlpComponentDefinition<KlpNode>> definitions, {
    Iterable<KlpDefinition<KlpNode>> sharedDefinitions = const [],
  }) {
    final snapshot = List<KlpComponentDefinition<KlpNode>>.of(definitions);
    final registry = KlpRegistry([
      ...sharedDefinitions,
      ...snapshot.map((definition) => definition.contract),
    ]);
    _semantics = KlpSemanticResolver(
  ...
  KlpBoundComponent bind(KlpNode node, KlpPrimitiveSet primitives) {
    final snapshot = KlpValidatedNode(node.id, node.definitionId, [
  ...
  KlpBoundComponent bindCaptured(
    KlpNode node,
    KlpValidatedNode snapshot,
    KlpPrimitiveSet primitives, {
    KlpSemanticResolution? resolved,
  }) {
    if (snapshot.slotRanges.isNotEmpty ||
        (_definitions[snapshot.definitionId]?.contract.slots.isNotEmpty ??
            false)) {
      throw KlpContractError('component_requires_child_context', snapshot.id);
    }
  ...
  KlpPreparedComponent prepareCaptured(
    KlpNode node,
    KlpValidatedNode snapshot,
    KlpPrimitiveSet primitives, {
    KlpSemanticResolution? resolved,
  ...
  (KlpComponentDefinition<KlpNode>, Map<KlpSlot<KlpNode>, KlpValidatedSlot>)
  _validateCaptured(KlpNode node, KlpValidatedNode snapshot) {
    final id = snapshot.id;
    final definitionId = snapshot.definitionId;
    if (id.trim().isEmpty) {
      throw const KlpContractError('empty_id', 'component.placement');
    }

    final definition = _definitions[definitionId];
    if (definition == null) {
      throw KlpContractError('unknown_component', '$id -> $definitionId');
    }
    if (!definition.contract.accepts(node)) {
  ...
  Map<KlpSlot<KlpNode>, KlpValidatedSlot> _validateSlots(
    KlpDefinition<KlpNode> definition,
    KlpValidatedNode snapshot,
  ) {
    if (definition.slots.isEmpty && snapshot.childrenIds.isNotEmpty) {
      throw KlpContractError('component_children_unsupported', snapshot.id);
    }
    if (snapshot.slotRanges.length != definition.slots.length) {
      throw KlpContractError('component_slot_snapshot_mismatch', snapshot.id);
    }
    ...
    for (var index = 0; index < definition.slots.length; index++) {
      ...
      if (!identical(range.slot, slot) ||
          range.start != offset ||
          count < slot.min ||
          (slot.max != null && count > slot.max!) ||
          range.end > snapshot.childrenIds.length) {
        throw KlpContractError('component_slot_snapshot_mismatch', snapshot.id);
      }
      ranges[slot] = range;
      offset = range.end;
    }
    if (offset != snapshot.childrenIds.length) {
      throw KlpContractError('component_slot_snapshot_mismatch', snapshot.id);
    }
  ...
  void _validate(KlpTemplate<KlpNode> template, String owner) {
    switch (template) {
      ...
      case KlpTextTemplate(:final semantics):
        for (final key in [
          semantics.color,
          semantics.fontFamily,
          semantics.fontSize,
          semantics.fontWeight,
          semantics.lineHeight,
          semantics.letterSpacing,
        ]) {
  ...
  void _validateInput(
    ...
    if (!template.accepts(node)) {
      throw KlpContractError('template_type_mismatch', '$id -> $path');
    }
    switch (template) {
  ...
  KlpPreparedTemplate _bind(
    KlpTemplate<KlpNode> template,
    KlpNode node,
    KlpSemanticResolution style,
    String id,
    String path,
    Map<KlpSlot<KlpNode>, KlpValidatedSlot> slots,
  ) {
    switch (template) {
      case KlpChildrenTemplate(:final axis, :final gap, :final slot):
        final range = slots[slot]!;
        return KlpPreparedChildren(
          axis: axis,
          gap: style.read(gap),
          start: range.start,
          end: range.end,
      ...
      case KlpLinearTemplate(:final axis, :final gap, :final children):
        return KlpPreparedLinear(axis, style.read(gap), [
          for (var index = 0; index < children.length; index++)
            _bind(
              children[index],
              node,
