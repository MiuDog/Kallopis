	...
	void paint(Canvas canvas, Size size) {
		...
		switch (selected.kind) {
			case KlpPageBackgroundElementKind.point:
				...
				canvas.drawCircle(viewport.pageToViewport(point.position), width + width, paint);
			case KlpPageBackgroundElementKind.line:
				...
				canvas.drawLine(viewport.pageToViewport(start.position), viewport.pageToViewport(end.position), paint);
		}
	}
...
bool _selectionExists(KlpPageBackgroundSelection selection, KlpCustomPageBackgroundRecipe recipe) {
	return switch (selection.kind) {
		KlpPageBackgroundElementKind.point => recipe.points.any((point) => point.id == selection.id),
		KlpPageBackgroundElementKind.line => recipe.lines.any((line) => line.id == selection.id),
	};
}
...
double _distanceToSegment(Offset point, Offset start, Offset end) {
	...
	final projection = (relative.dx * segment.dx + relative.dy * segment.dy) / lengthSquared;
