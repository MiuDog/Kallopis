...
extension _NavigationTransaction on KlpNavigationMachine {
  Future<void> _execute(_NavigationPending pending) async {
    try {
      ...
      final transition = KlpNavigationTransition(
        from,
        pending.candidate,
        pending.signal,
      );
      final guards = [
        ...
        if (pending.beforeEnter.isEmpty)
          _policies[pending.candidate.current.location.destination]!
              .beforeEnter,
      ...
      for (final guard in guards) {
        ...
        final allowed = await Future.any<bool>([
          Future<bool>.sync(() => guard(transition)),
          pending.signal.whenCancelled.then((_) => false),
        ]);
      ...
      try {
        ...
        dispose();
        pending.decision.completeError(
          KlpNavigationCommitContractException(error, stack),
          stack,
        );
        return;
      }
      ...
      final outcome = failure == null
          ? const KlpNavigationCompleted<void>(null)
          : KlpNavigationFailed<void>(failure, failureStack!);
      pending.decision.complete(KlpNavigationDecision(true, outcome));
    } catch (error, stack) {
      if (_current(pending)) {
        _finishRejected(pending, KlpNavigationFailed(error, stack));
      }
    } finally {
      _committing = false;
    }
  ...
  bool _current(_NavigationPending pending) =>
      !_disposed && identical(_pending, pending) && !pending.signal.isCancelled;
