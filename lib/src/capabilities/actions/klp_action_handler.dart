...
abstract interface class KlpActionHandler {
  ...
  FutureOr<KlpActionActivation> activate(
    KlpAction action,
...
bool acceptsKlpAction(KlpActionHandler? handler, KlpAction action) =>
    handler?.accepts(action) ?? action is KlpCallbackAction;

/// 唯一 action 派送入口；未綁定的導覽 action 一律拒絕。
FutureOr<KlpActionActivation> dispatchKlpAction(
  KlpActionHandler? handler,
  KlpAction action,
  KlpPlacementId source,
) {
