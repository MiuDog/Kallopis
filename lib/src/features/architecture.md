...
Explorer 列高修訂 EXP-V1-r2-density：使用者要求略降元素高度，分類與節點各由 32px 調為 28px 候選。兩個用途在 adapter 各自將自己的 resolved extent 校準為 7/8，不從圖示、padding、gap 或另一個列高相減推導；完整 primitive 替換仍依來源比例生效，不更改全域尺度。字級、圖示、間距、行內按鈕寬度保持。
...
Explorer 當前契約 [EXP-V1-r2](../../../docs/architecture/explorer-v1-plan/README.md) 已完成替換：workspace/explorer 擁有資料、快照與專用 adapter；唯一公開入口為 kallopis_declarative.dart。視覺 Catalog 待人類接受。
...
Explorer 目前配對及精確邊界見 [EXP-V1-r2](../../../docs/architecture/explorer-v1-plan/README.md)。
