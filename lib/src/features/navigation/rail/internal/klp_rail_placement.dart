...
final class KlpRailPlacement implements KlpPlacementResource {
	...
	void update(KlpValidatedNode node) {
		...
		_node = node;
		if (!_node.childrenPlacements.contains(_selection.value)) _selection.value = null;
	}

	Future<void> activate(KlpPlacementId id, KlpAction action, KlpActionHandler? actionHandler, void Function(String)? onSelected) async {
		...
		if (failures.length == 1) Error.throwWithStackTrace(failures.single.error, failures.single.stackTrace);
