...
import '../internal/klp_prepared_rail.dart';
import '../internal/klp_rail_semantics.dart';
...
final class KlpRailAdapter implements KlpNodeAdapter {
  ...
  @override
  KlpDefinition<KlpNode> get contract => KlpDefinition<KlpRail>(
    KlpRail.typeId,
    semantics: KlpRailSemantics.createSchema(),
  ...
  KlpPreparedNode prepare(
    ...
    if (extent.value <= 0 ||
        width.value <= inset.value * 2 ||
        focusWidth.value <= 0) {
      throw KlpContractError(
        'invalid_rail_geometry',
        '${snapshot.id}: positive item, inner width and focus stroke required.',
      );
    }
    ...
    for (final id in snapshot.childrenPlacements) {
      ...
      if (label.trim().isEmpty) {
        throw KlpContractError('invalid_rail_label', id.toString());
      }
      final action = item.action;
      if (action != null && !acceptsKlpAction(context.actionHandler, action)) {
        throw KlpContractError('unsupported_rail_action', id.toString());
      }
      items.add((id: id, label: label, action: action));
    }
    // 衍生幾何在配置資源前驗證有限性，避免提交後才發現溢位。
    KlpDistance groupExtent(int count) => KlpDistance(
      count == 0 ? 0 : count * extent.value + (count - 1) * gap.value,
    );
    final topCount = snapshot.slotRanges[0].end - snapshot.slotRanges[0].start;
    final centerCount =
        snapshot.slotRanges[1].end - snapshot.slotRanges[1].start;
    final bottomCount =
        snapshot.slotRanges[2].end - snapshot.slotRanges[2].start;
    ...
    if (!(topExtent.value + bottomExtent.value + minimumBodyExtent.value)
        .isFinite) {
      throw KlpContractError(
        'invalid_rail_geometry',
        '${snapshot.id}: combined region extent must be finite.',
      );
    }
    return KlpPreparedRail(
      ...
      choiceStyle: KlpBoundChoiceStyle(
        background: style.read(KlpRailSemantics.itemBackground),
        selectedBackground: style.read(KlpRailSemantics.selectedBackground),
        focusColor: style.read(KlpRailSemantics.focusColor),
        extent: extent,
        radius: style.read(KlpRailSemantics.radius),
