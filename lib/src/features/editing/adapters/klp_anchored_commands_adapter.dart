...
import 'package:kallopis/src/features/editing/contracts/klp_anchored_commands.dart';
import 'package:kallopis/src/features/editing/contracts/klp_editing_content.dart';

/// 命令子節點只攜帶結構身分；父編輯節點注入唯一來源能力。
final class KlpAnchoredCommandsAdapter implements KlpNodeAdapter {
	@override
	KlpDefinition<KlpNode> get contract => KlpDefinition<KlpAnchoredCommands>(KlpAnchoredCommands.typeId);
	...
	KlpPreparedNode prepare(KlpNode node, KlpValidatedNode snapshot, KlpPrepareContext context) {
		...
		if (parents.length != 1 || parents.single.definitionId != KlpEditingContent.typeId) {
			throw const KlpContractError('orphan_anchored_commands', 'Anchored commands must occupy an editing content slot.');
		}
		return _KlpPreparedAnchoredCommands(snapshot.id);
	}
}

final class _KlpPreparedAnchoredCommands implements KlpPreparedNode {
	final String id;
	const _KlpPreparedAnchoredCommands(this.id);

	@override
	KlpPlacementResource createResource(KlpValidatedNode node) => KlpDefaultPlacement(node);
	@override
	KlpBoundTemplate materialize(KlpPlacementResource resource, List<KlpBoundTemplate> children, KlpFrameLease lease) => KlpBoundAnchoredCommandsSlot(id);
}
