...
/// toolbar 是透明工具按鈕列：整列使用共用 header 高度，items 的命中區為同高正方形。
/// toolbar 不提供背景或內外距；群組負責外側留白，名稱僅用於提示及輔助語意。
enum KlpWorkspaceBlockKind { identity, action, paper, sticky, search, settings, board, cards, dialog, toolbar }
/// 工具按鈕列在可用寬度內的水平對齊，不改變項目順序。
enum KlpWorkspaceToolbarAlignment { start, end }
...
final class KlpWorkspaceBlock implements KlpCompositeNode {
	...
	final KlpWorkspaceToolbarAlignment toolbarAlignment;
	final String title;
	final String? symbol;
	final KlpWorkspaceIcon? icon;
	final String? subtitle;
