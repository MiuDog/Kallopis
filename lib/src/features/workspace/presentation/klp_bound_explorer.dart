...
final class KlpBoundExplorer extends KlpBoundTemplate {
	...
	KlpBoundExplorer({required this.items, Map<KlpId, KrepisPageReference> pageReferences = const {}, this.allowNesting = true, this.showCommandButtons = true, this.actionsLabel = 'Actions', this.expandLabel = 'Expand', this.collapseLabel = 'Collapse', this.spacing = 2, required this.onSelected, required this.onExpandedChanged, this.onSelectionChanged, this.canMove, this.onMove, required this.background, required this.foreground, required this.selectedBackground, required this.mutedForeground, required this.focusColor, required this.rowExtent, required this.gap, required this.indent, required this.inset, required this.radius, required this.focusWidth, required this.textStyle}) : pageReferences = Map.unmodifiable(pageReferences);
}
