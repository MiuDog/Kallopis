...
extension _KlpDockLayoutDataOperations on _KlpDockLayoutState {
  void _finishStageAreaDrop(
    ...
    final targetExtent = targetArea.extent
        .clamp(targetConstraints.minExtent, targetConstraints.maxExtent)
        .toDouble();
    ...
    widget.onLayoutChanged(
      _replaceArea(nextLayout, targetSlot, nextTargetArea),
    );
  }
  ...
  void _finishPanelDrop(
    ...
    if (data.slot == targetSlot &&
        ...
        placement == _KlpDockDropPlacement.merge) {
    ...
    final targetGroupIndex = targetGroups.indexWhere(
    ...
    if (data.slot == targetSlot) {
      nextLayout = _replaceArea(
        nextLayout,
        targetSlot,
        targetArea.copyWith(groups: targetGroups),
      );
    } else {
      nextLayout = _replaceArea(nextLayout, data.slot, nextSourceArea);
      nextLayout = _replaceArea(
  ...
  KlpDockAreaData? _removePanel(
    ...
    final activePanelId = group.activePanelId == data.panelId
        ? panelIds.first
        : group.activePanelId;
