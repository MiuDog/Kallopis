...
class _KlpPhaseToggleStyle {
	...
	factory _KlpPhaseToggleStyle.resolve(
		...
		if (!enabled) {
			activeBackground = colors.text.withValues(alpha: klp.surface.statusFillOpacity);
		}
		...
		final onActiveBackground = switch (selectedTone) {
			KlpFeedbackTone.success || KlpFeedbackTone.danger || KlpFeedbackTone.info =>
				klp.byBrightness(light: colors.surface, dark: colors.text),
			KlpFeedbackTone.warning => klp.byBrightness(light: colors.text, dark: colors.surface),
		...
		return _KlpPhaseToggleStyle._(
			...
			totalWidth: segmentExtent * optionCount + paddingExtent * 2 + borderExtent,
	...
	Color foregroundFor(KlpFeedbackTone? tone, {required bool selected, required bool enabled}) {
