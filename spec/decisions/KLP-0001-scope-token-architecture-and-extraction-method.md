...
`KlpVisualStyle.terminal` 不只是一個選項，它是**架構的驗收條件**：切換到它若需要改動任何
元件程式碼，就代表該元件還在硬編碼風格。

本決策接受時，這道條件已抓到一次真實缺口——`KlpText` 與 `buildKlpTheme` 仍讀舊的
`KlpTypography` 靜態常數，導致 terminal 的字體沒變成等寬。修法是把 `toTextStyle` 的
`KlpTypographyTheme` 參數改為必填：忘記傳會是編譯錯誤，而不是靜默用錯字體。
...
- 元件層 515 處引用舊 static token。以計數棘輪列管，只能下降。
- 6 個檔案內嵌寫死的中文文案。
