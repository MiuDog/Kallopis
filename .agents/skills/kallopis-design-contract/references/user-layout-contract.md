...
| NAV-003 | Navigation 原始碼結構 | 架構／程式碼所有權 | `lib/src/features/navigation/widgets` 下的 Explorer、Rail、Sidebar 各自使用獨立資料夾；公開型別仍統一由 `lib/kallopis.dart` 匯出，消費產品不依賴內部檔案路徑。 | 使用者明確要求，2026-09-04 | confirmed |
...
| ARCH-005 | 所有 Adaptive 平台元件 | 架構／平台策略所有權 | Adaptive 必須是單一元件並自行從 App scope 偵測平台；父組合層只注入共用資料與事件。Adaptive 以策略分發至獨立平台 layout／builder，父層與平台 layout 不得再建立第二個平台判斷來源。 | 使用者明確修訂 Adaptive 定義，2026-09-08 | confirmed |
| ARCH-006 | 所有 Kallopis 元件 | 架構／元件組裝所有權 | Flutter UI 只能出現在明確的 Kallopis 基礎原語；非基礎元件必須 dogfood Kallopis layout、surface、interaction、typography 與 control 元件，不得直接建立 Flutter UI Widget。 | 使用者要求全元件掃描並明確指出 Kallopis 非基礎元件必須 dogfooding，2026-09-08 | confirmed |
