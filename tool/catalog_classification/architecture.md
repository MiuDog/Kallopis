...
狀態：`CAT-TAX-01` PLAN READY。接受規格為 [`spec/catalog-classification.md`](../../spec/catalog-classification.md)；本 module 只建立固定 254 項的 consumer-oriented 分類資料、確定性檢查與人類審閱輸出，不決定 owning module、公開 API 或 migration 處置。
...
`specRevision` 固定指向包含已接受 TAX-01～10 的 Git commit。`reviewStatus=proposed` 可以通過資料完整性，但不能解除 migration 暫停；只有使用者審閱 `review.md` 後才能更新為 accepted。
...
分類資料由 taxonomy owner 逐項判斷，renderer 只做 deterministic projection；禁止用舊 capability ID、檔案路徑或 regex 批次決定 primary。Verifier 擁有資料形狀與集合完整性，不判斷分類語意是否合理；語意由人類審閱。
...
### `TAX-V` — Verifier and review renderer
...
當前 test state 為 Yellow：taxonomy 已接受，但 classification、test、verifier 與 review 尚不存在。確定性完成證據是精確 contract test Green、verifier Green、review 重建無 diff，以及 254/254 摘要；人類分類語意接受另列，不由程式推定。

受保護：`spec/**`、`docs/architecture/catalog-migration/legacy-baseline.json`、`coverage.json`、既有 migration evidence、`lib/**`、`example/**`、其他 tests、build 設定與本 `architecture.md`。所有 P1 已映射至 TAX-T／D／V／A，下一 slice TAX-T 沒有未決架構選擇。
