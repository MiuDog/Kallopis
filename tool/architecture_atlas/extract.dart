...
void main(List<String> args) {
  ...
  for (final file in files) {
    ...
    int line(AstNode node) =>
        parsed.lineInfo.getLocation(node.offset).lineNumber;
    String summary(AnnotatedNode node) => node.documentationComment == null
        ? ''
        : compact(
            node.documentationComment!.tokens
                .map(
                  (token) => token.lexeme.replaceAll(
                    RegExp(r'^///\s?|^/\*\*|\*/$'),
                    '',
                  ),
                )
    ...
    Map<String, Object?> member(
      ...
      'signature': signature ?? header(node),
      'summary': node is AnnotatedNode ? summary(node) : '',
    };
    ...
    for (final declaration in parsed.unit.declarations) {
      ...
      void relation(String verb, AstNode target) => relations.add({
        'kind': verb,
        'target': target.toSource(),
        'line': line(target),
      });
      if (declaration is ClassDeclaration) {
        ...
        children = declaration.body.childEntities.whereType<ClassMember>();
        for (final item
            in declaration.body.childEntities
        ...
        children = declaration.body.childEntities.whereType<ClassMember>();
        for (final item
            in declaration.onClause?.superclassConstraints ?? <NamedType>[]) {
          relation('on', item);
        }
        for (final item
            in declaration.implementsClause?.interfaces ?? <NamedType>[]) {
        ...
        for (final variable in declaration.variables.variables) {
          final prefix = source.substring(
            ...
            declaration.variables.variables.first.offset,
          );
          final signature =
              '$prefix${declaration.variables.type == null ? '(inferred) ' : ''}${variable.name.lexeme}';
          declarations.add({
            ...member(
              variable,
        ...
        throw StateError(
          'Unhandled declaration ${declaration.runtimeType} in ${file.path}',
        );
      }
      for (final child in children) {
        if (child is FieldDeclaration) {
          for (final variable in child.fields.variables) {
            final prefix = source.substring(
              child.firstTokenAfterCommentAndMetadata.offset,
              child.fields.variables.first.offset,
            );
            final signature =
                '$prefix${child.fields.type == null ? '(inferred) ' : ''}${variable.name.lexeme}';
            members.add({
              ...member(
                variable,
                variable.name.lexeme,
          ...
          throw StateError(
            'Unhandled member ${child.runtimeType} in ${file.path}',
          );
        }
      }
      declarations.add({
        ...member(declaration, name, kind),
        'members': members,
        'relations': relations,
      });
    }
    ...
    for (final directive in parsed.unit.directives) {
      if (directive is UriBasedDirective) {
        ...
        directives.add({
          'kind': kind,
          'target': directive.uri.stringValue,
          'line': line(directive),
          'signature': directive.toSource(),
        });
        if (directive is NamespaceDirective) {
          for (final config in directive.configurations) {
            directives.add({
              'kind': '$kind conditional',
              'target': config.uri.stringValue,
              'line': line(config),
              'signature': config.toSource(),
            });
          }
        }
  ...
  stdout.writeln(
    'Extracted ${result.length} Dart files without syntax errors.',
  );
}
