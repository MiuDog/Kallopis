...
放置快照唯一保存 `KlpPlacementId` 與完整子放置識別，local `id`／`childrenIds` 是衍生 getter。`nodes/klp_scope_boundary.dart` 是本庫封閉的 scope 授權接點，capture 只對該型別延伸作用域；沒有消費端開 scope 的布林參數。不同 entry 可有相同 local id，同一 scope 仍禁止重複；來源索引直接使用結構化識別，不串接路徑字串。
