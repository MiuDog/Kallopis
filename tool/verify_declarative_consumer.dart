...
List<String> verifyKlpDeclarativeConsumer(FileSystemEntity root) {
  if (!root.existsSync()) {
    throw ArgumentError.value(
      root.path,
      'root',
      'Consumer source target does not exist.',
    );
  }
  final violations = <String>[];
  final files = switch (root) {
    File file when file.path.endsWith('.dart') => [file],
    File() => throw ArgumentError.value(
      root.path,
      'root',
      'Consumer source file must end with .dart.',
    ),
    Directory directory =>
      directory
    ...
    _ => throw ArgumentError.value(
      root.path,
      'root',
      'Consumer source target must be a file or directory.',
    ),
  };
...
bool _isForbidden(String uri) {
  ...
  if (uri == 'package:flutter' || uri.startsWith('package:flutter/')) {
    return true;
  }
...
void main(List<String> arguments) {
  if (arguments.length != 1) {
    stderr.writeln(
      'Usage: dart run tool/verify_declarative_consumer.dart <consumer-source-directory>',
    );
    exitCode = 64;
    return;
  }
  final path = arguments.single;
  final target = FileSystemEntity.typeSync(path) == FileSystemEntityType.file
      ? File(path)
      : Directory(path);
