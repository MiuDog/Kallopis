...
const _outputPath =
    'example/lib/catalog/generated/catalog_style_semantics.g.dart';
...
final _directContextReference = RegExp(
  r'context\.klp(?:Colors)?(?:\.[A-Za-z_][A-Za-z0-9_]*)+',
);
...
void main(List<String> arguments) {
  final generated = generateCatalogStyleSemantics(
    Directory('lib/src'),
    publicEntry: File('lib/kallopis.dart'),
  );
  ...
  if (checkOnly) {
    ...
    if (current.replaceAll('\r\n', '\n') != generated) {
      stderr.writeln(
        'Catalog style semantics are stale. Run: dart run tool/generate_catalog_style_semantics.dart',
      );
      exitCode = 1;
    }
    return;
...
String generateCatalogStyleSemantics(
...
Map<String, _ComponentSource> _readComponents(
  Directory sourceDirectory, {
  File? publicEntry,
}) {
...
String _withPrivateDependencies(File owner, String body) {
  ...
  while (true) {
    final names = RegExp(
      r'\b(_Klp[A-Za-z0-9]+)\b',
    ).allMatches(pending).map((match) => match.group(1)!).toSet();
...
String _withStyleDependencies(File owner, String body) {
  ...
  final calls = RegExp(
    r'\b(_?Klp[A-Za-z0-9]+Style)\.resolve\s*\(',
  ).allMatches(body);
  ...
  for (final dependency in candidates) {
    ...
    if (!names.any(
      (name) =>
          RegExp('\\bclass\\s+${RegExp.escape(name)}\\b').hasMatch(source),
    )) {
      continue;
    }
...
_StyleReferences _collectReferences(
  String name,
  Map<String, _ComponentSource> components,
) {
  ...
  if (_literalColor.hasMatch(component.body)) {
    references.colors.add('literal Color（非語意，請查看來源）');
  }
  ...
  for (final entry in aliases.entries) {
    final usage = RegExp(
      '\\b${RegExp.escape(entry.key)}((?:\\.[A-Za-z_][A-Za-z0-9_]*)+)',
    );
  ...
  final dependencies = _construction
      .allMatches(component.body)
