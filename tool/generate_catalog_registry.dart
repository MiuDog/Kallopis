...
void _validateManifest(Map<String, dynamic> root) {
  ...
  _expectExactList(policy['overridable'], const [
  ...
  final primary = _map(
    derivations['color.action.primary'],
    'consumerPolicy.derivations.color.action.primary',
  );
  if (derivations.length != 1 ||
      jsonEncode(primary) != jsonEncode(expectedPrimary)) {
    throw const FormatException(
      'color.action.primary must use the canonical opaque contrast rule.',
    );
  }
  _expectExactList(policy['locked'], _expectedLayers, 'consumerPolicy.locked');
  _expectExactList(policy['isolatedFrom'], const [
    'user-design-system.recipe',
...
String _generateRegistry(Map<String, dynamic> root) {
  ...
  buffer
    ...
    ..writeln('\tfor (final page in catalogPages) ...page.coveredComponents,')
    ..writeln('};');
...
String _requiredString(Map<String, dynamic> object, String key, String path) {
