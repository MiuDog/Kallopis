...
void main() {
  testWidgets(
    'completed placement identity preserves nested focus during sibling reorder',
    (tester) async {
      final selection = KlpMutableState<KlpPlacementId?>(null);
      addTearDown(selection.dispose);
      KlpBoundPlacement placement(String id) => KlpBoundPlacement(
        KlpPlacementId(localId: id),
        KlpBoundSurface(
          background: KlpColor(0, 0, 0),
          radius: KlpRadius(0),
          inset: KlpDistance(0),
          child: klpRendererChoice(
            id: KlpPlacementId(localId: id),
            selection: selection.readOnly,
            onActivate: () => selection.value = KlpPlacementId(localId: id),
          ),
        ),
      );
      ...
      await tester.pumpWidget(
        klpRendererHost(
          KlpBoundLinear(KlpAxis.vertical, KlpDistance(0), [first, second]),
        ),
      );
      final selected = find.byWidgetPredicate(
        (widget) =>
            widget is KlpFlutterChoice &&
            widget.content.id == KlpPlacementId(localId: 'first'),
      );
      ...
      await tester.pumpWidget(
        klpRendererHost(
          KlpBoundLinear(KlpAxis.vertical, KlpDistance(0), [second, first]),
        ),
      );
