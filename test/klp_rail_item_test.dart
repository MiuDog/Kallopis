...
void main() {
  testWidgets('rail item 使用共用 action region 並保留 selected 視覺', (tester) async {
    var pressed = false;
  ...
  testWidgets('rail item hover 顯示 Kallopis tooltip surface', (tester) async {
    ...
    await tester.pump();
    expect(find.byKey(const ValueKey('rail-hover-label-Inbox')), findsNothing);
  });
  ...
  testWidgets('rail menu entry 透過既有 context menu 開啟動作', (tester) async {
    ...
    final entry = KlpRailMenuEntry(
      ...
      label: 'More',
      items: [
        KlpMenuItemData(label: 'Archive', onPressed: () => selected = true),
      ],
    );
    await tester.pumpWidget(
      MaterialApp(
