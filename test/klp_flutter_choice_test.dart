...
void main() {
  testWidgets('pointer and keyboard share selection activation', (
    tester,
  ) async {
    final selection = KlpMutableState<KlpPlacementId?>(null);
    addTearDown(selection.dispose);
    var activated = 0;
    final choice = klpRendererChoice(
      id: KlpPlacementId(localId: 'one'),
      selection: selection.readOnly,
      onActivate: () {
        activated++;
        selection.value = KlpPlacementId(localId: 'one');
      },
    );
  ...
  testWidgets('style replacement retains selection and focus identity', (
    tester,
  ) async {
    final selection = KlpMutableState<KlpPlacementId?>(null);
    addTearDown(selection.dispose);
  ...
  testWidgets(
    'choice semantics expose label selection and disabled state once',
    (tester) async {
      final selection = KlpMutableState<KlpPlacementId?>(
        KlpPlacementId(localId: 'one'),
      );
      addTearDown(selection.dispose);
  ...
  testWidgets(
    'source replacement cancels old subscription and unmount preserves borrowed source',
    (tester) async {
      final oldSelection = KlpMutableState<KlpPlacementId?>(null);
      final newSelection = KlpMutableState<KlpPlacementId?>(
        KlpPlacementId(localId: 'one'),
      );
      ...
      await tester.pumpWidget(
        klpRendererHost(
          klpRendererChoice(
            id: KlpPlacementId(localId: 'one'),
            selection: oldSelection.readOnly,
          ),
        ),
      );
      await tester.pumpWidget(
        klpRendererHost(
          klpRendererChoice(
            id: KlpPlacementId(localId: 'one'),
            selection: newSelection.readOnly,
          ),
        ),
      );
  ...
  testWidgets('tab traversal skips disabled choices', (tester) async {
    ...
    final content = KlpBoundLinear(KlpAxis.vertical, KlpDistance(4), [
      klpRendererChoice(
        id: KlpPlacementId(localId: 'one'),
        selection: selection.readOnly,
        onActivate: () => activated.add('one'),
      ),
      klpRendererChoice(
        id: KlpPlacementId(localId: 'disabled'),
        selection: selection.readOnly,
      ),
      klpRendererChoice(
        id: KlpPlacementId(localId: 'two'),
        selection: selection.readOnly,
        onActivate: () => activated.add('two'),
      ),
    ]);
  ...
  testWidgets(
    'selection matches complete scope even when choices share a source and label',
    (tester) async {
      ...
      try {
        ...
        await tester.pumpWidget(klpRendererHost(content));
        expect(
          tester.getSemantics(find.byType(KlpFlutterChoice).first),
          matchesSemantics(
            label: 'Action same',
            isButton: true,
            hasEnabledState: true,
        ...
        expect(
          tester.getSemantics(find.byType(KlpFlutterChoice).last),
          matchesSemantics(
            label: 'Action same',
            isButton: true,
            hasEnabledState: true,
