...
void main() {
  ...
  test(
    'consumer component adapter binds data and changes style without resource replacement',
    () {
      final definition = klpComponentTestDefinition();
      final component = KlpComponentAdapter(definition);
      runtime.update(
        root: KlpComponentTestItem(),
        adapters: [component],
      ...
      runtime.update(
        root: KlpComponentTestItem(label: 'Changed'),
        adapters: [component],
        components: [definition],
        primitives: klpTestPrimitives(alternate: true),
      );
      expect(
        runtime.resources[KlpPlacementId(localId: 'placement')],
        same(resource),
      );
