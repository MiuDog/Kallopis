...
void main() {
	...
	testWidgets('compact categories have no persistent fill and document rows use button gaps', (tester) async {
		...
		await harness.show(tester, KlpExplorer(id: KlpId.parse('explorer'), expandedIds: {category}, selectedId: category, onExpandedChanged: (_, _) {}, items: [KlpExplorerItem(id: category, label: 'Category', kind: KlpExplorerItemKind.category, children: [for (var i = 0; i < 2; i++) KlpExplorerItem(id: KlpId.parse('file$i'), label: 'File$i', kind: KlpExplorerItemKind.file)])]));
		final rows = find.byType(KlpFlutterSelectionSurface);
		expect(tester.getSize(rows.at(0)).height, 24);
		...
		expect(tester.widgetList<KlpFlutterLucideIcon>(find.byType(KlpFlutterLucideIcon)).where((icon) => icon.name == 'file-text').map((icon) => icon.size), everyElement(18));
	...
	testWidgets('Explorer selection and click focus use the hover fill without borders', (tester) async {
		...
		KlpExplorer tree(bool selected) => KlpExplorer(id: KlpId.parse('explorer'), selectedId: selected ? file : null, onSelected: (_) {}, items: [KlpExplorerItem(id: file, label: 'Document', kind: KlpExplorerItemKind.file)]);
	...
	testWidgets('category toggles without selection and keeps icon badge children controlled', (tester) async {
		...
		KlpExplorer tree(bool open) => KlpExplorer(id: KlpId.parse('explorer'), expandedIds: open ? {category} : {}, onSelected: selections.add, onExpandedChanged: (id, expanded) => toggles.add((id, expanded)), items: [KlpExplorerItem(id: category, label: 'Documents', kind: KlpExplorerItemKind.category, children: [KlpExplorerItem(id: file, label: 'Photo', kind: KlpExplorerItemKind.file, icon: KlpExplorerIcon.image, badge: '3')])]);
		await harness.show(tester, tree(false));
		...
		expect(tester.getSize(find.byType(KlpFlutterSelectionSurface).first).height, 24);
	});

	testWidgets('flat explorer preserves descendants and removes folder disclosure and indentation', (tester) async {
		final harness = _Harness();
		addTearDown(harness.runtime.dispose);
		final folder = KlpId.parse('folder');
		final file = KlpId.parse('file');
		final selections = <KlpId>[];
		await harness.show(tester, KlpExplorer(id: KlpId.parse('explorer'), allowNesting: false, onSelected: selections.add, items: [KlpExplorerItem(id: folder, label: 'Folder', kind: KlpExplorerItemKind.folder, children: [KlpExplorerItem(id: file, label: 'File', kind: KlpExplorerItemKind.file)])]));
		expect(find.text('File'), findsOneWidget);
		expect(find.bySemanticsLabel('Expand Folder'), findsNothing);
		expect(tester.getTopLeft(find.text('Folder')).dx, tester.getTopLeft(find.text('File')).dx);
	...
	testWidgets('collapsed explorer descendants still participate in tree validation', (tester) async {
		...
		final node = KlpExplorer(id: KlpId.parse('explorer'), items: [
			KlpExplorerItem(id: KlpId.parse('folder'), label: 'Folder', kind: KlpExplorerItemKind.folder, children: [KlpExplorerItem(id: duplicate, label: 'Hidden', kind: KlpExplorerItemKind.file)]),
			KlpExplorerItem(id: duplicate, label: 'Visible', kind: KlpExplorerItemKind.file),
		]);
		await expectLater(harness.show(tester, node), throwsA(isA<KlpContractError>().having((error) => error.code, 'code', 'duplicate_placement')));
	});
	...
	testWidgets('explorer expansion and selection remain consumer controlled', (tester) async {
		...
		KlpExplorer tree(Set<KlpId> open) => KlpExplorer(
			id: KlpId.parse('explorer'), expandedIds: open, selectedId: null,
			items: [KlpExplorerItem(id: folder, label: 'Folder', kind: KlpExplorerItemKind.folder, children: [KlpExplorerItem(id: file, label: 'Readme', kind: KlpExplorerItemKind.file)])],
			onSelected: selected.add, onExpandedChanged: (id, value) => expanded.add((id, value)),
		);
