...
void main() {
  testWidgets('PreviewCard 只在預覽區呈現語意虛線邊框', (tester) async {
    ...
    expect(
      find.descendant(of: border, matching: find.text('PREVIEW')),
      findsOneWidget,
    );
    expect(
      find.descendant(of: border, matching: find.text('Asset title')),
      findsNothing,
    );
  });
  ...
  testWidgets('PreviewCard 虛線邊框會解析對照風格 token', (tester) async {
    ...
    final defaultPainter = tester
        .widget<CustomPaint>(defaultPaint)
        .foregroundPainter!;
    ...
    final contrastingPainter = tester
        .widget<CustomPaint>(contrastingPaint)
        .foregroundPainter!;
