...
KlpApplication _app(
  ...
  router: KlpRouter(id: 'router', initial: home.location(0), routes: routes),
  components: [klpComponentTestDefinition()],
);

void main() {
  testWidgets(
    'refresh revokes an action captured from the prior committed entry',
    (tester) async {
      final home = KlpDestination<int, String>('home');
      final detail = KlpDestination<int, String>('detail');
      final firstRoute = KlpRoute<int, String>(
        home,
        screen: (input) =>
            _screen('First', action: input.navigate(detail.location(1))),
      );
  ...
  testWidgets('root back action is rejected without changing rail selection', (
    tester,
  ) async {
    final home = KlpDestination<int, String>('home');
