...
void main() {
		...
		await tester.pumpWidget(
			MaterialApp(
				...
				home: const KlpPasswordField(
					...
					requirements: [
						KlpPasswordRequirement(label: 'At least 8 characters', satisfied: true),
						KlpPasswordRequirement(label: 'Contains a number', satisfied: false),
					],
				),
			),
		);
	...
	testWidgets('disabled password action 保留 disabled semantics 且不切換遮罩', (
		...
		final semantics = tester.widget<Semantics>(
			find.byWidgetPredicate(
				(widget) =>
					widget is Semantics && widget.properties.label == '顯示密碼',
			),
		);
