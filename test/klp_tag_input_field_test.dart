...
void main() {
  testWidgets('tag input 以 Kallopis 排版並派送新增、移除與清空事件', (tester) async {
    ...
    expect(
      find.descendant(of: field, matching: find.byType(KlpColumn)),
      findsOneWidget,
    );
    expect(
      find.descendant(of: field, matching: find.byType(KlpWrap)),
      findsOneWidget,
    );
  ...
  testWidgets('tag chip 沒有移除事件時不呈現移除動作', (tester) async {
    await tester.pumpWidget(
      const KlpApp(
        showWindowHeader: false,
        home: KlpPanelFrame(
          content: KlpAppScreen(child: KlpTagChip(label: 'stable')),
        ),
      ),
    );
