...
void main() {
  group('KlpAsyncData', () {
    ...
    test(
      'loading replaces old data and completion follows publication',
      () async {
        final owner = KlpAsyncData<int>();
        addTearDown(owner.dispose);
    ...
    test(
      'cancel invalidates late errors without cancelling operation',
      () async {
        final owner = KlpAsyncData<int>();
        addTearDown(owner.dispose);
        final request = Completer<int>();
    ...
    test(
      'cancel or disposal during loading notification prevents start',
      () async {
    ...
    test(
      'publication listener errors are not data operation failures',
      () async {
        final owner = KlpAsyncData<int>();
        addTearDown(owner.dispose);
        owner.state.subscribe((value) {
    ...
    test(
      'replacement during notification prevents superseded operation',
      () async {
        final owner = KlpAsyncData<int>();
        addTearDown(owner.dispose);
        var replaced = false;
