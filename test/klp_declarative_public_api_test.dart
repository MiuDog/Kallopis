...
void main() {
	test('public declarative declarations do not expose Flutter presentation types', () {
		final forbidden = RegExp(r'\b(?:Widget|BuildContext|ThemeData|AnimationController|NavigatorState)\b');
		final violations = <String>[];
		for (final file in _reachableSources(File('lib/kallopis_declarative.dart'))) {
			...
			for (final declaration in unit.declarations) {
				...
				if (forbidden.hasMatch(source)) violations.add('${file.path.replaceAll(r'\\', '/')}:$name');
			}
		}
		expect(violations, isEmpty, reason: '宣告式公開契約洩漏 Flutter 呈現型別：\n${violations.join('\n')}');
	});
}
...
Set<File> _reachableSources(File entry) {
	...
	void visit(File file) {
		...
		final unit = parseString(content: file.readAsStringSync(), path: file.path, throwIfDiagnostics: false).unit;
		if (file.path.replaceAll(r'\\', '/').contains('/lib/src/') && !file.path.replaceAll(r'\\', '/').contains('/internal/')) files.add(file);
		for (final directive in unit.directives) {
			...
			if (reference == null || reference.hasScheme || reference.hasAuthority) continue;
...
String? _nameOf(CompilationUnitMember declaration) => switch (declaration) {
	...
	ExtensionTypeDeclaration(:final primaryConstructor) => primaryConstructor.typeName.lexeme,
	FunctionDeclaration(:final name) => name.lexeme,
	MixinDeclaration(:final name) => name.lexeme,
	TopLevelVariableDeclaration(:final variables) => variables.variables.firstOrNull?.name.lexeme,
	_ => null,
};
