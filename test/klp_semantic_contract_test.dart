...
KlpSemanticKey<KlpDistance> _key(String owner, String name) =>
    KlpSemanticKey(owner, name, KlpStyleKind.distance);
Matcher _error(String code) => throwsA(
  isA<KlpContractError>().having((error) => error.code, 'code', code),
);
KlpSemanticToken<KlpDistance> _primitive(
  KlpSemanticKey<KlpDistance> key, {
  bool isPublic = false,
}) => KlpSemanticToken(
  key,
  const KlpPrimitiveRef(KlpStyleKind.distance, KlpPrimitiveIndex.i2),
  isPublic: isPublic,
);

void main() {
  test(
    'public cross owner alias resolves with declared dependency independent of input order',
    () {
      final base = _key('foundation', 'gap');
      final alias = _key('custom', 'spacing');
      final custom = KlpSemanticSchema(
        'custom',
        [KlpSemanticToken(alias, KlpSemanticRef(base))],
        dependencies: ['foundation'],
      );
      final foundation = KlpSemanticSchema('foundation', [
        _primitive(base, isPublic: true),
      ]);
      final resolved = KlpSemanticResolver([
        custom,
        foundation,
      ]).resolve(klpTestPrimitives());
  ...
  test(
    'one resolver accepts two complete styles while preserving previous snapshot',
    () {
      final key = _key('custom', 'spacing');
      final resolver = KlpSemanticResolver([
        KlpSemanticSchema('custom', [_primitive(key)]),
      ]);
  ...
  test('common base generic cannot mix incompatible primitive kinds', () {
    ...
    const KlpStyleRef<KlpStyleValue> reference = KlpPrimitiveRef(
      KlpStyleKind.color,
      KlpPrimitiveIndex.i0,
    );
    expect(
      () => KlpSemanticToken<KlpStyleValue>(key, reference),
      _error('semantic_kind_mismatch'),
    );
  });
  ...
  test('forged semantic target kind is checked against registered target', () {
    ...
    final target = KlpSemanticToken(
      real,
      const KlpPrimitiveRef(KlpStyleKind.color, KlpPrimitiveIndex.i0),
    );
    final schema = KlpSemanticSchema('custom', [
      KlpSemanticToken(alias, KlpSemanticRef(forged)),
      target,
    ]);
    expect(
      () => KlpSemanticResolver([schema]),
      _error('semantic_kind_mismatch'),
    );
  });
  ...
  test('same owner and name cannot coexist with different kinds', () {
    ...
    final token = KlpSemanticToken(
      color,
      const KlpPrimitiveRef(KlpStyleKind.color, KlpPrimitiveIndex.i0),
    );
    expect(
      () => KlpSemanticSchema('custom', [
        _primitive(_key('custom', 'shared')),
        token,
      ]),
      _error('duplicate_semantic'),
    );
  });
  ...
  test('private semantic cannot be referenced across declared dependency', () {
    ...
    final custom = KlpSemanticSchema(
      'custom',
      [KlpSemanticToken(_key('custom', 'alias'), KlpSemanticRef(target))],
      dependencies: ['base'],
    );
    expect(
      () => KlpSemanticResolver([base, custom]),
      _error('private_semantic_reference'),
    );
  });

  test('public semantic still requires declared owner dependency', () {
    final target = _key('base', 'gap');
    final base = KlpSemanticSchema('base', [
      _primitive(target, isPublic: true),
    ]);
    final custom = KlpSemanticSchema('custom', [
      KlpSemanticToken(_key('custom', 'alias'), KlpSemanticRef(target)),
    ]);
    expect(
      () => KlpSemanticResolver([base, custom]),
      _error('undeclared_semantic_dependency'),
    );
  });
  ...
  test('private semantic can be aliased inside its own owner', () {
    ...
    final alias = _key('custom', 'alias');
    final schema = KlpSemanticSchema('custom', [
      KlpSemanticToken(alias, KlpSemanticRef(source)),
      _primitive(source),
    ]);
    expect(
      KlpSemanticResolver([
        schema,
      ]).resolve(klpTestPrimitives()).read(alias).value,
      8,
    );
  });
  ...
  test('unknown semantic reference fails before resolution', () {
    final token = KlpSemanticToken(
      _key('custom', 'alias'),
      KlpSemanticRef(_key('custom', 'missing')),
    );
    expect(
      () => KlpSemanticResolver([
        KlpSemanticSchema('custom', [token]),
      ]),
      _error('unknown_semantic'),
    );
  });
  ...
  test('semantic reference cycle includes complete traversal path', () {
    ...
    final matcher = isA<KlpContractError>()
        .having((error) => error.code, 'code', 'semantic_reference_cycle')
        .having(
          (error) => error.message,
          'path',
          'custom/a -> custom/b -> custom/c -> custom/a',
        );
  ...
  test('schema collections are copied and cannot be mutated', () {
    ...
    final schema = KlpSemanticSchema(
      'custom',
      tokens,
  ...
  test('resolution copies value map and rejects fake or missing read keys', () {
    ...
    expect(
      () => result.read(_key('custom', 'missing')),
      _error('unknown_semantic'),
    );
  });
  ...
  test('duplicate owners and mismatched token owners are rejected', () {
    final schema = KlpSemanticSchema('custom', []);
    expect(
      () => KlpSemanticResolver([schema, schema]),
      _error('duplicate_semantic_owner'),
    );
    expect(
      () => KlpSemanticSchema('custom', [_primitive(_key('other', 'gap'))]),
      _error('semantic_owner_mismatch'),
    );
  });
}
