...
KlpPlacementId _id(String scope, [String local = 'same']) => KlpPlacementId(scope: [scope], localId: local);
Finder _choice(String scope) => find.byWidgetPredicate((widget) => widget is KlpFlutterChoice && widget.content.id == _id(scope), skipOffstage: false);
FocusNode _focus(WidgetTester tester, String scope) => tester.widget<FocusableActionDetector>(find.descendant(of: _choice(scope), matching: find.byType(FocusableActionDetector, skipOffstage: false), skipOffstage: false)).focusNode!;

void main() {
	late KlpMutableState<KlpPlacementId?> selection;
	late List<String> activations;
	KlpBoundPlacement page(String scope, {bool enabled = true, bool removed = false}) {
		final content = removed ? klpRendererText('Removed') : klpRendererChoice(id: _id(scope), selection: selection.readOnly, onActivate: enabled ? () => activations.add(scope) : null);
		return KlpBoundPlacement(_id(scope, 'page'), content);
	}
	Widget host(String active, List<KlpBoundPlacement> pages) => klpRendererHost(KlpBoundRetainedStack(pages: pages, activeId: _id(active, 'page')));
	setUp(() {
	...
	test('retained stack rejects empty pages and a missing active identity', () {
		expect(() => KlpBoundRetainedStack(pages: [], activeId: _id('a', 'page')), throwsArgumentError);
		expect(() => KlpBoundRetainedStack(pages: [page('a')], activeId: _id('b', 'page')), throwsArgumentError);
	});

	test('retained stack rejects duplicate complete identities', () {
		expect(() => KlpBoundRetainedStack(pages: [page('a'), page('a')], activeId: _id('a', 'page')), throwsArgumentError);
	});

	test('retained stack accepts equal local identities in different scopes', () {
		final pages = [page('a'), page('b')];
		final stack = KlpBoundRetainedStack(pages: pages, activeId: _id('b', 'page'));
		pages.clear();
		expect(stack.pages, hasLength(2));
		expect(() => stack.pages.clear(), throwsUnsupportedError);
	});

	testWidgets('same local IDs retain separate elements across visibility and reordering', (tester) async {
	...
	testWidgets('hidden page excludes focus pointer semantics and ticker participation', (tester) async {
		...
		try {
			...
			expect(TickerMode.valuesOf(tester.element(_choice('a'))).enabled, isTrue);
			expect(TickerMode.valuesOf(tester.element(_choice('b'))).enabled, isFalse);
			await tester.pumpWidget(host('b', [page('a'), page('b')]));
			expect(TickerMode.valuesOf(tester.element(_choice('a'))).enabled, isFalse);
			expect(TickerMode.valuesOf(tester.element(_choice('b'))).enabled, isTrue);
			expect(changes, [true]);
			await tester.pumpWidget(host('a', [page('a'), page('b')]));
	...
	testWidgets('return restores the previously focused valid leaf', (tester) async {
		await tester.pumpWidget(host('a', [page('a'), page('b')]));
		await tester.tap(find.byType(KlpFlutterChoice));
		await tester.pump();
	...
	testWidgets('pending old focus cannot land on a hidden page when a new page mounts', (tester) async {
	...
	testWidgets('rapid visibility changes leave only the final page focusable', (tester) async {
		await tester.pumpWidget(host('a', [page('a'), page('b')]));
	...
	for (final removed in [false, true]) {
		testWidgets('return ignores a hidden leaf that becomes ${removed ? 'removed' : 'disabled'}', (tester) async {
			await tester.pumpWidget(host('a', [page('a'), page('b')]));
			await tester.tap(find.byType(KlpFlutterChoice));
			await tester.pump();
			final previous = _focus(tester, 'a');
			await tester.pumpWidget(host('b', [page('a'), page('b')]));
			await tester.pumpWidget(host('b', [page('a', enabled: false, removed: removed), page('b')]));
			await tester.pumpWidget(host('a', [page('a', enabled: false, removed: removed), page('b')]));
			await tester.pump();
	...
	testWidgets('removing an entry unmounts its choice and detaches its owned focus', (tester) async {
		await tester.pumpWidget(host('a', [page('a'), page('b')]));
