...
void main() {
  test('switch and drawer dimensions copy independently and round trip', () {
    ...
    expect(
      [
        control.switchTrackWidth,
        control.switchTrackHeight,
        control.switchThumb,
      ],
      [36, 20, 16],
    );
    ...
    final cases = <(String, String, KlpGeometryTheme)>[
      (
        'control',
        'switchTrackWidth',
        base.geometry.copyWith(control: control.copyWith(switchTrackWidth: 48)),
      ),
      (
        'control',
        'switchTrackHeight',
        base.geometry.copyWith(
          control: control.copyWith(switchTrackHeight: 48),
        ),
      ),
      (
        'control',
        'switchThumb',
        base.geometry.copyWith(control: control.copyWith(switchThumb: 48)),
      ),
      (
        'layout',
        'drawerWidth',
        base.geometry.copyWith(layout: layout.copyWith(drawerWidth: 48)),
      ),
      (
        'layout',
        'drawerHeight',
        base.geometry.copyWith(layout: layout.copyWith(drawerHeight: 48)),
      ),
    ];
    ...
    for (final (group, key, geometry) in cases) {
      final encoded = KlpVisualStyleJson.encode(
        base.copyWith(geometry: geometry),
      );
      final expected = <Object?, Object?>{
        ...(original['geometry'] as Map)[group] as Map,
        key: 48.0,
      };
  ...
  test(
    'legacy dimensions migrate from spacing with explicit geometry precedence',
    () {
      for (final version in <int?>[null, 1, 2]) {
        ...
        final input = <String, Object?>{
          'spacing': spacing,
          'geometry': {
            'control': {'switchThumb': 20},
            'layout': {'drawerHeight': 300},
          },
        };
  ...
  test(
    'dimension migration preserves omitted base values and reports invalid paths',
    () {
      final standard = KlpVisualStyle.defaultStyle;
      final base = standard.copyWith(
        geometry: standard.geometry.copyWith(
          control: standard.geometry.control.copyWith(switchThumb: 19),
        ),
      );
      expect(
        KlpVisualStyleJson.decode({
          'schemaVersion': 2,
          'spacing': {'drawerWidth': 444},
        }, base: base).geometry.control.switchThumb,
        19,
      );
      expect(
        () => KlpVisualStyleJson.decode({
          'schemaVersion': 2,
          'spacing': {'switchThumb': -1},
        }),
        throwsA(
          isA<FormatException>().having(
            (error) => error.message,
            'path',
            contains('spacing.switchThumb'),
          ),
        ),
      );
      expect(
        () => KlpVisualStyleJson.decode({
          'schemaVersion': 3,
          'spacing': {'drawerWidth': 320},
        }),
        throwsA(
          isA<FormatException>().having(
            (error) => error.message,
            'path',
            contains('spacing.drawerWidth'),
          ),
        ),
      );
      expect(
        ...
        throwsA(
          isA<FormatException>().having(
            (error) => error.message,
            'path',
            contains('geometry.layout.drawerHeight'),
          ),
        ),
      );
    },
  ...
  testWidgets(
    'compact switch preserves defaults and follows independently injected geometry',
    (tester) async {
      final base = KlpVisualStyle.defaultStyle;
      ...
      for (final style in [base, custom]) {
        ...
        expect(
          tester.getSize(find.byType(KlpCompactSwitch)),
          Size(control.switchTrackWidth, control.switchTrackHeight),
        );
        expect(
          find.byWidgetPredicate(
            (widget) =>
                widget is SizedBox &&
                widget.width == control.switchThumb &&
                widget.height == control.switchThumb,
          ),
          findsOneWidget,
        );
        changed = null;
  ...
  testWidgets('drawer geometry applies to all edges and explicit size wins', (
    tester,
  ) async {
    final base = KlpVisualStyle.defaultStyle;
    final custom = base.copyWith(
      ...
      for (final edge in KlpDrawerEdge.values) {
        for (final size in <double?>[null, 210]) {
          ...
          expect(
            ...
            size ??
                (horizontal
                    ? style.geometry.layout.drawerWidth
                    : style.geometry.layout.drawerHeight),
          );
