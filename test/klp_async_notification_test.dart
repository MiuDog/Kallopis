...
void main() {
	group('KlpAsyncData notification recovery', () {
		test('initial notification failure restores idle without starting operation', () async {
			...
			await expectLater(owner.load(() async => ++calls), throwsA(same(error)));
			expect(calls, 0);
			expect(owner.state.value, isA<KlpDataIdle<int>>());
		});

		test('recovery failure preserves both original errors and traces', () async {
			final owner = KlpAsyncData<int>();
			addTearDown(owner.dispose);
			...
			await expectLater(owner.load(() async => ++calls), throwsA(isA<KlpDataNotificationException>().having((error) => error.issues.map((issue) => issue.error).toList(), 'original errors', [same(loadingError), same(recoveryError)]).having((error) => error.issues.map((issue) => issue.stackTrace).toList(), 'original traces', [same(loadingTrace), same(recoveryTrace)])));
			expect(calls, 0);
			expect(owner.state.value, isA<KlpDataIdle<int>>());
		});
		...
		test('reentrant load survives the original notification failure', () async {
			...
			await expectLater(owner.load(() async => ++originalCalls), throwsA(same(error)));
		...
		test('cancel during a failing notification does not publish idle twice', () async {
			final owner = KlpAsyncData<int>();
			addTearDown(owner.dispose);
		...
		test('dispose during a failing notification preserves the listener error', () async {
			final owner = KlpAsyncData<int>();
			addTearDown(owner.dispose);
