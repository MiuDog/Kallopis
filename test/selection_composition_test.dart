...
void main() {
	test('sliding selection geometry copies independently and round trips', () {
		...
		final encoded = KlpVisualStyleJson.encode(base.copyWith(geometry: base.geometry.copyWith(control: customControl)));
	...
	testWidgets('sliding selection follows geometry and dispatches typed options', (tester) async {
		final base = KlpVisualStyle.defaultStyle;
				...
				home: Material(
					child: Center(
						child: KlpSlidingSelection(
							...
							selectedIndex: 0,
							options: const [
								KlpSelectionOption(icon: KlpIcons.grid, tone: KlpSelectionTone.info),
								KlpSelectionOption(icon: KlpIcons.container, tone: KlpSelectionTone.primary),
							],
		...
		expect(tester.getSize(find.byType(KlpSlidingSelection)), const Size(104, 40));
