...
void main() {
  testWidgets('KlpInlineNotice icon 等於 body 字級且混合字體共用基線', (tester) async {
    for (final style in [KlpVisualStyle.defaultStyle, contrastingStyle]) {
      ...
      expect(
        _baseline(tester, find.text('INFO')),
        closeTo(_baseline(tester, find.text('狀態')), 0.01),
      );
      expect(
        tester.getCenter(find.byType(KlpIcon)).dy,
        closeTo(tester.getCenter(find.text('狀態')).dy, 2),
      );
...
Widget _specimen(KlpVisualStyle style) {
  final home = const SizedBox(
    width: 220,
    child: KlpInlineNotice(title: '狀態', tone: KlpFeedbackTone.info),
  );
  return MaterialApp(
