...
void main() {
  test(
    'encodes and decodes one platform entry without Flutter routing types',
    () {
      ...
      expect(restoration.stack.single.parameters, {
        'note': '7',
        'tab': 'history',
      });
    },
  ...
  test(
    'round-trips a retained stack without colliding with route parameters',
    () {
      final encoded = KlpRouteUri.encodeRestoration(
        KlpNavigationRestoration(
          ...
          stack: [
            ...
            KlpRouteAddress(destinationId: 'detail', parameters: {'note': '7'}),
          ],
        ),
      );
  ...
  test(
    'rejects malformed complete-stack payloads before router restoration',
    () {
      expect(
