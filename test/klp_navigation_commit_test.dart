...
void main() {
  for (final committed in [false, true]) {
    test('commit failure preserves actual commit status $committed', () async {
      ...
      expect(
        await ticket.result,
        committed
            ? isA<KlpNavigationCancelled<String>>()
            : isA<KlpNavigationFailed<String>>(),
      );
    });
  }
  ...
  test('guard failure leaves stack and commit port unchanged', () async {
    ...
    final machine = klpNavigationTestMachine(
      policies: [
        ...
        KlpRoutePolicy(
          destination: detail,
          beforeEnter: (_) => throw StateError('guard'),
        ),
      ],
      initial: home.location(0),
      commit: (_) => commits++,
  ...
  test('initial committed notification error retains usable initial state', () {
    ...
    final machine = klpNavigationTestMachine(
      ...
      initial: home.location(0),
      commit: (_) => throw KlpNavigationCommitException(
        true,
        StateError('notification'),
        StackTrace.current,
      ),
    );
    addTearDown(machine.dispose);
