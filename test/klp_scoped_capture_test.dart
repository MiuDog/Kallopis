...
void main() {
	...
	test(
		...
		() {
			final first = KlpTestItem(KlpId.parse('same'), 'item');
			final second = KlpTestItem(KlpId.parse('same'), 'item');
			final root = KlpTestItem(KlpId.parse('root'), 'item', [
				KlpScopeBoundary(id: KlpId.parse('first'), child: first),
				KlpScopeBoundary(id: KlpId.parse('second'), child: second),
			]);
	...
	test('same scope duplicate placement still fails before installation', () {
		final root = KlpScopeBoundary(
			...
			child: KlpTestItem(KlpId.parse('root'), 'item', [
	...
	test(
		...
		() {
			final root = KlpTestItem(KlpId.parse('root'), 'item', [
				KlpScopeBoundary(
					id: KlpId.parse('a/b'),
					child: KlpTestItem(KlpId.parse('c'), 'item'),
				),
				KlpScopeBoundary(
					id: KlpId.parse('a'),
				...
				KlpTestItem(KlpId.parse('a/b/c'), 'item'),
			]);
			final captured = captureKlpTree(registry, root);
			expect(
	...
	test(
		...
		() {
			final impostor = KlpTestItem(
				KlpId.parse('entry'),
				KlpScopeBoundary.typeId,
				[KlpTestItem(KlpId.parse('child'), 'item')],
			);
			expect(
	...
	test('cycle remains invalid even when it crosses a scope boundary', () {
		final root = KlpTestItem(KlpId.parse('root'), 'item');
		root.children.add(KlpScopeBoundary(id: KlpId.parse('entry'), child: root));
		expect(
			() => registry.validate(root),
			throwsA(
				isA<KlpContractError>().having(
