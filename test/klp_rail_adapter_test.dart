...
void main() {
	test('prepared rail preserves regions and revokes old actions', () async {
		...
		final top = KlpComponentTestItem(id: KlpId.parse('top'));
		final bottom = KlpComponentTestItem(id: KlpId.parse('bottom'));
		final rail = KlpRail(
			id: KlpId.parse('rail'),
			top: [top],
			center: [item],
		...
		final snapshot = KlpRegistry([
			adapter.contract,
			KlpDefinition<KlpComponentTestItem>(item.definitionId),
		]).validate(rail).nodes.first;
		...
		final prepared = adapter.prepare(
			...
			KlpPrepareContext(
				sources: {
					KlpPlacementId(localId: top.id.value): top,
					KlpPlacementId(localId: item.id.value): item,
					KlpPlacementId(localId: bottom.id.value): bottom,
					snapshot.placementId: rail,
				},
		...
		final extent =
				prepared.materialize(resource, [child, child, child], lease)
						as KlpBoundExtent;
		final surface = extent.child as KlpBoundSurface;
		final regions = surface.child as KlpBoundRegions;
		final choice =
				(regions.body as KlpBoundLinear).children.single as KlpBoundChoice;
		expect(
			((regions.leading as KlpBoundLinear).children.single as KlpBoundChoice)
					.onActivate,
			isNull,
		);
		expect(
			((regions.trailing as KlpBoundLinear).children.single as KlpBoundChoice)
					.id,
			KlpPlacementId(localId: bottom.id.value),
		);
		...
		expect(resource.selection.value, KlpPlacementId(localId: item.id.value));
		expect(calls, 1);
		lease.revoke();
		await choice.onActivate!();
		expect(calls, 1);
	...
	test('blank accessibility label fails during preparation', () {
		...
		expect(
			() => adapter.prepare(
				rail,
				KlpValidatedNode('rail', KlpRail.typeId, [item.id.value]),
				KlpPrepareContext(
					sources: {KlpPlacementId(localId: item.id.value): item},
					nodes: const {},
					primitives: primitives,
					style: KlpSemanticResolver([
						adapter.contract.semantics,
	...
	test(
		'unsupported action is rejected before a rail placement is installed',
		() {
			...
			expect(
				() => adapter.prepare(
					...
					KlpPrepareContext(
						sources: {
							snapshot.placementId: rail,
							snapshot.childrenPlacements.single: item,
						},
						nodes: {snapshot.placementId: snapshot},
						primitives: primitives,
						style: KlpSemanticResolver([
	...
	test(
		'scoped preparation reads the exact item and emits only its local callback identity',
		() async {
			...
			final other = KlpComponentTestItem(
				id: KlpId.parse('item'),
				label: 'Other item',
			);
			final rail = KlpRail(
			...
			const adapter = KlpRailAdapter();
			final unscoped = KlpRegistry([
				adapter.contract,
				KlpDefinition<KlpComponentTestItem>(item.definitionId),
			]).validate(rail).nodes.first;
			final railId = KlpPlacementId(scope: ['entry'], localId: 'rail');
			final itemId = KlpPlacementId(scope: ['entry'], localId: 'item');
			final snapshot = KlpValidatedNode.scoped(railId, KlpRail.typeId, [
				itemId,
			], slotRanges: unscoped.slotRanges);
			final primitives = klpTestPrimitives();
			final prepared = adapter.prepare(
				rail,
				snapshot,
				KlpPrepareContext(
					sources: {
						railId: rail,
						itemId: item,
						KlpPlacementId(scope: ['other'], localId: 'item'): other,
					},
			...
			final child = KlpBoundLinear(KlpAxis.vertical, KlpDistance(0), []);
			final extent =
					prepared.materialize(resource, [child], KlpFrameLease())
							as KlpBoundExtent;
			final regions =
					(extent.child as KlpBoundSurface).child as KlpBoundRegions;
			final choice =
					(regions.body as KlpBoundLinear).children.single as KlpBoundChoice;
