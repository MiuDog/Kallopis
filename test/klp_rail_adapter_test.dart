...
void main() {
  test('prepared rail preserves regions and revokes old actions', () async {
    ...
    final prepared = adapter.prepare(
      ...
      KlpPrepareContext(
        sources: {
          KlpPlacementId(localId: top.id): top,
          KlpPlacementId(localId: item.id): item,
          KlpPlacementId(localId: bottom.id): bottom,
    ...
    expect(
      ((regions.trailing as KlpBoundLinear).children.single as KlpBoundChoice)
          .id,
      KlpPlacementId(localId: bottom.id),
    );
  ...
  test('blank accessibility label fails during preparation', () {
    ...
    expect(
      () => adapter.prepare(
        rail,
        KlpValidatedNode('rail', KlpRail.typeId, [item.id]),
        KlpPrepareContext(
