...
void main() {
	...
	test('router freezes declarations and preserves exact destination identity', () {
	...
	test('same named destination cannot impersonate initial registration', () {
		...
		expect(() => KlpRouter(id: 'router', initial: impostor.location(7), routes: [KlpRoute<int, String>(destination, screen: screen)]), throwsArgumentError);
	});

	test('duplicate names and duplicate destination registrations are rejected', () {
		final destination = KlpDestination<int, String>('main');
		final route = KlpRoute(destination, screen: screen);
		final sameName = KlpRoute(KlpDestination<int, String>('main'), screen: screen);
		for (final routes in [[route, route], [route, sameName]]) {
			expect(() => KlpRouter(id: 'router', initial: destination.location(7), routes: routes), throwsArgumentError);
		}
	});

	test('empty router identity and missing initial route are rejected', () {
		final destination = KlpDestination<int, String>('main');
		expect(() => KlpRouter(id: ' ', initial: destination.location(7), routes: [KlpRoute<int, String>(destination, screen: screen)]), throwsArgumentError);
		expect(() => KlpRouter(id: 'router', initial: destination.location(7), routes: []), throwsArgumentError);
	});
}
