...
void main() {
	...
	testWidgets('style replacement retains focus when region scrolling mode changes', (tester) async {
		// 小視窗讓完整原料替換跨越區域捲動門檻，只驗證互動生命週期。
		tester.view.devicePixelRatio = 1;
		tester.view.physicalSize = const Size(800, 60);
		addTearDown(tester.view.resetDevicePixelRatio);
		addTearDown(tester.view.resetPhysicalSize);
		final item = KlpComponentTestItem(id: 'a', label: 'A', action: KlpCallbackAction(() {}));
		...
		final selection = tester.widget<KlpFlutterChoice>(find.byType(KlpFlutterChoice)).content.selection;
	...
	testWidgets('throwing borrowed subscription cancellation still releases owned placements', (tester) async {
		// 外部來源的取消錯誤不可阻斷宿主所擁有資源的清理。
		final item = KlpComponentTestItem(id: 'a', label: 'A', action: KlpCallbackAction(() {}));
		final source = _ThrowingCancelSource(klpApplicationTestFixture(items: [item]));
		runKlpApp(source);
		await tester.pump();
		await tester.pump();
		final selection = tester.widget<KlpFlutterChoice>(find.byType(KlpFlutterChoice)).content.selection;
	...
	testWidgets('throwing old cancellation still installs replacement source and ignores stale events', (tester) async {
		// 取消失敗的舊來源仍可能派送事件；世代隔離必須守住新來源權威。
		final first = _ThrowingCancelSource(klpApplicationTestFixture(title: 'First'));
		final second = KlpMutableState(klpApplicationTestFixture(title: 'Second'));
		...
		await tester.pump();
		expect(tester.widget<WidgetsApp>(find.byType(WidgetsApp)).title, 'Current source');
	...
	testWidgets('initial source read failure preserves both read and cancellation causes', (tester) async {
		// 初始化失敗與外部取消失敗都應可追查，不互相掩蓋。
		final source = _ThrowingCancelSource(klpApplicationTestFixture(), throwOnRead: true);
		runKlpApp(source);
		await tester.pump();
		await tester.pump();
		...
		expect(issues.map((issue) => issue.error), [isA<ArgumentError>().having((error) => error.message, 'message', 'external read failed'), isA<StateError>().having((error) => error.message, 'message', 'external cancel failed')]);
...
final class _ThrowingCancelSource implements KlpState<KlpApplication> {
	...
	KlpSubscription subscribe(void Function(KlpApplication) listener) {
		_listener = listener;
		return subscription = KlpSubscription(() => throw StateError('external cancel failed'));
	}
