...
void main() {
	testWidgets('extent keeps semantic width within tight parent and fills cross axis', (tester) async {
		final text = klpRendererText('content');
		final extent = KlpBoundExtent(KlpAxis.horizontal, KlpDistance(48), text);
		final child = Directionality(textDirection: TextDirection.ltr, child: KlpFlutterRenderer(content: extent));
		await tester.pumpWidget(SizedBox.expand(child: child));
		final rendered = find.byWidgetPredicate((widget) => widget is KlpFlutterRenderer && identical(widget.content, text));
		expect(tester.getSize(rendered), const Size(48, 600));
		expect(tester.getTopLeft(rendered), Offset.zero);
		expect(tester.takeException(), isNull);
	});

	testWidgets('extent clamps to available space and follows directional start', (tester) async {
		final text = klpRendererText('content');
		final extent = KlpBoundExtent(KlpAxis.horizontal, KlpDistance(48), text);
		final child = Directionality(textDirection: TextDirection.rtl, child: KlpFlutterRenderer(content: extent));
		await tester.pumpWidget(SizedBox.expand(child: child));
		final rendered = find.byWidgetPredicate((widget) => widget is KlpFlutterRenderer && identical(widget.content, text));
		expect(tester.getTopLeft(rendered), const Offset(752, 0));
		final oversized = KlpBoundExtent(KlpAxis.horizontal, KlpDistance(900), text);
		await tester.pumpWidget(Directionality(textDirection: TextDirection.ltr, child: KlpFlutterRenderer(content: oversized)));
		expect(tester.getSize(rendered), const Size(800, 600));
		expect(tester.takeException(), isNull);
	});

	testWidgets('text conversion retains explicit style without inherited defaults', (tester) async {
	...
	testWidgets('small regions preserve access to trailing content without overflow', (tester) async {
		...
		final constrained = KlpBoundExtent(KlpAxis.vertical, KlpDistance(80), regions);
	...
	testWidgets('linear content scrolls instead of overflowing constrained main axis', (tester) async {
		final line = KlpBoundLinear(KlpAxis.horizontal, KlpDistance(10), [klpRendererText('first long text'), klpRendererText('second long text')]);
		final constrained = KlpBoundExtent(KlpAxis.horizontal, KlpDistance(60), line);
		await tester.pumpWidget(klpRendererHost(constrained));
		expect(tester.takeException(), isNull);
