...
void main() {
	...
	test('external item supports multiple interfaces and injected data callback', () {
		var presses = 0;
		void callback() => presses++;
		final action = KlpCallbackAction(callback);
		final item = KlpRailTestItem('custom', accessibilityLabel: 'Open notes', action: action);
	...
	test('registry validates rail and custom item in group order', () {
		final registry = KlpRegistry([KlpDefinition<KlpRail>(KlpRail.typeId, slots: [KlpRail.topSlot, KlpRail.centerSlot, KlpRail.bottomSlot]), KlpDefinition<KlpRailTestItem>('custom.railItem')]);
		final rail = KlpRail(id: 'rail', top: [KlpRailTestItem('a')], center: [KlpRailTestItem('b')], bottom: [KlpRailTestItem('c')]);
		expect(registry.validate(rail).nodes.map((node) => node.id), ['rail', 'a', 'b', 'c']);
	});

	test('registry rejects duplicate identity across different groups', () {
		final registry = KlpRegistry([KlpDefinition<KlpRail>(KlpRail.typeId, slots: [KlpRail.topSlot, KlpRail.centerSlot, KlpRail.bottomSlot]), KlpDefinition<KlpRailTestItem>('custom.railItem')]);
		final rail = KlpRail(id: 'rail', top: [KlpRailTestItem('same')], bottom: [KlpRailTestItem('same')]);
		final failure = isA<KlpContractError>().having((error) => error.code, 'code', 'duplicate_placement');
