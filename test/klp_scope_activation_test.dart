...
void main() {
	...
	KlpRail rail({String id = 'rail', String label = 'Item'}) => KlpRail(
		id: KlpId.parse(id),
		center: [
	...
	KlpScopeBoundary entry({bool active = true, String label = 'Item'}) =>
			KlpScopeBoundary(
				id: KlpId.parse('entry'),
				child: rail(label: label),
				active: active,
			);
	...
	test(
		'inactive scope blocks captured callbacks while retaining resource and selection',
		() async {
			update(entry());
	...
	test(
		'inactive ancestor cannot be reenabled by an active descendant',
		() async {
			KlpScopeBoundary nested(bool outer, bool inner) => KlpScopeBoundary(
				id: KlpId.parse('outer'),
				child: KlpScopeBoundary(
					id: KlpId.parse('inner'),
					child: rail(),
					active: inner,
				),
				active: outer,
	...
	test(
		'replaced and removed frames revoke all previous scoped callbacks',
		() async {
			KlpRuntimeNode tree(List<String> ids) => KlpRuntimeNode('root', [
				for (final id in ids) KlpScopeBoundary(id: KlpId.parse(id), child: rail()),
			]);
	...
	test('preparation failure keeps the previous active lease usable', () async {
		...
		expect(
			() => update(entry(active: false, label: ' ')),
	...
	test(
		'derived leases combine enabled ancestry and share frame revocation',
		() {
...
Iterable<KlpBoundChoice> _choices(KlpBoundTemplate content) sync* {
	switch (content) {
		case KlpBoundChoice():
			yield content;
		case KlpBoundPlacement():
			yield* _choices(content.content);
		case KlpBoundExtent():
			yield* _choices(content.child);
		case KlpBoundSurface():
			yield* _choices(content.child);
		case KlpBoundLinear():
