...
void main() {
	test('style changes preserve placement and existing bound snapshots', () {
		var callbacks = 0;
		final item = KlpComponentTestItem(action: KlpCallbackAction(() => callbacks++));
	...
	test('each text placement selects once and data creates a new snapshot', () {
		var calls = 0;
		final definition = klpComponentTestDefinition(select: (item) {
			calls++;
			return item.label;
		});
		...
		expect(oldRow.children.cast<KlpBoundText>().map((text) => text.text), ['First', 'First']);
		expect(nextRow.children.cast<KlpBoundText>().map((text) => text.text), ['Updated', 'Updated']);
		expect(next.id, first.id);
	});

	test('invalid node contracts fail before invoking selectors', () {
		var calls = 0;
		final compiler = KlpComponentCompiler([klpComponentTestDefinition(select: (item) {
			calls++;
			return item.label;
		})]);
		final invalid = <(KlpNode, String)>[
			...
			(KlpComponentTestItem(children: [KlpComponentTestItem(id: 'nested')]), 'component_children_unsupported'),
		];
		for (final (node, code) in invalid) {
			expect(() => compiler.bind(node, klpTestPrimitives()), _contractFailure(code));
		}
		expect(calls, 0);
	});
	...
	test('nested narrower templates fail before any sibling selector runs', () {
		...
		final broadText = KlpTextTemplate<KlpNode>(text: (_) {
			calls++;
			return 'broad';
		}, semantics: text.semantics);
		final narrower = <KlpTemplate<KlpNode>>[
			text,
			KlpLinearTemplate<KlpComponentTestItem>(axis: row.axis, children: [], gap: row.gap),
			KlpSurfaceTemplate<KlpComponentTestItem>(child: text, background: surface.background, radius: surface.radius, inset: surface.inset),
		];
		for (final template in narrower) {
			final content = KlpLinearTemplate<KlpNode>(axis: row.axis, children: [broadText, template], gap: row.gap);
			final definition = KlpComponentDefinition<KlpNode>('fixture', content: content, semantics: original.contract.semantics);
			final compiler = KlpComponentCompiler([definition]);
			final failure = isA<KlpContractError>().having((error) => error.code, 'code', 'template_type_mismatch');
			expect(() => compiler.bind(KlpComponentTestOtherNode(), klpTestPrimitives()), throwsA(failure));
			expect(calls, 0);
	...
	test('direct invalid template references fail at compilation', () {
		...
		for (final (key, dependencies, public, code) in cases) {
			final component = klpComponentTestDefinition(select: select, textColor: key, dependencies: dependencies);
			expect(() => KlpComponentCompiler([component], sharedDefinitions: [_shared(foreign, public)]), _contractFailure(code));
		}
		expect(calls, 0);
	});
	...
	test('declared public foreign template reference binds normally', () {
		final foreign = KlpSemanticKey('shared', 'color', KlpStyleKind.color);
		final component = klpComponentTestDefinition(textColor: foreign, dependencies: ['shared']);
		final compiler = KlpComponentCompiler([component], sharedDefinitions: [_shared(foreign, true)]);
		...
		expect((row.children.first as KlpBoundText).style.color, same(primitives.colors[2]));
	});
	...
	test('selector errors preserve cause stack and placement template path', () {
		...
		final component = klpComponentTestDefinition(select: (_) => Error.throwWithStackTrace(cause, trace));
		...
		final failure = isA<KlpComponentBindingException>()
			...
			.having((error) => error.templatePath, 'template', 'content/child/children/0');
		expect(() => compiler.bind(KlpComponentTestItem(), klpTestPrimitives()), throwsA(failure));
	});
	...
	test('component accessibility selector preserves its failure boundary', () {
		...
		final failure = isA<KlpComponentBindingException>()
			.having((error) => error.cause, 'cause', same(cause))
			.having((error) => error.stackTrace, 'stack', same(trace))
			.having((error) => error.templatePath, 'template', 'accessibilityLabel');
		expect(() => compiler.bind(KlpComponentTestItem(), klpTestPrimitives()), throwsA(failure));
	});
}

Matcher _contractFailure(String code) => throwsA(isA<KlpContractError>().having((error) => error.code, 'code', code));

KlpDefinition<KlpNode> _shared(KlpSemanticKey<KlpColor> color, bool public) => KlpDefinition(
	'shared',
	semantics: KlpSemanticSchema('shared', [KlpSemanticToken(color, const KlpPrimitiveRef(KlpStyleKind.color, KlpPrimitiveIndex.i2), isPublic: public)]),
);
