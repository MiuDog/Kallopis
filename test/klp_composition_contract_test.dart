...
void main() {
	test('typed slots permit a node with multiple qualifications', () {
		final item = KlpTestItem(KlpId.parse('item'), 'item');
		final rail = KlpTestRail(KlpId.parse('root'), [item]);
		final menuItems = <KlpTestMenuItem>[item];
		final registry = KlpRegistry([
			KlpDefinition<KlpTestRail>('rail'),
			KlpDefinition<KlpTestRailItem>('item'),
		]);
	...
	test('registry and dependencies keep immutable input snapshots', () {
		...
		final definitions = <KlpDefinition<KlpNode>>[
			KlpDefinition<KlpNode>('root', dependencies: dependencies),
			KlpDefinition<KlpNode>('leaf'),
		];
		...
		expect(
			() => registry.definition('root').dependencies.clear(),
	...
	test('registry rejects duplicate and unknown definitions', () {
		...
		expect(
			() => KlpRegistry([
				KlpDefinition<KlpNode>('root', dependencies: ['missing']),
			]),
			failsWith('unknown_definition'),
		);
		expect(
			() => KlpRegistry([]).definition('missing'),
			failsWith('unknown_definition'),
		);
	});
	...
	test('registry rejects direct and transitive dependency cycles', () {
		expect(
			() => KlpRegistry([
				KlpDefinition<KlpNode>('root', dependencies: ['root']),
			]),
			failsWith('dependency_cycle'),
		);
		final definitions = [
			KlpDefinition<KlpNode>('a', dependencies: ['b']),
			KlpDefinition<KlpNode>('b', dependencies: ['a']),
		];
	...
	test('shared dependency is valid', () {
		final definitions = [
			KlpDefinition<KlpNode>('root', dependencies: ['a', 'b']),
			KlpDefinition<KlpNode>('a', dependencies: ['b']),
			KlpDefinition<KlpNode>('b'),
		];
	...
	test('registry rejects empty registration and dependency identifiers', () {
		expect(
			() => KlpRegistry([KlpDefinition<KlpNode>(' ')]),
			failsWith('empty_id'),
		);
		expect(
			() => KlpRegistry([
				KlpDefinition<KlpNode>('root', dependencies: ['']),
			]),
			failsWith('empty_id'),
		);
	});
	...
	test('tree rejects empty identifiers and unknown definitions', () {
		...
		expect(
			() => registry.validate(KlpTestItem(KlpId.parse('node'), ' ')),
			failsWith('empty_id'),
		);
		expect(
			() => registry.validate(KlpTestItem(KlpId.parse('node'), 'missing')),
			failsWith('unknown_definition'),
		);
		expect(
			() => registry.validate(KlpTestItem(KlpId.parse('node'), 'missing')),
			throwsA(
				isA<KlpContractError>().having(
					(error) => error.message,
					'message',
					contains('placement=node definition=missing'),
				),
			),
		);
	});
	...
	test('tree rejects duplicate placements and identity cycles', () {
		final registry = KlpRegistry([KlpDefinition<KlpNode>('item')]);
		final root = KlpTestItem(KlpId.parse('root'), 'item', [
			KlpTestItem(KlpId.parse('same'), 'item'),
			KlpTestItem(KlpId.parse('same'), 'item'),
		]);
	...
	test('definition qualification cannot accept an incompatible node', () {
		...
		expect(
			() => registry.validate(KlpTestItem(KlpId.parse('node'), 'rail')),
			failsWith('node_type_mismatch'),
		);
	});

	test('validated tree remains frozen when consumer nodes change', () {
		final registry = KlpRegistry([KlpDefinition<KlpNode>('item')]);
		final child = KlpTestItem(KlpId.parse('child'), 'item');
		final root = KlpTestItem(KlpId.parse('root'), 'item', [child]);
		final result = registry.validate(root);
		root.id = KlpId.parse('changed');
		child.id = KlpId.parse('changed-child');
		...
		expect(
			() => result.nodes.first.childrenIds.clear(),
			throwsUnsupportedError,
		);
	});
}
