...
void main() {
	...
	FutureOr<KlpNavigationStart> start({
		...
		return KlpNavigationMachine.start(
			policies: policies ?? [KlpRoutePolicy(destination: home, beforeEnter: guard)],
			initial: home.location(parameter),
			commit: (snapshot, {required replaceRouteInformation}) => commits.add(snapshot),
			cancellation: KlpNavigationCancellation(cancellation.future, () => cancellation.isCompleted),
		);
	}
	...
	test('synchronous initial guard sees absent origin and exact committed candidate', () {
		late KlpNavigationSnapshot candidate;
		final result = start(guard: (transition) {
	...
	test('synchronous denial and thrown guard never commit', () {
		...
		expect(failed.decision.outcome, isA<KlpNavigationFailed<void>>().having((value) => value.error, 'error', same(error)));
		expect(commits, isEmpty);
	});
	...
	for (final throwsError in [false, true]) {
		test('asynchronous guard rejection or failure does not commit $throwsError', () async {
			final guard = Completer<bool>();
			final pending = start(guard: (_) => guard.future);
			if (throwsError) {
			...
			expect(result.decision.outcome, throwsError ? isA<KlpNavigationFailed<void>>() : isA<KlpNavigationRejected<void>>());
			expect(commits, isEmpty);
		});
	}
	...
	test('pre-cancelled bootstrap never invokes its guard', () {
		...
		var guards = 0;
		final result = start(cancel: cancel, guard: (_) { guards++; return true; }) as KlpNavigationStart;
	...
	for (final lateError in [false, true]) {
		test('new source commits while cancelled old guard cannot overwrite it $lateError', () async {
			final guard = Completer<bool>();
			final cancel = Completer<void>();
			final old = start(guard: (_) => guard.future, cancel: cancel, parameter: 1);
			cancel.complete();
			final current = start(parameter: 2) as KlpNavigationStart;
			addTearDown(current.machine!.dispose);
			expect((await old).decision.outcome, isA<KlpNavigationCancelled<void>>());
			if (lateError) {
	...
	test('cancellation inside a synchronous guard is checked before commit', () {
		final cancel = Completer<void>();
		final result = start(cancel: cancel, guard: (_) { cancel.complete(); return true; }) as KlpNavigationStart;
		expect(result.machine, isNull);
		expect(result.decision.outcome, isA<KlpNavigationCancelled<void>>());
		expect(commits, isEmpty);
	});

	test('initial commit failures preserve declared authority and reject unknown authority', () {
		final cancel = Completer<void>();
		final signal = KlpNavigationCancellation(cancel.future, () => cancel.isCompleted);
		for (final committed in [false, true]) {
			final result = KlpNavigationMachine.start(policies: [KlpRoutePolicy(destination: home)], initial: home.location(0), cancellation: signal, commit: (_, {required replaceRouteInformation}) => throw KlpNavigationCommitException(committed, StateError('commit'), StackTrace.current)) as KlpNavigationStart;
		...
		expect(() => KlpNavigationMachine.start(policies: [KlpRoutePolicy(destination: home)], initial: home.location(0), cancellation: signal, commit: (_, {required replaceRouteInformation}) => throw StateError('unknown')), throwsA(isA<KlpNavigationCommitContractException>()));
	});
}
