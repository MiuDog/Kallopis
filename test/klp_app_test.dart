...
void main() {
	testWidgets('stage body paints above overflowing window header chrome', (
		...
		await tester.pumpWidget(
			KlpApp(
				...
				home: KlpPanelFrame(content: GestureDetector(
					behavior: HitTestBehavior.opaque,
	...
	Future<(KlpAppController, KlpThemeData)> pumpAndRead(
	  ...
	  await tester.pumpWidget(
	    KlpApp(
	      ...
	      router: router,
				home: KlpPanelFrame(content: Builder(
	        builder: (context) {
	...
	testWidgets('視窗轉場暫時低於 header 高度時不產生垂直溢出', (tester) async {
	  ...
	  await tester.pumpWidget(
			const KlpApp(title: 'Notist', home: KlpPanelFrame(content: SizedBox.expand())),
	  );
	...
	testWidgets('KlpApp 預設只在首次建立時確保視窗最大化', (tester) async {
	  ...
	  await tester.pumpWidget(
			const KlpApp(key: ValueKey('app'), home: KlpPanelFrame(content: SizedBox.shrink())),
	  );
	  await tester.pump();
	  await tester.pumpWidget(
			const KlpApp(key: ValueKey('app'), home: KlpPanelFrame(content: SizedBox.shrink())),
	...
	testWidgets('原生視窗已最大化時不切換回視窗化', (tester) async {
	  ...
	  addTearDown(() => messenger.setMockMethodCallHandler(windowChannel, null));
		...
		await tester.pumpWidget(const KlpApp(home: KlpPanelFrame(content: SizedBox.shrink())));
	  await tester.pump();
	...
	testWidgets('消費端可停用啟動最大化', (tester) async {
	  ...
	  await tester.pumpWidget(
			const KlpApp(startMaximized: false, home: KlpPanelFrame(content: SizedBox.shrink())),
	  );
	  await tester.pump();
	...
	test('最小視窗尺寸必須是正的邏輯像素', () {
	  expect(
			() => KlpApp(minWidth: 0.0, home: KlpPanelFrame(content: const SizedBox.shrink())),
	    throwsAssertionError,
	  );
	  expect(
			() => KlpApp(minHeight: -1.0, home: KlpPanelFrame(content: const SizedBox.shrink())),
	    throwsAssertionError,
	  );
	});
