  ...
  testWidgets('Settings 所有顏色模式都維持左深右淺與可讀文字', (tester) async {
    final themes = <(String, ThemeData)>[
      for (final variant in KlpThemeVariant.values)
        (variant.name, buildKlpThemeVariant(variant)),
    ...
    for (final (label, theme) in themes) {
      ...
      expect(
        navigationColor.computeLuminance(),
        lessThan(contentColor.computeLuminance()),
        reason: label,
      );
      expect(
        _contrast(
          navigationColor,
          _textColor(tester, const ValueKey('navigation-text')),
        ),
      ...
      expect(
        _contrast(
          contentColor,
          _textColor(tester, const ValueKey('content-text')),
        ),
        greaterThanOrEqualTo(4.5),
...
Widget _specimen(ThemeData theme) {
  final page = KlpSettingsPage(
    navigation: const KlpSettingsNavigationPane(
      children: [KlpText('Navigation', key: ValueKey('navigation-text'))],
    ),
    content: const KlpSettingsContentPane(
      title: 'Settings',
      child: KlpText('Content', key: ValueKey('content-text')),
    ),
  );
  return MaterialApp(
    theme: theme,
    home: SizedBox(width: 1000, height: 600, child: page),
  );
}

Color _background(WidgetTester tester, Finder pane) {
  final surface = find
      .descendant(of: pane, matching: find.byType(KlpSurface))
      .first;
