...
void main() {
  ...
  setUp(() {
    root = Directory.systemTemp.createTempSync('kallopis-style-source-');
  });
  tearDown(() {
  ...
  test(
    'Style trace discovers public widgets and private styles in owned parts',
    () {
  ...
  test(
    'Public library source discovery excludes imports and includes owned parts',
    () {
  ...
  test(
    'Owned library source discovery resolves an internal part back to its owner',
    () {
      final owner = write(
        'owner.dart',
        "part 'internal/widget.dart';\npart 'internal/state.dart';",
      );
      final widget = write('internal/widget.dart', "part of '../owner.dart';");
      write('internal/state.dart', "part of '../owner.dart';");
      final paths = ownedLibrarySources(
        widget,
      ).map((file) => file.absolute.uri.normalizePath()).toSet();
  ...
  test(
    'Public source traversal accepts transitive exports and owned parts',
    () {
      ...
      expect(
        publicLibraryViolations(entry, Directory('${root.path}/src')),
        isEmpty,
      );
    },
  );
  ...
  test('Public source traversal rejects hidden and orphan source files', () {
    ...
    final issues = publicLibraryViolations(
      entry,
  ...
  test(
    'Public source traversal rejects direct part export and wrong owner',
    () {
      final entry = write(
        'entry.dart',
  ...
  test('Isolated public entries reject shared sources and cross exports', () {
    ...
    expect(
      issues.every(
        (issue) => issue.startsWith('Shared source across isolated entries:'),
      ),
      isTrue,
    );
  });
  ...
  for (final directive in [
    "export 'src/legacy.dart' show Legacy;",
    "export 'src/legacy.dart' hide Legacy;",
    "export\n'src/legacy.dart'\nshow Legacy;",
    "export 'src/next.dart' if (dart.library.io) 'src/legacy.dart';",
  ]) {
  ...
  for (final uri in [
    'package:kallopis/src/legacy.dart',
    'package:foreign/legacy.dart',
  ]) {
  ...
  test('AST resolves named and multiline part owners', () {
    ...
    expect(
      publicLibraryViolations(entry, Directory('${root.path}/src')).join('\n'),
      contains('Part owner mismatch'),
    );
  });
}
