...
void main() {
  ...
  test('invalid update preserves previous tree and releases busy guard', () {
    ...
    expect(
      () => fixture.installation.update(_tree([_item('root')])),
      throwsA(isA<KlpContractError>()),
    );
    ...
    expect(
      fixture.installation.resources.keys.map((identity) => identity.localId),
      ['root', 'valid'],
    );
  });
  ...
  test('same identities retain resources and suppress no op notifications', () {
    ...
    final root =
        installation.resources[KlpPlacementId(localId: 'root')]!
            as KlpDefaultPlacement;
    var notifications = 0;
  ...
  test('creation failure rolls back new resources despite cleanup failure', () {
    ...
    fixture.events.clear();
    final failure = _capture(
      () => fixture.installation.update(
        _tree([_item('a'), _item('b'), _item('fail')]),
      ),
    );
    expect(failure.committed, isFalse);
    expect(failure.issues.map((issue) => issue.error), [
      fixture.creationError,
  ...
  test('notification failure commits and still cleans removed placements', () {
    ...
    fixture.events.clear();
    final failure = _capture(
      () => fixture.installation.update(_tree([_item('new')])),
    );
  ...
  test(
    'creation rejects nested update and dispose without unlocking outer transaction',
    () {
      fixture.onCreate = () {
        expect(() => fixture.installation.update(_tree()), throwsStateError);
  ...
  test(
    'default placement disposal releases controller state and subscription',
    () {
      final installation = KlpInstallation(KlpInstallationFixture.registry());
