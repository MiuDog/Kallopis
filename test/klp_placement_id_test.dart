...
void main() {
	...
	test('scope is snapshotted and cannot change a map key after insertion', () {
		...
		expect(values[KlpPlacementId(scope: ['router', 'entry'], localId: 'item')], 'resource');
		expect(() => identity.scope.clear(), throwsUnsupportedError);
	});

	test('scope segments and local identifier never collapse across delimiters', () {
		final identities = [
	...
	test('different scopes isolate equal local identifiers and preserve order', () {
		final first = KlpPlacementId(scope: ['first', 'second'], localId: 'same');
		final reversed = KlpPlacementId(scope: ['second', 'first'], localId: 'same');
		final sibling = KlpPlacementId(scope: ['first', 'third'], localId: 'same');
		expect({first, reversed, sibling}, hasLength(3));
	});

	test('identity preserves unicode whitespace and empty scope without normalization', () {
		...
		expect(KlpPlacementId(localId: '項目'), KlpPlacementId(scope: [], localId: '項目'));
