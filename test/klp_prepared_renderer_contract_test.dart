...
// 獨立列舉接受基線的 26 個 concrete 呈現變體，避免與產品登錄表同源。
const _generic = {
...
void main() {
	...
	catalog.main();
	test('real catalog retains all 28 baseline IDs in order', () {
		final actual = klpApplicationAdapters().map((adapter) => adapter.contract.id).toList();
		expect(_catalogIds.length, 28);
		expect(actual, _catalogIds);
		expect(actual.toSet().length, actual.length);
	});
	test('all concrete prepared source declarations match 10 generic 6 editing 10 workspace variants', () {
		expect([_generic.length, _editing.length, _workspace.length], [10, 6, 10]);
		final expected = {..._generic, ..._editing, ..._workspace};
		final actual = <String>[];
		...
		expect(actual.length, 26, reason: '不可用重複宣告填補變體數量');
	});
	test('renderer handles every concrete variant once and labels six nonvisual variants', () {
		...
		expect(branches.keys.toSet(), {..._generic, ..._editing, ..._workspace});
