...
void main() {
	...
	test('semantic spacing and exact geometry copy independently and round trip', () {
		...
		final spacingCases = <(String, KlpSpacingTheme)>[
			...
			('navigationRailItemGap', base.spacing.copyWith(navigationRailItemGap: 37)),
			...
			('navigationSectionGap', base.spacing.copyWith(navigationSectionGap: 37)),
			('appFrameInset', base.spacing.copyWith(appFrameInset: 37)),
			('workbenchContentInset', base.spacing.copyWith(workbenchContentInset: 37)),
		...
		for (final (key, value) in spacingCases) {
			...
			final expected = <Object?, Object?>{...original['spacing'] as Map, key: 37.0};
		...
		final controlCases = <(String, KlpControlGeometry)>[
			('pageBackgroundHitRadius', base.geometry.control.copyWith(pageBackgroundHitRadius: 37)),
			('presenceMarkerExtent', base.geometry.control.copyWith(presenceMarkerExtent: 37)),
			('colorPickerCursorRadius', base.geometry.control.copyWith(colorPickerCursorRadius: 37)),
			('swatchExtent', base.geometry.control.copyWith(swatchExtent: 37)),
			('segmentedProgressHeight', base.geometry.control.copyWith(segmentedProgressHeight: 37)),
		];
		for (final (key, value) in controlCases) {
			final style = base.copyWith(geometry: base.geometry.copyWith(control: value));
			final encoded = KlpVisualStyleJson.encode(style);
			final expected = <Object?, Object?>{...(original['geometry'] as Map)['control'] as Map, key: 37.0};
		...
		final layoutCases = <(String, KlpLayoutGeometry)>[
			('resizeHandleExtent', base.geometry.layout.copyWith(resizeHandleExtent: 37)),
			('overlayViewportInset', base.geometry.layout.copyWith(overlayViewportInset: 37)),
			('railDropTargetExtent', base.geometry.layout.copyWith(railDropTargetExtent: 37)),
			('disclosureIconSize', base.geometry.layout.copyWith(disclosureIconSize: 37)),
		...
		for (final (key, value) in layoutCases) {
			final style = base.copyWith(geometry: base.geometry.copyWith(layout: value));
			final encoded = KlpVisualStyleJson.encode(style);
			final expected = <Object?, Object?>{...(original['geometry'] as Map)['layout'] as Map, key: 37.0};
	...
	test('legacy compact migrates across spacing and geometry without mutating input', () {
		for (final version in <int?>[null, 1]) {
			...
			for (final key in <String>['contentInlineGap', 'contentStackGap', 'contentInset', 'controlContentGap', 'controlInset', 'actionGap', 'chromeGap', 'chromePanelInset', 'chromeToolbarGap', 'navigationItemInset', 'navigationRailInset', 'navigationRailItemGap', 'overlayContentInset', 'overlayHeadingGap', 'overlayItemGap']) {
				expect(spacing[key], 12.0, reason: key);
			}
			for (final key in <String>['appFrameInset', 'workbenchContentInset', 'windowHeaderMargin', 'dockMargin']) {
				expect(spacing[key], 6.0, reason: key);
			}
			expect(spacing['navigationSectionGap'], 9.0);
			for (final key in <String>['pageBackgroundHitRadius', 'presenceMarkerExtent', 'colorPickerCursorRadius', 'swatchExtent', 'segmentedProgressHeight']) {
				expect(((encoded['geometry'] as Map)['control'] as Map)[key], 12.0, reason: key);
			}
			for (final key in <String>['resizeHandleExtent', 'overlayViewportInset', 'railDropTargetExtent', 'disclosureIconSize', 'treeLeadingGap', 'tooltipOffsetX']) {
				expect(((encoded['geometry'] as Map)['layout'] as Map)[key], 12.0, reason: key);
			}
	...
	test('explicit semantic fields win over legacy migration and preserve custom base', () {
		final base = KlpVisualStyle.defaultStyle.copyWith(spacing: KlpVisualStyle.defaultStyle.spacing.copyWith(actionGap: 29));
		final result = KlpVisualStyleJson.decode({
			'spacing': {'compact': 12, 'controlContentGap': 17, 'appFrameInset': 11},
			'geometry': {'control': {'pageBackgroundHitRadius': 19}, 'layout': {'overlayViewportInset': 23}},
		}, base: base);
		...
		expect(KlpVisualStyleJson.decode({'schemaVersion': 2, 'spacing': {'contentInlineGap': 13}}, base: base).spacing.actionGap, 29);
		expect(KlpVisualStyleJson.decode({'schemaVersion': 1}, base: base).spacing.actionGap, 29);
	});

	test('legacy hairline recomputes navigation gap and narrow compact remains loadable', () {
		final hairlineOnly = KlpVisualStyleJson.decode({'schemaVersion': 1, 'spacing': {'hairline': 3}});
		final narrowCompact = KlpVisualStyleJson.decode({'schemaVersion': 1, 'spacing': {'compact': 1}});

		expect(hairlineOnly.spacing.navigationSectionGap, 5);
		expect(narrowCompact.spacing.navigationSectionGap, 0);
	});

	test('v2 rejects compact and validates independently injected semantic fields', () {
		expect(() => KlpVisualStyleJson.decode({'schemaVersion': 2, 'spacing': {'compact': 8}}), throwsA(_formatExceptionContaining('spacing.compact')));
		expect(() => KlpVisualStyleJson.decode({'spacing': {'compact': 'wide'}}), throwsA(_formatExceptionContaining('spacing.compact')));
		expect(() => KlpVisualStyleJson.decode({'schemaVersion': 2, 'spacing': {'actionGap': -1}}), throwsA(_formatExceptionContaining('spacing.actionGap')));
		expect(() => KlpVisualStyleJson.decode({'schemaVersion': 2, 'geometry': {'control': {'presenceMarkerExtent': -1}}}), throwsA(_formatExceptionContaining('geometry.control.presenceMarkerExtent')));
		expect(() => KlpVisualStyleJson.decode({'schemaVersion': 2, 'geometry': {'layout': {'resizeHandleExtent': 'wide'}}}), throwsA(_formatExceptionContaining('geometry.layout.resizeHandleExtent')));
	});
	...
	group('KlpVisualStyleJson', () {
		...
		expect((geometry['control'] as Map).length, 32);
			expect((geometry['data'] as Map).length, 21);
			expect((geometry['layout'] as Map).length, 32);
