...
void main() {
  testWidgets('KlpButton 預設為 sm 且 compact 為 xs', (tester) async {
    for (final style in [KlpVisualStyle.defaultStyle, contrastingStyle]) {
      ...
      expect(
        tester.getSize(_button(const ValueKey('default'))).height,
        style.geometry.control.buttonHeightSmall,
      );
      expect(
        tester.getSize(_button(const ValueKey('compact'))).height,
        style.geometry.control.buttonHeightXSmall,
      );
    }
  });
  ...
  testWidgets('KlpButton hover 與 selected 使用不同語意 wash', (tester) async {
    ...
    expect(
      tester
          .getSemantics(_button(const ValueKey('selected')))
          .flagsCollection
          .isSelected,
      Tristate.isTrue,
    );
  ...
  testWidgets('全部 selected tone 在暗色模式維持緊湊布局', (tester) async {
    for (final tone in KlpButtonTone.values) {
      ...
      expect(
        ...
        isNull,
        reason: '${tone.name} selected overflow',
      );
    }
  });
...
Widget _isolatedButton({
  KlpButtonTone tone = KlpButtonTone.primary,
  KlpControlSize? size,
  bool selected = false,
}) {
  final home = Center(
...
Finder _button(Key key) => find.byWidgetPredicate(
  (widget) => widget is KlpButton && widget.key == key,
);
