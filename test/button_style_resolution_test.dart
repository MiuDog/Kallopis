...
void main() {
	test('public entry exposes primitive scale and palette', () {
		...
		expect(semantic.KlpShapeTheme.standardShape.stroke, primitive.KlpScale.stroke200);
	});

	test('JSON overrides reach resolved button slots without replacing defaults', () {
		...
		final custom = KlpVisualStyleJson.decode({
			...
			'components': {'buttonPaddingX': 31.0, 'buttonRadius': 5.0, 'buttonBorderWidth': 3.0},
		});
		...
		expect(medium.insets.vertical, 0, reason: 'Existing button content uses horizontal padding only.');
		expect(small.insets.horizontal, 34, reason: 'MD component overrides must not replace the small size mapping.');
		...
		expect(KlpVisualStyleJson.encode(KlpVisualStyleJson.decode(KlpVisualStyleJson.encode(custom))), KlpVisualStyleJson.encode(custom));
	});

	test('fresh theme snapshot replaces previously resolved button values', () {
		final first = _resolve(_tokens(KlpVisualStyle.defaultStyle), KlpControlSize.md);
		final next = KlpVisualStyle.defaultStyle.copyWith(
			spacing: KlpVisualStyle.defaultStyle.spacing.copyWith(controlContentGap: 19),
	...
	test('button control spacing is isolated from content and action spacing', () {
		...
		expect(medium.insets, _resolve(_tokens(original), KlpControlSize.md).insets);
	});
	...
	test('invalid JSON fails before a style can be injected', () {
		expect(() => KlpVisualStyleJson.decode({'spacing': {'controlContentGap': 'wide'}}), throwsFormatException);
		expect(() => KlpVisualStyleJson.decode({'schemaVersion': 999}), throwsFormatException);
	});
}
