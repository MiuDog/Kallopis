...
final item = ExternalItem(KlpId.parse('item'));
final panel = ExternalPanel(KlpId.parse('panel'), [item]);
''';
...
void main() {
	...
	final cases = <({String name, String source, String code})>[
		(
			name: 'unqualified child',
			source: 'final invalid = slot.assign([OtherNode()]);',
			code: 'list_element_type_not_assignable',
		),
		(
			name: 'native widget child',
			source: 'final invalid = slot.assign([const SizedBox()]);',
			code: 'list_element_type_not_assignable',
		),
		(
			name: 'native widget qualification',
			source:
					"final invalid = KlpSlot<Widget>(owner: 'external.panel', name: 'widgets');",
			code: 'type_argument_not_matching_bounds',
		),
		(
			name: 'raw children without assignment',
			source: 'final invalid = KlpChildren([item]);',
			code: 'list_element_type_not_assignable',
		),
		(
			name: 'assignment direct construction',
			source: 'final invalid = KlpSlotAssignment<CardItem>(slot, [item]);',
			code: 'new_with_undefined_constructor_default',
		),
		(
			name: 'plain iterable children getter',
			source:
					"final class Invalid implements KlpCompositeNode { @override String get id => 'bad'; @override String get definitionId => 'bad'; @override Iterable<KlpNode> get children => const []; }",
			code: 'invalid_override',
		),
		(
			name: 'list children getter',
			source:
					"final class Invalid implements KlpCompositeNode { @override String get id => 'bad'; @override String get definitionId => 'bad'; @override List<KlpNode> get children => const []; }",
			code: 'invalid_override',
		),
		(
			name: 'missing children getter',
			source:
					"final class Invalid implements KlpCompositeNode { @override String get id => 'bad'; @override String get definitionId => 'bad'; }",
			code: 'non_abstract_class_inherits_abstract_member_one',
		),
		(
			name: 'assignment style override',
			source: 'final invalid = slot.assign([item], gap: gap);',
			code: 'undefined_named_parameter',
		),
		(
			name: 'children raw style',
			source: 'final invalid = KlpChildren([slot.assign([item])], gap: 12);',
			code: 'undefined_named_parameter',
		),
		(
			name: 'template raw gap',
			source:
					'final invalid = KlpChildrenTemplate<ExternalPanel, CardItem>(slot: slot, axis: KlpAxis.vertical, gap: KlpDistance(12));',
			code: 'argument_type_not_assignable',
		),
		(
			name: 'template wrong qualification',
			source:
					'final invalid = KlpChildrenTemplate<ExternalPanel, OtherNode>(slot: slot, axis: KlpAxis.vertical, gap: gap);',
			code: 'argument_type_not_assignable',
		),
		(
			name: 'template children selector',
			source:
					'final invalid = KlpChildrenTemplate<ExternalPanel, CardItem>(slot: slot, axis: KlpAxis.vertical, gap: gap, children: (panel) => [item]);',
			code: 'undefined_named_parameter',
		),
		(
			name: 'template widget builder',
			source:
					'final invalid = KlpChildrenTemplate<ExternalPanel, CardItem>(slot: slot, axis: KlpAxis.vertical, gap: gap, builder: (BuildContext context) => const SizedBox());',
			code: 'undefined_named_parameter',
		),
	];
	...
	for (final type in [
		'KlpChildren',
		'KlpSlot<CardItem>',
		'KlpSlotAssignment<CardItem>',
		'KlpChildrenTemplate<ExternalPanel, CardItem>',
	]) {
		for (final relation in ['extends', 'implements']) {
			final code = relation == 'extends'
					? 'final_class_extended_outside_of_library'
					: 'final_class_implemented_outside_of_library';
			cases.add((
				name: '$relation $type',
				source: 'abstract final class Escape $relation $type {}',
				code: code,
			));
		}
	}
	...
	setUpAll(() async {
		fixture = await KlpExternalCompileFixture.create({
			'positive':
					"${_prefix}final screen = KlpScreen(id: KlpId.parse('screen'), accessibilityLabel: 'Screen', child: panel);\nfinal rail = KlpRail(id: KlpId.parse('rail'), top: [item]);\n",
	...
	test(
		'external composite uses public slots and supports multiple child qualifications',
		() async {
