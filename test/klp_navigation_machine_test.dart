...
void main() {
  ...
  setUp(() {
    ...
    machine = klpNavigationTestMachine(
      policies: [
        ...
        KlpRoutePolicy(destination: detail),
      ],
  ...
  test(
    'retains entry identity and completes nullable results exactly once',
    () async {
      ...
      expect(
        machine.state.value.entries.map((entry) => entry.id).toSet(),
        hasLength(3),
      );
      ...
      expect(
        await second.result,
        isA<KlpNavigationCompleted<String?>>().having(
  ...
  test('validates original generic types after widening', () async {
    ...
    expect(
      (await machine.complete(home, 'wrong destination')).committed,
      isFalse,
    );
  });
  ...
  test(
    'pending guards preserve state and reject concurrent requests',
    () async {
      final guard = Completer<bool>();
      machine.replacePolicies([
        KlpRoutePolicy(destination: home),
        KlpRoutePolicy(destination: detail, beforeEnter: (_) => guard.future),
      ]);
      final ticket = machine.push(detail.location(1));
      final busy = machine.push(detail.location(2));
      expect(
        (await busy.decision).outcome,
        isA<KlpNavigationRejected<void>>().having(
  ...
  test(
    'source replacement invalidates pending guards and ignores late failures',
    () async {
      final guard = Completer<bool>();
      machine.replacePolicies([
        KlpRoutePolicy(destination: home),
  ...
  test(
    'registry replacement cannot remove retained destinations or duplicate IDs',
    () async {
      final ticket = machine.push(detail.location(1));
      await ticket.decision;
      expect(
        () => machine.replacePolicies([KlpRoutePolicy(destination: home)]),
        throwsArgumentError,
      );
      expect(
        () => machine.replacePolicies([
  ...
  test(
    'leave guard rejection does not complete the retained screen result',
    () async {
      final ticket = machine.push(detail.location(1));
      await ticket.decision;
      var completed = false;
      ...
      machine.replacePolicies([
        KlpRoutePolicy(destination: home),
        KlpRoutePolicy(destination: detail, beforeLeave: (_) => false),
      ]);
      expect((await machine.pop()).committed, isFalse);
      await Future<void>.delayed(Duration.zero);
  ...
  test(
    'restoration runs every candidate guard and cancels replaced results',
    () async {
      final entered = <int>[];
      machine.replacePolicies([
        KlpRoutePolicy(
      ...
      final restored = await machine.restore([
        home.location(2),
        detail.location(3),
      ]);
      expect(restored.committed, isTrue);
      expect(
        machine.state.value.entries.map((entry) => entry.location.parameters),
        [2, 3],
      );
      expect(entered, [1, 3, 3]);
      expect(
        await retained.result,
        isA<KlpNavigationCancelled<String?>>().having(
          (outcome) => outcome.reason,
          'reason',
          'restored',
        ),
  ...
  test(
    'dispose settles pending and retained results and revokes the state',
    () async {
      final retained = machine.push(detail.location(1));
      await retained.decision;
      final guard = Completer<bool>();
