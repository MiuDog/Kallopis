...
void main() {
  ...
  test('tokens contain primitives without a semantic forwarding barrel', () {
    expect(
      File('lib/src/styling/legacy_tokens/primitive_token.dart').existsSync(),
      isTrue,
    );
    expect(
      File('lib/src/styling/legacy_tokens/semantic_token.dart').existsSync(),
      isFalse,
    );
  });

  test(
    'action region exposes typed style and keeps Flutter UI in primitives',
    () {
      final entry = File(
        'lib/src/foundation/interaction/klp_action_region.dart',
      ).readAsStringSync();
      final primitive = File(
        'lib/src/foundation/interaction/primitives/klp_action_region_widget.dart',
      ).readAsStringSync();
      final publicLibrary = File(
  ...
  test('command menu composition uses typed Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/actions/command_menu'),
    )) {
  ...
  test('navigation rail does not depend on shell composition', () {
    final violations = <String>[];
    for (final file in _dartFiles(
      Directory('lib/src/features/navigation/widgets/rail'),
    )) {
      if (RegExp(
  ...
  test('navigation rail composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Navigation rail 組裝層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });

  test(
    'navigation rail implementation files contain one structural definition',
    () {
      final declaration = RegExp(
        r'^\s*(?:(?:abstract|base|final|interface|sealed)\s+)*'
        r'(?:class|enum|extension|mixin|typedef)\s+',
      ...
      expect(
        violations,
        isEmpty,
        reason: 'Navigation rail 實作每檔只能有一份結構定義：\n${violations.join('\n')}',
      );
    },
  );
  ...
  test('window components dogfood Kallopis primitives', () {
    final protectedPath = RegExp(
      r'lib/src/features/workspace/shell/window/.+\.dart$',
    );
    final nativeUi = RegExp(
  ...
  test('window implementation files contain one structural definition', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/workspace/shell/window'),
    )) {
      final count = declaration.allMatches(file.readAsStringSync()).length;
      if (count > 1) {
  ...
  test('app screen composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/workspace/shell/composition/app_screen'),
    )) {
      final path = _relativePath(file);
    ...
    expect(
      violations,
      isEmpty,
      reason: 'App screen 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('app screen files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 App screen Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('stage header dogfoods Kallopis layout primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Stage header 只能使用 Kallopis 排版原語：\n${violations.join('\n')}',
    );
  ...
  test('stage top bar composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Stage top bar 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('stage top bar files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File('lib/src/features/workspace/shell/stage/klp_stage_tab.dart'),
      ..._dartFiles(
        Directory('lib/src/features/workspace/shell/stage/primitives'),
      ).where((file) => _relativePath(file).contains('klp_stage_tab_')),
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Stage top bar Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('settings components dogfood Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/workspace/settings'),
    )) {
      final path = _relativePath(file);
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
        violations.add('$path:${match.start} ${match.group(0)}');
  ...
  test('overlay composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/overlays/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
  ...
  test('feedback composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/feedback/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
      r'\b(?:Align|Center|Column|ConstrainedBox|Container|DecoratedBox|Dialog|Expanded|Flexible|Focus|GestureDetector|Icon|IconButton|InkWell|LayoutBuilder|ListView|Material|MouseRegion|Padding|Positioned|PositionedDirectional|Row|Semantics|SizedBox|Spacer|Stack|StatefulBuilder|Text|TextField|Tooltip|Transform|Wrap)\s*(?:\.|<|\()',
    );
  ...
  test('button composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/actions/button'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('selection composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/forms/selection'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('form selection fields dogfood Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      ...
      final path = _relativePath(file);
      final inScope =
          path.contains('klp_multi_select_') ||
  ...
  test('form selection fields expose typed style interfaces', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Form selection 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('form selection field files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/foundation/interaction/primitives/klp_pointer_blocker.dart',
      ),
  ...
  test('compound field composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/forms/input/internal'),
      ).where((file) => file.path.contains('klp_compound_field_')),
      ..._dartFiles(
        Directory('lib/src/features/forms/input/primitives'),
      ).where((file) => file.path.contains('klp_compound_field_')),
    ];
    final violations = <String>[];
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Compound field 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('structured form composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/forms/structured'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('password field composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/forms/input'),
      ).where((file) => _relativePath(file).contains('klp_password_')),
    ];
    final violations = <String>[];
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Password field 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('password field files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Password field 每檔只能有一份結構定義：\n${violations.join('\n')}',
    );
  });

  test('code data composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/collections/code/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/code'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('code data public widgets accept typed viewport geometry', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/code/internal'),
    )) {
      final path = _relativePath(file);
  ...
  test('advanced data composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/collections/advanced/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/advanced'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('docking composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/workspace/shell/docking/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
      r'\b(?:Align|Center|ClipRRect|Column|ConstrainedBox|Container|CustomPaint|DecoratedBox|Dialog|Expanded|Flexible|Focus|GestureDetector|Icon|IconButton|InkWell|LayoutBuilder|ListView|Material|MouseRegion|Padding|Positioned|PositionedDirectional|RotatedBox|Row|Semantics|SizedBox|Spacer|Stack|StatefulBuilder|Text|TextField|TextFormField|Tooltip|Transform|Wrap)\s*(?:\.|<|\()',
    );
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/workspace/shell/docking'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('explorer composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/navigation/widgets/explorer'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('navigator composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/navigation/widgets/navigator'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('stepper composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/collections/stepper/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/stepper'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('calendar composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/forms/selection/internal'),
      ).where((file) => file.path.contains('klp_calendar_')),
      ..._dartFiles(
        Directory('lib/src/features/forms/selection/models'),
      ).where((file) => file.path.contains('klp_calendar_')),
      ..._dartFiles(
        Directory('lib/src/features/forms/selection/primitives'),
      ).where((file) => file.path.contains('klp_calendar_')),
    ];
    final violations = <String>[];
  ...
  test('select field composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/forms/selection/internal'),
      ).where((file) => file.path.contains('klp_select_field_')),
      ..._dartFiles(
        Directory('lib/src/features/forms/selection/primitives'),
      ).where((file) => file.path.contains('klp_select_field_')),
    ];
  ...
  test('key value composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/collections/key_value/internal'),
      ).where((file) => file.path.contains('klp_key_value_')),
      ..._dartFiles(
        Directory('lib/src/features/collections/key_value/models'),
      ).where((file) => file.path.contains('klp_key_value_')),
      ..._dartFiles(
        Directory('lib/src/features/collections/key_value/primitives'),
      ).where((file) => file.path.contains('klp_key_value_')),
    ];
    final violations = <String>[];
  ...
  test('key value public style inputs use typed interfaces', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Key value 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('accordion composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      ..._dartFiles(
        Directory('lib/src/features/collections/accordion/internal'),
      ).where((file) => file.path.contains('klp_accordion_')),
      ..._dartFiles(
        Directory('lib/src/features/collections/accordion/models'),
      ).where((file) => file.path.contains('klp_accordion_')),
      ..._dartFiles(
        Directory('lib/src/features/collections/accordion/primitives'),
      ).where((file) => file.path.contains('klp_accordion_')),
    ];
    final violations = <String>[];
  ...
  test('accordion public style inputs use typed interfaces', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Accordion 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('artifact workspace composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      File('lib/src/features/workspace/artifact/klp_artifact_workspace.dart'),
      ..._dartFiles(Directory('lib/src/features/workspace/artifact/internal')),
      ..._dartFiles(Directory('lib/src/features/workspace/artifact/models')),
      ..._dartFiles(
  ...
  test('artifact workspace files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Artifact workspace 每檔只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('canvas workspace composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Canvas workspace 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('canvas workspace files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Canvas workspace 每檔只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('page chrome composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Page chrome 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('page chrome files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Page chrome 每檔只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('entity picker composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Entity picker 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('entity picker files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Entity picker 每檔只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('card composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/collections/card/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/card'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('timeline composition dogfoods Kallopis primitives', () {
    final primitivePath = RegExp(
      r'lib/src/features/collections/timeline/primitives/.+\.dart$',
    );
    final nativeUi = RegExp(
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/timeline'),
    )) {
      final path = _relativePath(file);
      if (primitivePath.hasMatch(path)) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('message composer composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Message composer 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('filter composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/foundation/interaction/filter'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('badge composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/badge'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('progress composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/progress'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('list tile composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/list_tile'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('message thread composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/message_thread'),
    )) {
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
        violations.add(
          '${_relativePath(file)}:${match.start} ${match.group(0)}',
  ...
  test('message thread files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 MessageThread Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('preview card composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/preview_card'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('preview card files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 PreviewCard Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('sort control composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/sort_control'),
    )) {
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
        violations.add(
          '${_relativePath(file)}:${match.start} ${match.group(0)}',
  ...
  test('sort control files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 SortControl Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('date grid composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/collections/date_grid'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('date grid public style inputs use typed interfaces', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'DateGrid 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('date grid files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 DateGrid Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('primary sidebar composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      File(
        'lib/src/features/workspace/shell/sidebar/klp_primary_sidebar_frame.dart',
      ),
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Primary Sidebar 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('primary sidebar files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/sidebar/klp_primary_sidebar_header_inset.dart',
      ),
      File('lib/src/features/workspace/shell/sidebar/klp_sidebar_frame.dart'),
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Primary Sidebar Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('sidebar identity composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Sidebar identity 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('sidebar identity files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Sidebar identity Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('sidebar navigation button files contain one definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/navigation/widgets/sidebar/klp_navigation_icon_box_key.dart',
      ),
  ...
  test('navigation controls composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/navigation/widgets/controls'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('preview tree composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/navigation/widgets/preview_tree'),
    )) {
      final path = _relativePath(file);
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
        violations.add('$path:${match.start} ${match.group(0)}');
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Preview tree 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('preview tree files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Preview tree Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('pane composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/workspace/shell/composition/pane'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
  ...
  test('pane public style inputs use typed interfaces', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Pane 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('reference picker composition dogfoods Kallopis primitives', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Reference picker 組合層只能使用 Kallopis 原語：\n${violations.join('\n')}',
    );
  });
  ...
  test('reference picker files contain one structural definition', () {
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Reference picker Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('status bar composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/status/internal/klp_status_group.dart',
      ),
    ];
    final violations = <String>[];
  ...
  test('status bar public style inputs use typed interfaces', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Status bar 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('section composition dogfoods Kallopis primitives', () {
    ...
    final file = File(
      'lib/src/foundation/surface/legacy_components/klp_section.dart',
    );
    final violations = <String>[];
  ...
  test('panel frame composition delegates Flutter UI to its primitive', () {
    ...
    final file = File(
      'lib/src/features/workspace/shell/panel/klp_panel_frame.dart',
    );
    final violations = <String>[];
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Panel frame 組合層只能委派給 primitive frame：\n${violations.join('\n')}',
    );
  });
  ...
  test('panel frame files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/panel/primitives/klp_panel_frame_view.dart',
      ),
    ...
    expect(
      violations,
      isEmpty,
      reason: '每個 Panel frame Dart 檔案只能有一份結構定義：\n${violations.join('\n')}',
    );
  });
  ...
  test('panel header public style inputs use typed interfaces', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/panel/klp_panel_header_drag_region_builder.dart',
      ),
    ];
    final violations = <String>[];
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Panel header 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('panel header files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/panel/primitives/klp_panel_header_frame.dart',
      ),
  ...
  test('tabs files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File('lib/src/features/navigation/widgets/tabs/internal/klp_tab.dart'),
      File(
        'lib/src/features/navigation/widgets/tabs/primitives/klp_tab_frame.dart',
      ),
      File(
        'lib/src/features/navigation/widgets/tabs/primitives/klp_tabs_viewport.dart',
      ),
    ];
    final violations = <String>[];
  ...
  test('stage frame composition dogfoods Kallopis primitives', () {
    ...
    final file = File(
      'lib/src/features/workspace/shell/stage/klp_stage_frame.dart',
    );
    final violations = <String>[];
  ...
  test('stage frame public style inputs use typed interfaces', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Stage frame 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('stage frame files contain one structural definition', () {
    ...
    final files = <File>[
      ...
      File(
        'lib/src/features/workspace/shell/stage/primitives/klp_stage_header_slot.dart',
      ),
      File(
        'lib/src/features/workspace/shell/stage/primitives/klp_stage_status_slot.dart',
      ),
      File(
        'lib/src/features/workspace/shell/stage/primitives/klp_stage_surface.dart',
      ),
    ];
    final violations = <String>[];
  ...
  test('tag input files contain one structural definition', () {
    ...
    final files = <File>[
      ..._dartFiles(
        Directory('lib/src/features/forms/selection'),
      ).where((file) => _relativePath(file).contains('klp_tag_')),
  ...
  test('theme preview composition dogfoods Kallopis primitives', () {
    ...
    final files = <File>[
      File(
        'lib/src/features/workspace/shell/theme/klp_theme_preview_mode.dart',
      ),
      File(
        'lib/src/features/workspace/shell/theme/klp_theme_preview_tile.dart',
      ),
  ...
  test('theme preview public style inputs use typed interfaces', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Theme preview 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('theme preview files contain one structural definition', () {
    ...
    final files = <File>[
      ..._dartFiles(
        Directory('lib/src/features/workspace/shell/theme'),
      ).where((file) => _relativePath(file).contains('klp_theme_preview_')),
      File('lib/src/features/workspace/settings/klp_theme_mode_picker.dart'),
  ...
  test('split layout composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(Directory('lib/src/foundation/layout'))) {
      final path = _relativePath(file);
      if (!path.contains('/klp_split_') || path.contains('/primitives/')) {
        continue;
      }
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
        violations.add('$path:${match.start} ${match.group(0)}');
  ...
  test('split layout public style inputs use typed interfaces', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          'Split layout 公開樣式輸入必須使用 typed interface：\n${violations.join('\n')}',
    );
  });
  ...
  test('split layout files contain one structural definition', () {
    ...
    final files = <File>[
      ..._dartFiles(
        Directory('lib/src/foundation/layout'),
  ...
  test('editor action bars composition dogfoods Kallopis primitives', () {
    ...
    for (final file in _dartFiles(
      Directory('lib/src/features/actions/editor'),
    )) {
      final path = _relativePath(file);
      if (path.contains('/primitives/')) continue;
      for (final match in nativeUi.allMatches(file.readAsStringSync())) {
