...
void main() {
      ...
      final machine = klpNavigationTestMachine(
        ...
        commit: (snapshot) {
          if (snapshot.revision == 2) {
            throw StateError('unknown removal status');
          }
        },
      );
  ...
  test(
    ...
    () async {
      ...
      final machine = klpNavigationTestMachine(
        policies: [
          KlpRoutePolicy(destination: home),
          KlpRoutePolicy(destination: detail, beforeLeave: (_) => gate.future),
        ],
        initial: home.location(0),
        commit: (_) {},
      );
  ...
  test(
    ...
    () async {
      ...
      final machine = klpNavigationTestMachine(
        policies: [
          KlpRoutePolicy(destination: home, beforeLeave: (_) => leave.future),
          KlpRoutePolicy(
            destination: detail,
            beforeEnter: (_) {
              enterCalls++;
