...
List<String> publicLibraryViolations(
  File entry,
  Directory sources, {
  Iterable<File> isolatedEntries = const [],
}) {
  final reached = <Uri>{};
  ...
  CompilationUnit parse(File file) {
    ...
    if (result.errors.isNotEmpty) {
      issues.add('Invalid Dart source: ${file.path}');
    }
    return result.unit;
  }
  ...
  void visit(File file) {
    ...
    for (final directive in source.directives.whereType<ExportDirective>()) {
      if (directive.combinators.isNotEmpty) {
        issues.add(
          'Restricted export cannot prove full source coverage: ${file.path}',
        );
      }
      final references = [
        directive.uri,
    ...
    for (final directive in source.directives.whereType<PartDirective>()) {
      ...
      final declarations = parse(
        unit,
      ).directives.whereType<PartOfDirective>().toList();
      final libraries = source.directives
          .whereType<LibraryDirective>()
          .toList();
      final declaration = declarations.length == 1 ? declarations.single : null;
      final uriMatches =
          declaration?.uri != null &&
          resolve(target, declaration!.uri!.stringValue) == uri;
      final nameMatches =
          declaration?.libraryName != null &&
          libraries.length == 1 &&
          declaration!.libraryName!.toSource() ==
  ...
  for (final root in [entry, ...isolatedEntries]) {
    ...
    for (final overlap in reached.intersection(allReached)) {
      issues.add(
        'Shared source across isolated entries: ${File.fromUri(overlap).path}',
      );
    }
  ...
  for (final file in sources.listSync(recursive: true).whereType<File>()) {
    ...
    if (!allReached.contains(file.absolute.uri.normalizePath())) {
      issues.add('Not exported: $path');
    }
  }
  return issues;
}
