...
final klpCompositeLeading = KlpSlot<KlpRailItem>(
  owner: 'fixture',
  name: 'leading',
);
final klpCompositeTrailing = KlpSlot<KlpRailItem>(
  owner: 'fixture',
  name: 'trailing',
);

/// 由同一份模板推導插槽，不讓消費端重複列出安裝清單。
KlpComponentDefinition<KlpCompositeTestNode> klpCompositeTestDefinition({
  String Function(KlpCompositeTestNode)? select,
}) {
  ...
  return KlpComponentDefinition(
    ...
    content: KlpLinearTemplate(
      axis: KlpAxis.vertical,
      gap: row.gap,
      children: [
        KlpTextTemplate<KlpCompositeTestNode>(
          text: select ?? (node) => node.label,
          semantics: text.semantics,
        ),
        KlpChildrenTemplate<KlpCompositeTestNode, KlpRailItem>(
        ...
        KlpSurfaceTemplate<KlpCompositeTestNode>(
          ...
          inset: surface.inset,
          child: KlpChildrenTemplate<KlpCompositeTestNode, KlpRailItem>(
            slot: klpCompositeTrailing,
            axis: KlpAxis.vertical,
            gap: row.gap,
          ),
        ),
...
KlpChildren klpCompositeChildren({
  List<KlpRailItem> leading = const [],
  List<KlpRailItem> trailing = const [],
}) => KlpChildren([
  klpCompositeLeading.assign(leading),
  klpCompositeTrailing.assign(trailing),
]);
