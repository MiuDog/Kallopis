...
void main() {
	test(
		'one transaction retains independent scoped resources and removes one entry',
		() {
			final runtime = KlpTreeRuntime();
			addTearDown(runtime.dispose);
			final adapter = KlpRuntimeFixture();
			KlpScopeBoundary entry(String id) =>
					KlpScopeBoundary(id: KlpId.parse(id), child: KlpRuntimeNode('same'));
			void update(List<String> entries) => runtime.update(
				root: KlpRuntimeNode('root', entries.map(entry).toList()),
				adapters: [adapter, const KlpScopeBoundaryAdapter()],
	...
	test(
		'invalid scoped descendant preserves every previously committed entry',
		() {
			final runtime = KlpTreeRuntime();
			addTearDown(runtime.dispose);
			final adapter = KlpRuntimeFixture();
			void update(KlpNode child) => runtime.update(
				root: KlpRuntimeNode('root', [
					KlpScopeBoundary(id: KlpId.parse('entry'), child: child),
				]),
				adapters: [adapter, const KlpScopeBoundaryAdapter()],
				primitives: klpTestPrimitives(),
			);
			...
			final resource =
					runtime.resources[KlpPlacementId(scope: ['entry'], localId: 'same')];
			adapter.failPrepare = 'broken';
			expect(
				() => update(KlpRuntimeNode('same', [KlpRuntimeNode('broken')])),
				throwsStateError,
			);
