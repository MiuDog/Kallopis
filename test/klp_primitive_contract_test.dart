...
void main() {
	group('KlpPrimitiveSet', () {
		...
		for (final (kind, values, field) in cases) {
			...
			test('$field rejects missing and excess slots', () {
				for (final length in [7, 9]) {
					...
					expect(() => _withSlots(source, field, changed), throwsA(isA<KlpContractError>().having((error) => error.message, 'path', contains('primitives.$field'))));
				}
			});
		}
		...
		test('validates the captured slots when source length changes on read', () {
			...
			final rejectedSource = anyOf(isA<KlpContractError>(), isA<ConcurrentModificationError>(), isA<RangeError>());
		...
		test('alternate fixture varies every physical quantity', () {
			...
			expect(alternate.strokeWidths[1].value, isNot(source.strokeWidths[1].value));
			expect(alternate.fontSizes[1].value, isNot(source.fontSizes[1].value));
			expect(alternate.fontWeights[1].value, isNot(source.fontWeights[1].value));
			expect(alternate.lineHeights[1].value, isNot(source.lineHeights[1].value));
			expect(alternate.letterSpacings[1].value, isNot(source.letterSpacings[1].value));
			expect(alternate.durations[1].milliseconds, isNot(source.durations[1].milliseconds));
			expect(alternate.fontFamilies[1].family, isNot(source.fontFamilies[1].family));
	...
	group('KlpStyleValue', () {
		test('rejects non finite physical values', () {
			for (final value in [double.nan, double.infinity, double.negativeInfinity]) {
				final constructors = <KlpStyleValue Function()>[
					() => KlpDistance(value), () => KlpRadius(value), () => KlpStrokeWidth(value),
					() => KlpFontSize(value), () => KlpLineHeight(value), () => KlpLetterSpacing(value),
					() => KlpCurve(value, 0, 1, 1), () => KlpCurve(0, value, 1, 1),
					() => KlpCurve(0, 0, value, 1), () => KlpCurve(0, 0, 1, value),
				];
		...
		test('rejects domain violations including every color channel', () {
			final invalid = <KlpStyleValue Function()>[
				() => KlpDistance(-1), () => KlpRadius(-1), () => KlpStrokeWidth(-1),
				() => KlpFontSize(0), () => KlpFontSize(-1), () => KlpLineHeight(0), () => KlpLineHeight(-1),
				() => KlpFontWeight(0), () => KlpFontWeight(1001), () => KlpDuration(-1),
				() => KlpCurve(-0.1, 0, 1, 1), () => KlpCurve(0, 0, 1.1, 1),
				() => KlpColor(-1, 0, 0), () => KlpColor(0, -1, 0), () => KlpColor(0, 0, -1),
				() => KlpColor(256, 0, 0), () => KlpColor(0, 256, 0), () => KlpColor(0, 0, 256),
				() => KlpColor(0, 0, 0, alpha: -1), () => KlpColor(0, 0, 0, alpha: 256),
			];
		...
		test('validates and snapshots font names and fallback order', () {
			for (final name in ['', ' ', ' Font', 'Font ']) {
				...
				expect(() => KlpFontFamily('Font', fallback: [name]), throwsA(isA<KlpContractError>().having((error) => error.message, 'path', contains('fallback[0]'))));
			}
...
KlpPrimitiveSet _withSlots(KlpPrimitiveSet source, String field, List<KlpStyleValue> values) => KlpPrimitiveSet(
	colors: field == 'colors' ? values.cast<KlpColor>() : source.colors,
	distances: field == 'distances' ? values.cast<KlpDistance>() : source.distances,
	radii: field == 'radii' ? values.cast<KlpRadius>() : source.radii,
	strokeWidths: field == 'strokeWidths' ? values.cast<KlpStrokeWidth>() : source.strokeWidths,
	fontSizes: field == 'fontSizes' ? values.cast<KlpFontSize>() : source.fontSizes,
	fontWeights: field == 'fontWeights' ? values.cast<KlpFontWeight>() : source.fontWeights,
	lineHeights: field == 'lineHeights' ? values.cast<KlpLineHeight>() : source.lineHeights,
	letterSpacings: field == 'letterSpacings' ? values.cast<KlpLetterSpacing>() : source.letterSpacings,
	durations: field == 'durations' ? values.cast<KlpDuration>() : source.durations,
	fontFamilies: field == 'fontFamilies' ? values.cast<KlpFontFamily>() : source.fontFamilies,
	curves: field == 'curves' ? values.cast<KlpCurve>() : source.curves,
);

KlpPrimitiveSet _withColors(KlpPrimitiveSet source, List<KlpColor> colors) => KlpPrimitiveSet(
	colors: colors,
