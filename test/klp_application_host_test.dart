...
void main() {
  testWidgets(
    'public bootstrap mounts data and style replacement retains focus and selection',
    (tester) async {
      var calls = 0;
      ...
      final choice = tester.widget<KlpFlutterChoice>(
        find.byType(KlpFlutterChoice),
      );
      ...
      await tester.pump();
      final updated = tester.widget<KlpFlutterChoice>(
        find.byType(KlpFlutterChoice),
      );
      expect(updated.content.selection, same(selection));
      expect(
        updated.content.selection.value,
        KlpPlacementId(scope: ['fixture.router', 'route-1'], localId: 'a'),
      );
      expect(FocusManager.instance.primaryFocus, same(focus));
      expect(
        updated.content.style.selectedBackground,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        tester.widget<KlpFlutterChoice>(find.byType(KlpFlutterChoice)).content,
        same(old),
      );
      await tester.tap(find.byType(KlpFlutterChoice));
      await tester.pump();
  ...
  testWidgets(
    'removal clears selection and stale actions cannot reach removed callbacks',
    (tester) async {
      var calls = 0;
      final item = KlpComponentTestItem(
        id: 'a',
    ...
    (tester) async {
      ...
      expect(
        MediaQuery.disableAnimationsOf(
          tester.element(find.byType(KlpFlutterChoice)),
        ),
        isTrue,
      );
      expect(
