...
void main() {
  testWidgets('avatar size 由 typed token 尺寸解析', (tester) async {
      ...
      const KlpApp(
        ...
        home: KlpPanelFrame(
          content: KlpColumn(
            ...
            children: [
              KlpAvatar(key: ValueKey('standard-avatar'), label: 'S'),
              KlpAvatar(
    ...
    expect(tester.getSize(standard), Size.square(tokens.controlHeightLarge));
    expect(tester.getSize(small), Size.square(tokens.avatarSmall));
    expect(tester.widget<KlpAvatar>(small).tone, KlpAvatarTone.emphasized);
  });
