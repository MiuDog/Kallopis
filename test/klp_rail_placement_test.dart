...
void main() {
  test('selection belongs to placement and removal clears it', () async {
    final resource = KlpRailPlacement(
      KlpValidatedNode('rail', 'kallopis.rail', ['a', 'b']),
    );
    addTearDown(resource.dispose);
    final events = <KlpPlacementId?>[];
    resource.selection.subscribe(events.add);
    await resource.activate(
      KlpPlacementId(localId: 'a'),
      const KlpCallbackAction(_noop),
      null,
      null,
    );
  ...
  test('callback failure does not create a local selection', () async {
    final resource = KlpRailPlacement(
      KlpValidatedNode('rail', 'kallopis.rail', ['a']),
    );
    addTearDown(resource.dispose);
    final events = <String>[];
    ...
    await expectLater(
      resource.activate(
        ...
        null,
        (_) => events.add('selected'),
      ),
      throwsA(same(pressedError)),
    );
  ...
  test('removed and disposed placements cannot dispatch callbacks', () async {
    ...
    var calls = 0;
    await resource.activate(
      KlpPlacementId(localId: 'missing'),
      KlpCallbackAction(() => calls++),
      null,
      (_) => calls++,
    );
    resource.dispose();
  ...
  test('removal commits metadata before failing notification', () async {
    ...
    expect(
      () => resource.update(KlpValidatedNode('rail', 'kallopis.rail', [])),
      throwsA(same(error)),
    );
    var calls = 0;
    await resource.activate(
  ...
  test(
    'equal local items in different scopes never share membership or selection',
    () async {
      final firstItem = KlpPlacementId(scope: ['entry-one'], localId: 'item');
      final secondItem = KlpPlacementId(scope: ['entry-two'], localId: 'item');
      final first = KlpRailPlacement(
        KlpValidatedNode.scoped(
          KlpPlacementId(scope: ['entry-one'], localId: 'rail'),
          'kallopis.rail',
          [firstItem],
        ),
      );
      final second = KlpRailPlacement(
        KlpValidatedNode.scoped(
          KlpPlacementId(scope: ['entry-two'], localId: 'rail'),
          'kallopis.rail',
          [secondItem],
        ),
      );
