...
void main() {
	...
	test('primitive library owns scale and palette definitions through single-definition parts', () {
		final source = File('lib/src/styling/legacy_tokens/primitive_token.dart').readAsStringSync();
		final palette = File('lib/src/styling/legacy_tokens/internal/klp_palette.dart').readAsStringSync();
		expect(RegExp(r'abstract final class KlpScale\s*\{').allMatches(source), hasLength(1));
		expect(source, contains("part 'internal/klp_palette.dart';"));
		expect(palette, contains("part of '../primitive_token.dart';"));
		expect(RegExp(r'abstract final class KlpPalette\s*\{').allMatches(palette), hasLength(1));
		expect(source, isNot(contains('export ')));
		expect(source, contains("part 'internal/klp_accent.dart';"));
		final files = Directory('lib/src/styling/legacy_tokens').listSync().whereType<File>().where((file) => file.path.endsWith('.dart')).map((file) => file.uri.pathSegments.last).toSet();
		expect(files, {'primitive_token.dart'});
	});

	test('accent shares the primitive library and decoration stays separate', () {
		final accent = File('lib/src/styling/legacy_tokens/internal/klp_accent.dart').readAsStringSync();
		final foundation = File('lib/src/foundation/klp_palette.dart').readAsStringSync();
		...
		expect(File('lib/src/foundation/internal/klp_accent.dart').existsSync(), isFalse);
		...
		expect(public_api.KlpDecorativePalette.previewWallpaper, same(decorative.KlpDecorativePalette.previewWallpaper));
	});
}
