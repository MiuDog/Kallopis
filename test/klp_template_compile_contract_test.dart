...
String _textSemantics([Map<String, String> overrides = const {}]) =>
    'KlpTextSemantics(${_rawTextValues.keys.map((field) => '$field: ${overrides[field] ?? '${field}Key'}').join(', ')})';
String _linear(String children, String gap) =>
    'KlpLinearTemplate<ExternalItem>(axis: KlpAxis.vertical, children: $children, gap: $gap)';
String _surface(Map<String, String> overrides) =>
    'KlpSurfaceTemplate<ExternalItem>(child: ${overrides['child'] ?? 'linear'}, background: ${overrides['background'] ?? 'colorKey'}, radius: ${overrides['radius'] ?? 'radiusKey'}, inset: ${overrides['inset'] ?? 'distanceKey'})';
...
''';
  ...
  for (final type in [
    ...
    for (final relation in ['extends', 'implements']) {
      ...
      if (type != 'KlpTemplate') {
        code = relation == 'extends'
            ? 'final_class_extended_outside_of_library'
            : 'final_class_implemented_outside_of_library';
      }
      cases.add((
        name: '$relation $type',
        source: 'abstract final class Escape $relation $type<ExternalItem> {}',
        code: code,
      ));
    }
  }
  for (final value in ['const SizedBox()', 'text']) {
    cases.add((
      name: 'selector cannot return $value',
      source:
          'final invalid = KlpTextTemplate<ExternalItem>(text: (item) => $value, semantics: textSemantics);',
      code: 'return_of_invalid_type_from_closure',
    ));
  }
  cases.add((
    name: 'selector cannot receive context',
    source:
        'final invalid = KlpTextTemplate<ExternalItem>(text: contextSelector, semantics: textSemantics);',
    code: 'argument_type_not_assignable',
  ));
  cases.add((
    name: 'template has no widget builder',
    source:
        'final invalid = KlpTextTemplate<ExternalItem>(text: (item) => item.label, semantics: textSemantics, builder: (BuildContext context) => const SizedBox());',
    code: 'undefined_named_parameter',
  ));
  cases.add((
    name: 'linear children reject widget',
    source: 'final invalid = ${_linear('[const SizedBox()]', 'distanceKey')};',
    code: 'list_element_type_not_assignable',
  ));
  cases.add((
    name: 'surface child rejects widget',
    source: 'final invalid = ${_surface({'child': 'const SizedBox()'})};',
    code: 'argument_type_not_assignable',
  ));
  cases.add((
    name: 'component content rejects widget',
    source:
        "final invalid = KlpComponentDefinition<ExternalItem>('external', content: const SizedBox(), semantics: schema);",
    code: 'argument_type_not_assignable',
  ));
  cases.add((
    name: 'accessibility selector rejects widget',
    source:
        "final invalid = KlpComponentDefinition<ExternalItem>('external', content: surface, semantics: schema, accessibilityLabel: (item) => const SizedBox());",
    code: 'return_of_invalid_type_from_closure',
  ));
  cases.add((
    name: 'accessibility selector rejects context',
    source:
        "final invalid = KlpComponentDefinition<ExternalItem>('external', content: surface, semantics: schema, accessibilityLabel: contextSelector);",
    code: 'argument_type_not_assignable',
  ));
  cases.add((
    name: 'component has no accessibility builder',
    source:
        "final invalid = KlpComponentDefinition<ExternalItem>('external', content: surface, semantics: schema, accessibilityBuilder: (BuildContext context) => const SizedBox());",
    code: 'undefined_named_parameter',
  ));
  ...
  for (final entry in _rawTextValues.entries) {
    for (final value in [entry.value, 'distanceKey']) {
      cases.add((
        name: 'text ${entry.key} rejects $value',
        source: 'final invalid = ${_textSemantics({entry.key: value})};',
        code: 'argument_type_not_assignable',
      ));
    }
  }
  cases.add((
    name: 'text rejects raw Flutter color',
    source:
        "final invalid = ${_textSemantics({'color': 'const Color(0xff000000)'})};",
    code: 'argument_type_not_assignable',
  ));
  cases.add((
    name: 'text rejects raw font size',
    source: "final invalid = ${_textSemantics({'fontSize': '12.0'})};",
    code: 'argument_type_not_assignable',
  ));
  for (final value in ['1.0', 'KlpDistance(1)', 'colorKey']) {
    cases.add((
      name: 'linear gap rejects $value',
      source: 'final invalid = ${_linear('[text]', value)};',
      code: 'argument_type_not_assignable',
    ));
  }
  final surfaceValues = {
  ...
  for (final entry in surfaceValues.entries) {
    for (final value in entry.value) {
      cases.add((
        name: 'surface ${entry.key} rejects $value',
        source: 'final invalid = ${_surface({entry.key: value})};',
        code: 'argument_type_not_assignable',
      ));
    }
  }
  ...
  setUpAll(() async {
    fixture = await KlpExternalCompileFixture.create({
      'positive':
          "${prefix}final definition = KlpComponentDefinition<ExternalItem>('external', content: surface, semantics: schema, accessibilityLabel: (item) => item.label);\nfinal rail = KlpRail(id: 'rail', top: [ExternalItem('item', 'Label')]);\n",
  ...
  test(
    'external rail item definition composes text linear and surface templates',
    () async {
