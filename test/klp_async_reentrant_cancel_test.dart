...
void main() {
	test('cancel in queued replacement loading notification prevents operation', () async {
	...
	test('replacement loading error belongs to replacement future', () async {
		...
		owner.state.subscribe((state) {
			...
			notifications++;
			if (notifications == 1) {
				replacementCheck = expectLater(owner.load(() async => ++calls), throwsA(same(error)));
			}
			else {
	...
	test('cancel nested publication preserves later listener replacement safety', () async {
		final owner = KlpAsyncData<int>();
		addTearDown(owner.dispose);
		var notifications = 0;
		var calls = 0;
	...
	for (final fails in [false, true]) {
		test('replacement from completed state waits for loading cancellation fails=$fails', () async {
			final owner = KlpAsyncData<int>();
			addTearDown(owner.dispose);
			var loadingCount = 0;
