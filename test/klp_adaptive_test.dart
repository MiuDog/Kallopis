...
void main() {
  testWidgets('adaptive selects the explicit platform and other fallback', (
    tester,
  ) async {
    for (final hasOther in [true, false]) {
      for (final platform in KlpAppPlatform.values) {
        ...
        final adaptive = KlpAdaptive(
          windows: (_) {
            calls.add('windows');
            return const _ProbePanel('windows');
          },
          android: (_) {
            calls.add('android');
            return const _ProbePanel('android');
          },
          other: hasOther
              ? (_) {
                  calls.add('other');
                  return const _ProbePanel('other');
                }
              : null,
        );
        await tester.pumpWidget(
          Directionality(
            ...
            child: KlpEnvironmentScope(
              platform: KlpPlatformInfo(platform: platform),
              child: adaptive,
            ),
          ),
        );
  ...
  testWidgets(
    'adaptive detects the current platform without an environment scope',
    (tester) async {
      await tester.pumpWidget(
        Directionality(
          textDirection: TextDirection.ltr,
  ...
  testWidgets(
    'adaptive accepts arbitrary widget strategies at component depth',
    (tester) async {
      await tester.pumpWidget(
        Directionality(
          textDirection: TextDirection.ltr,
  ...
  testWidgets(
    'adaptive rebuilds for platform changes but not legacy app theme changes',
    (tester) async {
  ...
  testWidgets(
    'local constraints choose layout without changing the adaptive platform',
    (tester) async {
      var windowsBuilds = 0;
      var androidBuilds = 0;
      final adaptive = KlpAdaptive(
        windows: (_) {
          windowsBuilds++;
          return const _ProbePanel('windows', responsive: true);
        },
        android: (_) {
          androidBuilds++;
          return const _ProbePanel('android', responsive: true);
        },
      );
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) =>
          Text('$label ${constraints.maxWidth >= 600 ? 'wide' : 'narrow'}'),
    );
  }
...
class _TestAppController implements KlpAppController {
  ...
  void toggleBrightness() =>
      currentBrightness = currentBrightness == Brightness.light
      ? Brightness.dark
      : Brightness.light;
}
