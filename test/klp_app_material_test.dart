...
Widget _hostileTextStyle(BuildContext context, Widget? child) => DefaultTextStyle(style: _hostileStyle, child: child!);

void main() {
	for (final showHeader in [false, true]) {
		for (final hostile in [false, true]) {
			testWidgets('app supplies Material with header=$showHeader and hostile=$hostile', (tester) async {
				var taps = 0;
				...
				for (final key in ['klp-copy', 'native-copy']) {
					...
					final paragraph = tester.renderObject<RenderParagraph>(find.descendant(of: copy, matching: find.byType(RichText)).first);
					expect(paragraph.text.style?.decoration ?? TextDecoration.none, TextDecoration.none);
					expect(DefaultTextStyle.of(context).style.decoration ?? TextDecoration.none, TextDecoration.none);
				}
