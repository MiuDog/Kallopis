...
void main() {
	/// 舊 ink 色梯；僅用於未修訂角色與 UltraDark 相容配方。
	...
	const offRamp = <String, Color>{
		...
		'green500': KlpPalette.green500,
		'amber500': KlpPalette.amber500,
		'red300': KlpPalette.red300,
		'blue500': KlpPalette.blue500,
	};
	...
	// v1 對每個已採用角色驗證精確色值；未修訂角色與 UltraDark 保留舊色梯契約。
	const v1Roles = <String, Map<String, Color>>{
		'light': {
			'app': Color(0xFFE7E4DD),
			'surface': Color(0xFFEFEDE7),
			'surfaceInset': Color(0xFFEFEDE7),
			'surfaceMuted': Color(0xFFE1DDD2),
			'component': Color(0xFFF8F6F1),
			'stageSurface': Color(0xFFF8F6F1),
			'overlay': Color(0xFFF8F6F1),
			'surfaceRaised': Color(0xFFF8F6F1),
			'guide': Color(0xFFD3CFC4),
			'divider': Color(0xFFD3CFC4),
			'pagePattern': Color(0xFFD3CFC4),
			'text': Color(0xFF21201B),
			'textMuted': Color(0xFF6A655B),
			'textFaint': Color(0xFF6A655B),
			'border': Color(0xFFD3CFC4),
			'borderStrong': Color(0xFFD3CFC4),
		},
		'dark': {
			'app': Color(0xFF201F1C),
			'surface': Color(0xFF292724),
			'surfaceInset': Color(0xFF292724),
			'surfaceMuted': Color(0xFF484339),
			'component': Color(0xFF35322D),
			'stageSurface': Color(0xFF35322D),
			'overlay': Color(0xFF35322D),
			'surfaceRaised': Color(0xFF35322D),
			'guide': Color(0xFF514B40),
			'divider': Color(0xFF514B40),
			'pagePattern': Color(0xFF514B40),
			'text': Color(0xFFF0EDE6),
			'textMuted': Color(0xFFBAB5AA),
			'textFaint': Color(0xFFBAB5AA),
			'border': Color(0xFF514B40),
			'borderStrong': Color(0xFF514B40),
		},
	};

	group('中性欄位遵守 v1 精確角色或保留的 ink 色梯', () {
		...
		presets.forEach((name, tokens) {
			test(name, () {
				...
				fields.forEach((field, color) {
					final exactColor = v1Roles[name]?[field];
					if (exactColor != null) {
						expect(color, exactColor, reason: '$name.$field 必須符合 v1 已確認色值');
						return;
					}
	...
	test('元件不得直接取用具體顏色', () {
		...
		for (final file
			...
			if (path.startsWith('lib/src/styling/legacy_theme/') ||
					...
					path ==
							'lib/src/styling/presets/legacy/default_data_visualization.dart' ||
