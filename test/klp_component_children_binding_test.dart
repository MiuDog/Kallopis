...
void main() {
  test('nested qualified children materialize once in template slot order', () {
    ...
    final grandchild = KlpCompositeTestNode(
      'grandchild',
      klpCompositeChildren(),
    );
    final first = KlpCompositeTestNode(
      'first',
      klpCompositeChildren(trailing: [grandchild]),
    );
    final second = KlpCompositeTestNode('second', klpCompositeChildren());
    final root = KlpCompositeTestNode(
      'root',
      klpCompositeChildren(leading: [first], trailing: [second]),
    );
    ...
    final prepared = {
      for (final snapshot in captured.validation.nodes)
        snapshot.placementId: compiler.prepareCaptured(
          captured.sources[snapshot.placementId]!,
          snapshot,
          primitives,
        ),
    };
    ...
    for (final snapshot in captured.validation.nodes.reversed) {
      bound[snapshot.placementId] = prepared[snapshot.placementId]!.materialize(
        [for (final id in snapshot.childrenPlacements) bound[id]!],
      ).content;
    }
    expect(selected, ['root', 'first', 'grandchild', 'second']);
    expect(_textValues(bound[KlpPlacementId(localId: 'root')]!), [
      'root',
      'first',
      'grandchild',
      'second',
    ...
    expect(
      leading.children.single,
      same(bound[KlpPlacementId(localId: 'first')]),
    );
    final trailing =
        (row.children[2] as KlpBoundSurface).child as KlpBoundLinear;
    expect(
      trailing.children.single,
      same(bound[KlpPlacementId(localId: 'second')]),
    );
  });

  test(
    'definition derives nested slots and rejects owner or duplicate positions',
    () {
      final definition = klpCompositeTestDefinition();
      expect(definition.contract.slots, [
        klpCompositeLeading,
        klpCompositeTrailing,
      ]);
      final row = definition.content as KlpLinearTemplate<KlpCompositeTestNode>;
      final duplicate = KlpLinearTemplate<KlpCompositeTestNode>(
        axis: row.axis,
        gap: row.gap,
        children: [row.children[1], row.children[1]],
      );
      expect(
        () => KlpComponentDefinition(
          'fixture',
          content: duplicate,
          semantics: definition.contract.semantics,
        ),
        _failure('duplicate_slot'),
      );
      final foreign = KlpChildrenTemplate<KlpCompositeTestNode, KlpNode>(
        slot: KlpSlot(owner: 'foreign', name: 'items'),
        axis: row.axis,
        gap: row.gap,
      );
      expect(
        ...
        _failure('slot_owner_mismatch'),
      );
    },
  );

  test(
    'standalone binding explicitly requires child context even for empty slots',
    () {
      ...
      expect(
        ...
        _failure('component_requires_child_context'),
      );
    },
  );
  ...
  test('bad captured ranges fail before any data selector', () {
    ...
    final invalid = [
      <KlpValidatedSlot>[],
      [
        KlpValidatedSlot(klpCompositeLeading, 0, 1),
        KlpValidatedSlot(klpCompositeTrailing, 0, 0),
      ],
      [
        KlpValidatedSlot(klpCompositeLeading, 0, 0),
        KlpValidatedSlot(klpCompositeLeading, 0, 0),
      ],
      [
        KlpValidatedSlot(klpCompositeTrailing, 0, 0),
    ...
    for (final ranges in invalid) {
      final snapshot = KlpValidatedNode(
        node.id,
        node.definitionId,
        [],
        slotRanges: ranges,
      ...
      expect(
        () => compiler.validateCaptured(node, snapshot),
        _failure('component_slot_snapshot_mismatch'),
      );
      expect(
        () => compiler.prepareCaptured(node, snapshot, klpTestPrimitives()),
        _failure('component_slot_snapshot_mismatch'),
      );
    }
    final valid = captureKlpTree(
      KlpRegistry([definition.contract]),
      node,
    ).validation.nodes.single;
  ...
  test('slot semantic references retain ownership validation', () {
    final definition = klpCompositeTestDefinition();
    final content = KlpChildrenTemplate<KlpCompositeTestNode, KlpRailItem>(
      slot: klpCompositeLeading,
      axis: KlpAxis.vertical,
      gap: KlpSemanticKey('fixture', 'missing', KlpStyleKind.distance),
    );
    final invalid = KlpComponentDefinition(
      'fixture',
      content: content,
      semantics: definition.contract.semantics,
    );
  ...
  test('materialization cannot omit or append direct child results', () {
    ...
    final root = KlpCompositeTestNode(
      'root',
      klpCompositeChildren(leading: [child]),
    );
    final captured = captureKlpTree(KlpRegistry([definition.contract]), root);
    final prepared = KlpComponentCompiler([definition]).prepareCaptured(
      root,
      captured.validation.nodes.first,
      klpTestPrimitives(),
    );
    expect(
      () => prepared.materialize([]),
      _failure('component_child_count_mismatch'),
    );
  });
}
