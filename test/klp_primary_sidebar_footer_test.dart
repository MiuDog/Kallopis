...
void main() {
	testWidgets('primary sidebar footer adds no vertical padding', (tester) async {
		...
		final frameRect = tester.getRect(find.byKey(const ValueKey('sidebar-frame')));
		final explorerRect = tester.getRect(find.byKey(const ValueKey('sidebar-explorer')));
	...
	testWidgets('primary sidebar resolves typed content and header spacing', (tester) async {
		await tester.pumpWidget(
			const KlpApp(
				showWindowHeader: false,
				home: KlpPanelFrame(
					content: KlpAppScreen(
						child: SizedBox(
							...
							child: KlpPrimarySidebarFrame(
								...
								header: SizedBox(key: ValueKey('typed-sidebar-header'), height: 20),
								navigation: SizedBox(key: ValueKey('typed-sidebar-navigation'), height: 24),
		...
		expect(headerRect.left, frameRect.left + theme.space.dockMargin + theme.space.contentInset);
