# KlpDashedBorder：元件樹架構

## 範圍

- **核心元件**：`KlpDashedBorder`
- **所屬領域**：`foundation — 圖示、色盤、度量`
- **核心職責**：虛線邊框容器。為子元件提供自訂粗細、圓角、顏色與虛線間距的虛線外框。  預設使用 theme 的 [KlpShapeTheme.dashedOpacity] 輔助線顏色與 [KlpShapeTheme.control] 圓角。
...
- [`KlpStrokeFrame`](./klp_stroke_frame.md) — `foundation — 圖示、色盤、度量` *(純容器，已繼續向下展開)*

## 程式碼證據

- 檔案路徑：[`lib/src/foundation/surface/klp_dashed_border.dart`](../../../../lib/src/foundation/surface/klp_dashed_border.dart#L9)
