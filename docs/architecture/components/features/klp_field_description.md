# KlpFieldDescription：元件樹架構

## 範圍

- **核心元件**：`KlpFieldDescription`
- **所屬領域**：`features`
- **核心職責**：欄位輔助說明文字，統一使用低對比（[KlpTextTone.muted]）的 [KlpTextRole.caption] 樣式。  [KlpField] 內部就是用它畫 `description`——需要在 [KlpField] 版面之外 單獨放一段樣式一致的欄位說明時才需要直接用它。
...
- 檔案路徑：[`lib/src/features/forms/core/klp_field_description.dart`](../../../../lib/src/features/forms/core/klp_field_description.dart#L8)
