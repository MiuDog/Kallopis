# KlpAvatarGroup：元件樹架構

## 範圍

- **核心元件**：`KlpAvatarGroup`
- **所屬領域**：`features`
- **核心職責**：以緊密水平排列呈現多個 Avatar，超出上限時顯示剩餘數量。
...
flowchart TD
  ...
  root["KlpAvatarGroup"]:::root
  n1["KlpRow"]:::reference
  root --> n1
  n2["KlpAvatar"]:::reference
  root --> n2
```

## 外部元件引用

- [`KlpAvatar`](./klp_avatar.md) — `features`
- [`KlpRow`](../foundation/klp_row.md) — `foundation — 圖示、色盤、度量`

## 程式碼證據

- 檔案路徑：[`lib/src/features/collections/avatar/klp_avatar_group.dart`](../../../../lib/src/features/collections/avatar/klp_avatar_group.dart#L9)
