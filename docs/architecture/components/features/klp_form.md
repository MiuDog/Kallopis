...
- **核心元件**：`KlpForm`
- **所屬領域**：`features`
- **核心職責**：整份表單的最外層版面：錯誤總覽、各個區塊（[sections]）與底部動作列 依序排列，各區塊之間插入固定間距。  不管理欄位資料或驗證邏輯——[sections] 由呼叫端組好（通常是多個 [KlpFormSection]），[errorSummary] 通常放 [KlpFormErrorSummary]， [actions] 通常放 [KlpFormActions]。三者皆為可選，缺席時不佔版位。
...
- 檔案路徑：[`lib/src/features/forms/core/klp_form.dart`](../../../../lib/src/features/forms/core/klp_form.dart#L9)
