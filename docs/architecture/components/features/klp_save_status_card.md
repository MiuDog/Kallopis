# KlpSaveStatusCard：元件樹架構

## 範圍

- **核心元件**：`KlpSaveStatusCard`
- **所屬領域**：`features`
- **核心職責**：顯示最後儲存時間與一組相關狀態訊息的卡片，用於編輯器頁面告知使用者 目前的儲存／同步狀況。  [savedAt] 是已經格式化好的顯示文字（例如「2 分鐘前」），這個元件不處理 時間格式化或相對時間更新。
...
- 檔案路徑：[`lib/src/features/workspace/page_chrome/internal/klp_save_status_card.dart`](../../../../lib/src/features/workspace/page_chrome/internal/klp_save_status_card.dart#L8)
