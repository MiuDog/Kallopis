# KlpConditionalFieldRegion：元件樹架構

## 範圍

- **核心元件**：`KlpConditionalFieldRegion`
- **所屬領域**：`features`
- **核心職責**：依條件顯示／隱藏一段欄位，並用 Kallopis 尺寸動畫補間高度變化，避免表單 其他欄位因為突然增減內容而跳動。  [visible] 為 false 時 [child] 會被整個換成零尺寸盒，因此 child 的 widget 狀態不會保留——若 child 內有輸入控制項且需要在重新顯示時 保住使用者輸入，請自行在 child 上加 [GlobalKey] 或改用其他方式保存資料。
...
flowchart TD
  ...
  root["KlpConditionalFieldRegion"]:::root
  n1["child / slot"]:::slot
  root --> n1
```
...
- 檔案路徑：[`lib/src/features/forms/core/klp_conditional_field_region.dart`](../../../../lib/src/features/forms/core/klp_conditional_field_region.dart#L11)
