# KlpSettingsNavigationItem：元件樹架構

## 範圍

- **核心元件**：`KlpSettingsNavigationItem`
- **所屬領域**：`features`
- **核心職責**：設定 section 導覽列；只有選取項目會建立其 field deep links。
...
flowchart TD
  ...
  root["KlpSettingsNavigationItem"]:::root
  n1["KlpColumn"]:::reference
  root --> n1
  n2["KlpListTile"]:::reference
  root --> n2
...
- [`KlpListTile`](./klp_list_tile.md) — `features`
- [`KlpSurface`](../foundation/klp_surface.md) — `foundation — 圖示、色盤、度量` *(純容器，已繼續向下展開)*

## 程式碼證據

- 檔案路徑：[`lib/src/features/workspace/settings/navigation/klp_settings_navigation_item.dart`](../../../../lib/src/features/workspace/settings/navigation/klp_settings_navigation_item.dart#L4)
