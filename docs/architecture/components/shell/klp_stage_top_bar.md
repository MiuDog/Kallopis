...
- **核心元件**：`KlpStageTopBar`
- **所屬領域**：`shell — 應用外殼`
- **核心職責**：位於 Workbench window header 中央 Stage 區域的檔案分頁與動作列。  此列不屬於 Stage body。產品只注入檔案分頁與動作，不自行決定對齊。
...
- 檔案路徑：[`lib/src/shell/stage/klp_stage_top_bar.dart`](../../../../lib/src/shell/stage/klp_stage_top_bar.dart#L11)
