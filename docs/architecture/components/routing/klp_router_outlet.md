# KlpRouterOutlet：元件樹架構

## 範圍

- **核心元件**：`KlpRouterOutlet`
- **所屬領域**：`routing — 分發`
- **核心職責**：渲染目前的目的地。  它只做一件事：呼叫 `router.current.builder`。**不做轉場動畫**——轉場屬於產品外殼 的決定（有些頁該滑入，有些該直接換），庫替它決定就等於替所有產品決定。
...
- 檔案路徑：[`lib/src/routing/klp_router.dart`](../../../../lib/src/routing/klp_router.dart#L181)
