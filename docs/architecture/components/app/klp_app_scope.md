...
- **核心職責**：供 [KlpApp.of] 查找的 `InheritedWidget`。  [brightness] 與 [themeMode] 是資料欄位而非只有 [controller] 一個引用， 這樣 [updateShouldNotify] 才能在它們改變時真正回傳 `true`，讓依賴它的 子樹重建——只放 controller 引用的話，同一個物件永遠 `==` 自己，不會觸發重建。
...
- 檔案路徑：[`lib/src/app/klp_app.dart`](../../../../lib/src/app/klp_app.dart#L453)
